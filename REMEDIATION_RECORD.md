...
*Moved verbatim from `REMEDIATION_PLAN.md` lines 428–486 (commit `c1b7d56`) on 2026-09-07. Nothing in this section has been rewritten; corrections, if any, are appended dated notes.*

**Status:** `COMPLETED`

**Why it is its own batch.** Split out of Batch 1.3 on 2026-09-07 so that 1.3's record stays exactly as written. 1.3 is `IMPLEMENTED — TESTING REQUIRED` and blocked on the physical printer; L-21 is neither — it is a code defect, fixable and testable from this machine. The plan's own instruction is what opens it: *Hardware-dependent validation* says to **re-triage every open finding whose severity was discounted for want of an audience the moment an install date exists, starting with L-21**, and `docs/mise-en-service.md` is that date.

### L-21 — `renderReceipt()` centres but never wraps

**Status:** `COMPLETED` · Severity: MEDIUM (unchanged — the row already conditioned it on "once the printer is live"; what changed is that the condition is about to be met) · Category: fiscal document content

**Problem.** `renderReceipt()`'s `center()` helper pads a string to the middle of the paper and returns it unchanged when it is already wider than the paper: `" ".repeat(Math.max(0, Math.floor((w - str.length) / 2))) + str`. The `Math.max(0, …)` clamps the padding, not the string. Any centred field longer than the column count is therefore emitted at full length, and nothing downstream can rescue it — `buildPrintJob()` passes the text through verbatim on purpose (`escpos.ts:203`: *"Nothing here may reflow it, or the printed ticket would stop matching the archived one"*), and the stored `Receipt.content` is what both print routes send.

**Evidence (measured 2026-09-07, read-only against `db/custom.db`).** The live `Setting` rows hold `restaurantAddress = "23 Grande Rue 45210, 45210 Ferrières-en-Gâtinais, France"` — **56 characters** — and `receiptWidth = 80`, which `getSettings()` normalises to **48 columns** (L-20, Batch 3.1d). A ticket rendered from those exact values has **one line over the paper, and it is the address, at 56 columns against 48 — eight characters over.** Every other line is 48 or under. `restaurantName` is 9 characters and `footerNote` is 35, so neither overflows *today*; `settingsSchema` allows 80 and 200 respectively, against a maximum paper width of 48, so both are one operator edit away from the same defect. All 20 archived `Receipt.content` rows are 80 columns wide, which is **L-14** and is not this batch — they are development trading data that Batch 8.0 deletes.

**Location.** `src/lib/services/receipt.ts:56` (the helper), `:67`, `:68`, `:151` (the three fields the finding names), and `:69`, `:70`, `:72` (the phone / SIRET / TVA lines, which are settings-derived and overflow at 32 columns on the schema's own maxima).

**Impact.** Once `printerEnabled` is turned on at the commissioning session, every ticket carries an over-long line. What the printer does with it is the printer's business and cannot be asserted from here — the common ESC/POS behaviour is a hard wrap at the column boundary, which would split `Ferrières-en-Gâtinais` mid-word; some models truncate instead, which would drop the town from the establishment block of a fiscal document. Neither is acceptable and the renderer must not offer the printer the choice.

**Remediation direction.** Wrap at render time, in `renderReceipt()`, because the stored snapshot is the fiscal artifact and the printed ticket must equal it byte for byte. Word-wrap on spaces, hard-break a single token wider than the paper, then centre each resulting line. **A string that already fits must be returned unchanged** — that is what makes the existing snapshot the proof that the fix cannot alter a ticket that was already correct.

**Explicitly NOT in this batch** (safety rules 1 and 10 — recorded in *Newly Discovered Issues*, not fixed here):
- `leftRight()`'s different overflow — `Math.max(1, …)` means an over-wide pair is emitted as `left + " " + right`, past the paper — and the raw `  · choice` / `  + addon` pushes, which are not laid out at all.
- The same three-line `center()` helper copied into `renderDayCloseTicket()` (`day-close-ticket.ts:76`) and `renderTestPage()` (`printer.ts:150`).
- **L-14**, the archived 80-column receipts, which must not be re-rendered.

### Batch 1.3b — Validation Required

- The defect is **measured before it is fixed**, from the production `Setting` values read read-only: the rendered ticket contains a 56-character line at 48 columns, and it is the address.
- Unit tests for the wrap itself: a string that fits comes back **byte-identical**; an over-long string splits on spaces; a single token wider than the paper is hard-broken rather than emitted whole; the empty string and an all-whitespace string terminate and produce no stray line.
- **The existing receipt snapshot is UNCHANGED.** `baseSettings` fits at 42 columns, so the fix must be a no-op for it. A snapshot diff here would mean the change reached a ticket that was already correct — it is not to be amended to go green.
- A **new** snapshot rendered at the production width from the production address, showing the wrap. Added, not substituted.
- The invariant asserted directly across the whole supported range: for every width 32…48, no **centred** line of a ticket rendered from the live settings exceeds the width.
- All three fields the finding names — `restaurantName`, `restaurantAddress`, `footerNote` — covered, each at a length that overflows, and the phone / SIRET / TVA lines covered at 32 columns.
- **Prove the tests fail on the old code.** Revert one property at a time, in both directions, never two together; restore from a copy taken before the revert; and **say which tests pass under no revert and why**.
- **Drive the real route, not the service underneath it.** Ring a real sale through `POST /api/orders` on a scratch copy carrying the production settings, then read `Receipt.content` back out of the scratch database: the stored fiscal artifact must be wrapped, because that stored string is what both print routes send verbatim.
- **Check the artifact on the wire.** Send that stored content through the printer service to a mock ESC/POS printer on loopback, decode the bytes that arrive, and confirm no printed line exceeds 48 characters.
- `bun run test`, `bun run typecheck`, `bun run lint` and `bun run build` all pass, and the suite count moves only by the tests this batch adds.
- Production untouched: `db/custom.db` sha256, size and mtime unchanged; no `-wal`/`-shm` beside it; `db/fiscal-archives/` still absent from the tree; `db/backups/` still nine files.
- **[HW]** A real print of a wrapped ticket at 80 mm, confirming the two address lines land where the rendering says they do. Belongs to Batch 1.3's `[HW]` sign-off at the commissioning session and cannot be satisfied here.

### Batch 1.3b — Status Record

**Status:** `COMPLETED` (the code half of L-21; the **[HW]** print stays with Batch 1.3)
**Completed:** 2026-09-07
**Changes:** One property added to the receipt renderer: **no centred line may exceed the paper**. `wrapToWidth(text, width)` in `src/lib/services/receipt.ts` word-wraps on spaces, hard-breaks a single token wider than the paper, and — the load-bearing half — **returns a string that already fits byte-identical**, which is what makes the pre-existing snapshot the proof that this batch cannot have altered a ticket that was already correct. Every `lines.push(center(x))` in `renderReceipt` now goes through one `pushCentred(x)` helper, so the guarantee is an invariant over the eleven centred lines rather than a list of the three fields somebody remembered; the FACTICE stamp, `Caisse N° 1`, the article count and the software identity are bounded by construction and cost nothing to route through it. `center()` and `leftRight()` are unchanged. **The wrap is at render time on purpose**: the stored `Receipt.content` is the fiscal artifact, both print routes send it verbatim, and `buildPrintJob()` is contractually forbidden from reflowing it (`escpos.ts`) — so a wrap anywhere downstream would make the printed ticket differ from the archived one. Also corrected one stale comment: Batch 3.10 justified putting `Caisse N° 1` in the establishment block by citing L-21 ("this renderer centres but never wraps"), which was true when written and is not now; the placement is restated on its own merits and the superseded reason is kept beside it rather than deleted.
**Files:** `src/lib/services/receipt.ts`, `src/lib/services/receipt.test.ts`, `src/lib/services/__snapshots__/receipt.test.ts.snap` (**+33 lines, −0** — one snapshot added, the existing one untouched).
**Tests:** `bun run test` — **943 pass, 0 fail** (934 + 9), `2 snapshots`. `bun run typecheck`, `bun run lint`, `bun run build` — all PASS. Nine new tests: four on `wrapToWidth` (fits ⇒ byte-identical, including the exact-`width` boundary; splits on spaces with nothing lost — `parts.join(" ")` reads back the whole address; hard-breaks an over-long token and keeps its neighbours on their own lines; terminates on empty and whitespace-only input) and five on the renderer (the 56-character address is gone from 48-column paper but still whole and in order across two lines; a **second** snapshot at the production width and address; the invariant over **every** width 32…48 with all three named fields over-long; the phone/SIRET/TVA lines at 32 columns; and a control that a ticket whose fields already fit is untouched).
**Reverts (eight, one property at a time, both directions).** *Which tests pass under NO revert and why:* the two controls — "returns a string that already fits byte-identical" and "does not touch a ticket whose fields already fit" — survive every un-wrapping revert **by design**; they exist to prove the fix is inert on correct input, not to cover the wrap, and are named as controls rather than counted as coverage. **R1** (wrap removed entirely — the pre-batch renderer): 7 of the 9 new tests fail; **run against the WHOLE suite it is 936 pass / 7 fail, and every one of the seven is a test this batch added** — no pre-existing test anywhere in the 82 files catches the defect, which is why L-21 survived from 2026-09-03 through eight batches that edited this file. The **existing** snapshot passes under R1 and the **new** one fails, which is exactly right in both directions. **R2** (hard-break removed): exactly 1 fail, the hard-break test — isolated. **R3/R4/R5** (un-wrap `restaurantAddress` / `restaurantName` / `footerNote`, one at a time): 3, 1 and 1 fail — each of the three fields the finding names is individually caught. **R6** (un-wrap the phone/SIRET/TVA lines): 1 fail, the 32-column test. **R7** (*other direction* — wrap one column early): 2 fail, proving the exact-`width` boundary assertion is load-bearing. **R8** (*other direction* — over-apply, one word per line even when it fits): 13 fail including **both** snapshots and **eight pre-existing tests** — the receipt snapshot is a real control against over-application, not a coincidence. `receipt.ts` was restored from a copy taken before each revert and `diff`-verified back to the fixed state.
**Real-route validation (2026-09-07).** Driven through the API, not the service. A scratch copy of production under the scratchpad, with **both** `DATABASE_URL` and `HIBAPOS_DATA_DIR` pointed at it; the marker (`admin` renamed to `SCRATCH-1.3B Admin`) read back from the pre-auth `GET /api/auth/profiles` **before the first write**, proving which file the server had open; a known PIN written into the **copy** with the app's own `hashPin`, by a script that refuses any path not under the scratchpad (**demonstrated**: it refused `db/custom.db`). `receiptWidth` deliberately left at the legacy `80` so the L-20 normalisation ran for real. `bun run build` + `bunx next start -p 3079`. A real sale rung through **`POST /api/orders`** — order #21, 2× 5 Nuggets, 11,80 € cash — then `Receipt.content` read back out of the scratch database read-only: **29 lines, widest 48, zero over**, the address on two centred lines. **`POST /api/orders/[id]/print`** → `{"printed":true,"drawerOpened":true}` against a mock ESC/POS printer on `127.0.0.1:9179` that logged every byte; decoding the raw bytes as the printer's own CP1252 gives **30 printed lines, widest 48, zero over, zero stray control bytes**, with `ESC @` / `ESC t 16` leading and `ESC d 4` / `GS V 66 0` / `ESC p 0 25 250` trailing. `POST /api/orders/[id]/reprint` sent a second job — **31 lines, widest 48, zero over** — confirming the `[COPIE]` path prints the stored, wrapped snapshot verbatim. Server and mock both stopped with `taskkill //PID <pid> //T //F`. **Production untouched**: `db/custom.db` still `c9f265163691c93e3402354616b1902f5898006657d47eb8da999e1c6813dceb`, 704 512 bytes, mtime 2026-09-06 17:49:32; no `-wal`/`-shm`; `db/fiscal-archives/` still absent; `db/backups/` still nine files.
**Commit:** `57f109e`
**Notes:**
1. **The measurement came first and it is the whole finding.** Read read-only from the live `Setting` rows: `restaurantAddress` is **56** characters, `receiptWidth` is the legacy **80** which `getSettings()` normalises to **48 columns**, and a ticket rendered from those exact values had **exactly one line over the paper — the address, eight characters over**. `restaurantName` is 9 and `footerNote` is 35, so neither overflows *today*; `settingsSchema` allows **80** and **200** against a maximum paper width of 48, so both are one operator edit away. That is why the fix is an invariant over the centred block and not three patched call sites.
2. **What the printer would have done is not asserted, because it cannot be from here.** The common ESC/POS behaviour is a hard wrap at the column boundary, which would split `Ferrières-en-Gâtinais` mid-word; some models truncate, which would drop the town from the establishment block of a fiscal document. The finding's own wording ("wraps mid-address") assumes the first. Both are unacceptable and the renderer no longer offers the printer the choice — but **only the `[HW]` print settles which one this Sunso does**, and nothing here substitutes for it.
3. **The snapshot was not amended, and that was the point.** `baseSettings` fits at 42 columns, so a correct fix is a no-op for it; the diff on `receipt.test.ts.snap` is **+33 / −0**. R8 proves the control works in the other direction — over-applying the wrap breaks that snapshot and eight other pre-existing tests immediately.
4. **All 20 archived `Receipt.content` rows are 80 columns wide and were not touched.** That is **L-14**, it is unresolved by choice, and an archived receipt is immutable. They are development trading data that Batch 8.0 deletes, so on this install L-14's population is emptied by the reset rather than fixed — worth knowing before anyone spends a batch on it.
5. **Lengths are counted in UTF-16 units**, the unit `center()` and `leftRight()` have always used, so the three agree. For the repertoire this receipt prints that equals the printed column count; a combining mark would over-count by one and wrap a column early, which errs towards fitting the paper. The live address is precomposed — 56 code units, 56 code points.
6. **`wrapToWidth` is exported from `receipt.ts` rather than hoisted into a shared module.** It has one caller. If the batch that fixes L-63 wants it in `day-close-ticket.ts` and `printer.ts` too, moving it is a five-line change; generalising it now for callers that do not exist is what L-07 was about.
7. **Out of scope and recorded, not fixed (safety rules 1 and 10): L-63.** `leftRight()` overflows by a *different* mechanism — `Math.max(1, …)` emits `left + " " + right` past the paper — and the raw `  · choice` / `  + addon` pushes are not laid out at all; the same three-line `center()` helper is copied into `renderDayCloseTicket()` and `renderTestPage()`. Measured with the real catalogue: nothing overflows today at 48 columns (widest item line 24 characters), but `productSchema.name` allows 80, `userSchema.name` 60, and `categoryOptionGroupSchema.choices[].name` has **no maximum at all** — so it is one catalogue edit away, on paper the operator will be printing within days.
8. **The operator's stored address reads `23 Grande Rue 45210, 45210 Ferrières-en-Gâtinais, France`** — the postcode appears twice, once inside the street line. Not touched: it is real settings data and the operator's to correct (warning 4). Shortening it would also have "fixed" the ticket without fixing the renderer, which is why it was never the answer.

---

## Batch 1.3c — The three overflows 1.3b did not own (L-63)

*Moved verbatim from `REMEDIATION_PLAN.md` lines 447–516 (commit `b6171cf`) on 2026-09-07. Nothing in this section has been rewritten; corrections, if any, are appended dated notes.*

**Status:** `COMPLETED`

**Why it exists and why now.** 1.3b fixed the *centred* lines of `renderReceipt()`, which is what L-21 named, and opened **L-63** for the rest (safety rules 1 and 10). The operator asked for it on 2026-09-07, **before the commissioning session**, which is the right moment: the finding is latent on today's catalogue and lives one operator edit away from every ticket, and the printer goes live in that session.

### L-63 — two more ways to overflow the paper, in three renderers

**Status:** `COMPLETED` · Severity: MEDIUM · Category: fiscal document content

**Problem, in three parts.**

*(a)* **`leftRight()` overflows by a different mechanism from `center()`.** `l + " ".repeat(Math.max(1, w - l.length - r.length)) + r` clamps the **gap** to one space, so an over-wide pair is emitted as `left + " " + right`, past the paper. A wrap of the whole string is not the fix: the amount must stay on the right.

*(b)* **The option and add-on lines are not laid out at all** — `  · ${choice}` and `  + ${name} (${price})` are raw `lines.push`, with no width arithmetic anywhere near them.

*(c)* **The same three-line `center()` helper is copied into two more renderers** — `renderDayCloseTicket()` (`day-close-ticket.ts`) and `renderTestPage()` (`printer.ts`) — and the day-close slip is a **fiscal document the operator files with the books**, produced for the first time on the first real trading day. `renderDayCloseTicket()` also has `leftRight()`, so it carries *(a)* as well.

**Evidence (measured 2026-09-07, at 48 columns unless stated).** A receipt built from the schema's own maxima put **four** lines over the paper: the cashier line at **62** (`userSchema.name` allows 60), the item line at **69** (`productSchema.name` allows 80), an option choice at **52** (`categoryOptionGroupSchema.choices[].name` has **no maximum at all**) and an add-on at **49**. The day-close slip put its `restaurantName` line at **61**; its `TVA … (HT …)` line reaches **33 columns at a width of 32** on four-figure daily takings, with no long name involved. The test page put `restaurantName` at **42 on 32-column paper**.

**Latent, and measured to be latent.** Read-only against the real catalogue 2026-09-07: the longest product name is **21** characters (`Fanta Fruit de Dragon`, a 24-character item line), the longest option choice **14**, the longest add-on **15**, the longest user name **14** (`Administrateur`) and `restaurantName` **9**. Nothing overflows today. Nothing but today's data prevents it, and the operator edits products, options and add-ons through Réglages routinely.

**Location.** `src/lib/services/receipt.ts` (`leftRight`, the option and add-on pushes), `src/lib/services/day-close-ticket.ts` (`center`, `leftRight`), `src/lib/services/printer.ts` (`renderTestPage`'s `center`).

**Design decisions taken in this batch, with their reasons.**

1. **Never truncate a label.** BOFiP § 50 lists « détail des articles (**libellé**, quantité, prix unitaire, total HT de la ligne, taux de TVA) » among the data in scope for the fonctionnalité de caisse. An ellipsis on the article's designation would remove required information from a fiscal document to make it fit the paper. Truncation is therefore rejected for every line, not only the item lines.
2. **`leftRight` wraps its LEFT side to the room the right side leaves** (`w − right.length − 1`) and puts the right side, right-aligned, on the **last** line. Each printed article therefore still ends with exactly one amount, which is what keeps a wrapped label distinguishable from a new article. Where the right side alone would fill the paper the left is laid out on its own lines and the right gets one of its own — a guard, not an expected path.
3. **Continuation lines of an option or add-on are indented four**, past the `· ` / `+ ` marker, so a continuation cannot be mistaken for a new choice.
4. **A line that already fits is emitted byte-identically**, exactly as in 1.3b, and for the same reason: it is what makes the untouched snapshots the proof that the batch cannot alter a ticket that was already correct. For `leftRight` the fitting condition and the no-wrap condition are the same inequality — `l.length + 1 + r.length ≤ w` — so this is a property of the construction rather than a special case.
5. **The layout primitives move to one module**, `src/lib/services/ticket-layout.ts`, now that three renderers need them. 1.3b's note 6 said `wrapToWidth` stayed in `receipt.ts` because it had one caller and that moving it would be a five-line change when a second appeared; this is that batch. `printer.ts` importing a layout helper from `receipt.ts` would have been the wrong dependency direction.
6. **The test page's ruler must NOT wrap.** It exists so that a wrapped ruler tells the operator the column count is wrong (L-13). Only its centred lines are laid out.

**Explicitly NOT in this batch** (safety rules 1 and 10): **L-14**, the archived 80-column receipts, which must not be re-rendered; the operator's own catalogue names (**L-39**'s leading spaces, and warning 4 generally); and the stored `restaurantAddress`, whose duplicated postcode is an operator edit in Réglages — prepared and handed over in this batch's record, not written from here.

### Batch 1.3c — Validation Required

- The three overflows are **measured before they are fixed**, from the schema's own maxima, with the line widths recorded; and the real catalogue is measured read-only to establish that all three are latent today.
- Unit tests for the layout module: `leftRight` returns one byte-identical line whenever `l + 1 + r ≤ w`; wraps the left and right-aligns the amount on the last line when it does not; never emits a line over `w`; and never loses a character of either side.
- The degenerate guard: a right side as wide as the paper does not produce a negative wrap width, an empty line, or a loop.
- Option and add-on continuations are indented past the marker, and a continuation can never be mistaken for a new `· ` or `+ ` entry.
- **Both existing snapshots are UNCHANGED** — `receipt.test.ts.snap`'s standard order and 1.3b's production-width ticket both fit, so a diff on either would mean the change reached a ticket that was already correct.
- The invariant asserted for all three renderers across the whole supported range, 32…48: **no line of any kind exceeds the width**, with every field at a length that overflows.
- `renderTestPage`'s ruler is asserted to be **exactly `columns` characters and unwrapped**, at both 32 and 48.
- **Prove the tests fail on the old code.** One property at a time, in both directions, never two together, restored from a copy taken before each revert, and **say which tests pass under no revert and why**.
- **Drive the real routes, not the services.** On a scratch copy: a product renamed to something that overflows, a sale rung through `POST /api/orders`, `Receipt.content` read back from the database; then `POST /api/orders/[id]/print` and `POST /api/print/test` to a mock ESC/POS printer, with the **raw bytes decoded** as the printer's own CP1252 and every printed line measured.
- The day-close slip driven through **`POST /api/fiscal/close-day`** on the scratch copy, with a `restaurantName` that overflows, and the sealed slip's text measured.
- `bun run test`, `bun run typecheck`, `bun run lint` and `bun run build` all pass; the suite count moves only by the tests this batch adds.
- Production untouched: `db/custom.db` sha256, size and mtime unchanged; no `-wal`/`-shm`; `db/fiscal-archives/` still absent; `db/backups/` still nine files.
- **[HW]** A real print of a wrapped item line at 80 mm. Belongs to Batch 1.3's `[HW]` sign-off with 1.3b's, and cannot be satisfied here.

### Batch 1.3c — Status Record

**Status:** `COMPLETED` (the code half of L-63; the **[HW]** print stays with Batch 1.3)
**Completed:** 2026-09-07
**Changes:** The three renderers' column layout moved into one module, `src/lib/services/ticket-layout.ts`, and both remaining overflows were closed there. **`leftRight`** wraps its LEFT side to the room the amount leaves it (`width − right.length − 1`) and right-aligns the amount on the **last** line, so every printed article still ends with exactly one amount — the old one-liner clamped the **gap** to a single space and pushed the amount off the paper. **`marked`** lays out the option (`· `), add-on (`+ `) and change sub-lines, which were raw `lines.push` with no width arithmetic near them, indenting continuations past the marker so a wrapped choice cannot read as a second choice. **`centred`** is 1.3b's wrap plus the padding, and `renderDayCloseTicket` and `renderTestPage` now use it instead of their own copies of the defect. **Nothing is ever truncated**: BOFiP § 50 lists « détail des articles (libellé, quantité, prix unitaire, total HT de la ligne, taux de TVA) » among the data in scope, so an ellipsis on an article's designation would remove required information from a fiscal document to make it fit the paper. **Every function returns a line that already fits byte-identically**, which for `leftRight` is a property of the arithmetic rather than a special case — the label's budget is `width − right − 1`, so it wraps on exactly the inequality that decides whether the pair fits at all. `renderTestPage`'s **ruler is deliberately not laid out**: it exists so that a wrapped ruler tells the operator the column count is wrong for the paper (L-13).
**Files:** `src/lib/services/ticket-layout.ts` (new), `src/lib/services/ticket-layout.test.ts` (new), `src/lib/services/receipt.ts`, `src/lib/services/day-close-ticket.ts`, `src/lib/services/printer.ts`, `src/lib/services/receipt.test.ts`, `src/lib/services/printer.test.ts`, `src/lib/services/daily-close.test.ts`. `wrapToWidth` moved out of `receipt.ts`, which is what 1.3b note 6 said would happen when a second caller appeared; `printer.ts` importing a layout helper from `receipt.ts` would have been the wrong dependency direction.
**Tests:** `bun run test` — **963 pass, 0 fail** (943 + 20), `2 snapshots`. `bun run typecheck`, `bun run lint`, `bun run build` — all PASS. Sixteen tests on the layout module (byte-identical at the boundary; the amount on the last line; no truncation at any width 32…48 with the label read back whole; the non-positive-width guard; an amount as wide as the paper; the hanging indent) and four on the renderers, plus the day-close slip and the test page.
**Reverts (ten, one property at a time, both directions).** *Which tests pass under NO revert and why:* the byte-identical tests — `leftRight` "returns ONE byte-identical line whenever the pair fits", `marked` "is the old raw push, exactly", `centred` "is the old centre-and-push, exactly" — survive every un-laying-out revert **by design**: they exist to prove the change is inert on content that already fits, and are controls, not coverage. **R1** (`leftRight` back to the one-liner): 7 fail across three test files, and **on the WHOLE suite it is 956 pass / 7 fail with every failure this batch's own** — no pre-existing test caught it, the same result 1.3b measured for L-21. **R2** (`marked` back to a raw prefix): 5. **R3/R4** (day-close's own `center` / `leftRight` back): 1 each — the slip catches its own regression. **R5** (test page's own `center` back): 1. **R6** (break at a non-breaking space again): 1. **R7** (*other direction* — amount on the FIRST line): 3. **R8** (*other direction* — continuations flush left): 2. **R10** (*other direction* — centre with `ceil` instead of `floor`): 3, including **both snapshots**, which pins the centring to the exact column. **R9 was a NO-OP and is reported as one**: laying the ruler out through `centred` changed nothing, because `centred(s, w)` is the identity when `s.length === w` and the ruler is built at exactly `columns` — proved at 32, 40 and 48. The "do not lay out the ruler" comment therefore states an **intention** no test can currently distinguish; what *is* pinned is the ruler's own construction (exactly `columns` digits, unwrapped, at both widths).
**Real-route validation (2026-09-07).** Fresh scratch copy under the scratchpad with **both** `DATABASE_URL` and `HIBAPOS_DATA_DIR` overridden; the marker — `admin` renamed to the 41-character `Jean-Baptiste de la Tour du Pin Verclause`, which is also the over-long cashier name this batch needs — read back from the pre-auth `GET /api/auth/profiles` **before the first write**; the prep script refused `db/custom.db` when pointed at it. On the **copy** (never real menu data): a product renamed to 58 characters, an option choice to 47, an add-on to 36 — all inside what `productSchema`, `categoryAddOnSchema` and the unbounded `categoryOptionGroupSchema.choices[].name` already allow. `receiptWidth` left at the legacy `80` so the L-20 normalisation ran for real. Then, through the routes: **`POST /api/orders`** — order #21, 2× the long product — and `Receipt.content` read back from the database at **32 lines, widest 48, zero over**, with the cashier name wrapped, the article label wrapped and its amount alone on the last line, and the option continuation indented four. **`POST /api/orders/[id]/print`** → `{"printed":true,"drawerOpened":true}`; decoding the raw bytes as CP1252 gives **33 printed lines, widest 48, zero over**. **`PUT /api/settings`** saved a 61-character `restaurantName` and a 66-character `footerNote`, and **`POST /api/print/test`** then printed a commissioning page whose name wrapped across two centred lines while the **ruler stayed exactly 48 characters, unwrapped** — the one line that must not be laid out. **`POST /api/shifts/[id]/close`** then **`POST /api/fiscal/close-day`** sealed a real `DailyClose` (2026-08-20, hash `50786aae…`) — the sequencing guard refused 2026-09-06 first, naming 2026-08-20, which is 3.8's `assertDaySequence` working — and the slip rendered from **that sealed row** is clean at 48 and at 32, integrity code intact. Servers stopped with `taskkill //PID <pid> //T //F`. **Production untouched**: `db/custom.db` still `c9f265163691c93e3402354616b1902f5898006657d47eb8da999e1c6813dceb`, 704 512 bytes, mtime 2026-09-06 17:49:32; no `-wal`/`-shm`; `db/fiscal-archives/` still absent; `db/backups/` still nine files.
**Commit:** `e2b14ed`
**Notes:**
1. **The best finding of this batch came from looking at the ticket, not at the tests.** Every unit test passed and the first rendered add-on line read `+ Supplément galette de pomme de terre (1,50` / `€)` — **an amount torn in half on a fiscal document**. The cause: `split(/\s+/)`, and JavaScript's `\s` includes U+00A0. `formatEuro` goes through Intl fr-FR, which puts a NO-BREAK SPACE before the euro sign and a narrow one between the thousands, and those characters exist precisely to forbid that break. Splitting on `\s+` would also have silently replaced the NBSP with a plain space when the parts were re-joined, changing the bytes of the archived `Receipt.content`. The wrap now breaks only at ORDINARY whitespace (`/[^\S   ]+/`, the negated-class trick), pinned by a test.
2. **Both invisible characters are written as escapes, in the source and in the test, and that is deliberate.** The first draft of both landed as literal NBSP characters — invisible in an editor, one careless edit from becoming a plain space, and the test would then have passed while asserting nothing.
3. **My own guard branch was wrong and my own test caught it.** `leftRight` with an amount as wide as the paper returned `justify("", right, width)`, and `justify` forces at least one separator space — it has nothing to separate, so the line came out one character **over** the paper. That is the branch that exists so the function has no undefined behaviour. It now right-aligns without a forced separator.
4. **A pre-existing guard caught a name, not a defect, and was right to.** `role-model.test.ts` (Batch 4.4b) asserts the removed role string appears nowhere in code under `src/`; a test constant called `LONG_CASHIER` tripped it. Renamed to `LONG_OPERATOR_NAME`. The guard was not weakened.
5. **`renderTestPage`'s existing "fits the configured width on both paper sizes" has asserted every line since Batch 1.3, and never reached this defect** — because it passed `"HIBA FOOD"`, nine characters, which fits at 32. A real assertion with an input that cannot fail it. Same shape as T-09's lesson, at a different site.
6. **The layout is a design decision and it is recorded as one, not smuggled in.** Truncation was rejected on BOFiP § 50; the amount goes on the last line so each article still ends with exactly one amount; continuations are indented past the marker. None of it is a business rule, so none of it was marked `REQUIRES DECISION` — but a reader should be able to disagree with it knowing what it was weighed against.
7. **`renderDayCloseTicket` gained `leftRight`'s fix as well as `centred`'s, and it needed it**: with no long name anywhere, `TVA 10 % (HT 1 127,73 €)` plus its amount reaches **33 columns on 32-column paper** on four-figure daily takings.
8. **The operator's stored `restaurantAddress` was still not written from here** (CLAUDE.md rule 3). The corrected value — the duplicated postcode removed — was rehearsed on the scratch copy and is handed over in this batch's note to the operator; at 50 characters it is **still wider than 48-column paper**, which is why the renderer had to be fixed rather than the address shortened.

---
## Batch 1.4b — The launcher's one silent refusal, and the corruption in the runbook it led to (L-65, DOC-16)

*Moved verbatim from `REMEDIATION_PLAN.md` lines 538–631 (commit `ce27fa4`) on 2026-09-07. Nothing in this section has been rewritten; corrections, if any, are appended dated notes.*
...
*Moved verbatim from `REMEDIATION_PLAN.md` lines 559–638 (commit `1dcbe79`) on 2026-09-08. Nothing in this section has been rewritten; corrections, if any, are appended dated notes.*
...
*Appended 2026-09-08, after the move, as the record's own rule requires — the section above is not rewritten.* **The `Commit` field reads `(this commit)`**, written before the commit existed. It resolves to **`1dcbe79`** — refusal 5, § 0b, the `README-windows.md` corrections, the backtick guard, `sliceBlock` and the tests — plus the commit that moved this section here and updated the plan's registers.

*Appended 2026-09-08, later the same day.* **§ 0b gained one more warning, and it was found by explaining the process aloud rather than by reading it.** The operator asked for the whole install walked through end to end; describing what happens to the carried `.env` surfaced what § 0b had not said. **Its `DATABASE_URL` is an absolute path into the development machine's OneDrive-synced project folder**, so on the till it resolves to nothing — and it needs rewriting **twice**, once before `bun run build` (at the clone) and once after `install-windows.ps1 -Apply` (at `C:\HibaPOS\data`), because the database moves between those two points. § 0b said to put the file in place and § 1 said to edit `.env` after the move; **neither said the value arrives wrong.** The failure is at least loud and self-naming: refusal 1 prints the path it tried, and a `C:\Users\…\OneDrive\…` path in a restaurant's `server.log` is this mistake and no other. Added to § 0b as a warning block plus a checkbox, and to `README-windows.md`'s Step 0. **The lesson is the batch's own, one turn later:** DOC-17 existed because every step was known to whoever had done it and written for nobody who had not — and the *first* attempt at writing it down still assumed the reader would notice a stale path. Walking a procedure out loud is a cheaper test of it than re-reading it.

*Appended 2026-09-08.* **One of note 7's two open questions is answered: the till has internet** (the operator, asked directly). So § 0b's path is `bun install` on the machine, and the 880 MB `node_modules` transfer stays written down only as a contingency for a connection that is down on the day. **The other stands**: how long `next build` takes on all-in-one hardware is not measurable from here, and it is the step most likely to make the first hour feel slow.
---
...
*Moved verbatim from REMEDIATION_PLAN.md (commit ecb619d) on 2026-09-06.*

## Batch 2.5 — The restore could not complete on Windows (L-61, L-62)

**Status:** `COMPLETED` · **Opened and completed:** 2026-09-06 · **Findings:** **L-61 (HIGH, closed)**, **L-62 (closed)** · **Decisions:** none.

Batch 8.2's rehearsal drove `POST /api/backups/[id]/restore` on a copy of
production and it failed with `EPERM`, reproducibly. 8.2 is a proof batch and
could not fix it (safety rule 10), so it recorded the failure and stopped. This
is the fix. **Stage 2 reopens for it**, because the restore is 2.1's subject.

### What was actually wrong, and how it was found

The swap is `fs.rename(staged, dbPath)` after `await db.$disconnect()`. Windows
refuses to rename over a file another handle still has open. **The first
diagnosis — "`$disconnect()` does not release the handle" — was wrong**, and
measuring it is what corrected it: in an isolated process the handle is released
**4–9 ms** after `$disconnect()` resolves, under rollback journal and WAL alike,
with or without `VACUUM INTO`, with or without a transaction. Four probe
configurations, all released.

So the difference was the server. `src/lib/db.ts` cached its PrismaClient on
`globalThis` **only when `NODE_ENV !== "production"`** — the standard Next.js
hot-reload guard. Next bundles server code per entry point, so in production
`instrumentation.ts` gets one module instance of `db.ts` and the route handlers
get another. **Instrumented and counted on the real production build: one
process, two `PrismaClient` constructions** — one before `Ready` (the startup
pragma hook) and one on the first request. Two clients, two open handles;
`restoreBackup` disconnects its own and the other one holds the file for the
life of the process.

### The fix

1. **`src/lib/db.ts` caches unconditionally.** One client per process. This is
   the root cause and the one-line change that removes it — and it is correct
   on its own terms: a second client is a second connection pool nobody asked
   for.
2. **`renameWithRetry`** replaces the bare rename: `EPERM`, `EACCES` and
   `EBUSY` are retried for up to 5 s at 100 ms intervals, anything else is
   raised at once. Belt to the braces — the handle is released a few
   milliseconds *after* `$disconnect()` resolves rather than synchronously, and
   a loaded machine can take longer. It is safe to retry because `rename` is
   atomic: each attempt either completes or changes nothing. On exhaustion it
   raises a French message that says **the data was not modified** and points at
   `scripts/decrypt-backup.ts`.
3. **L-62:** the staged database and the decrypted media archive are unlinked in
   a `finally`, on every exit.

### Validation Required

- [x] `bun run test`, `typecheck`, `lint`, `build` clean.
- [x] Revert protocol, one property at a time, both directions.
- [x] **The restore that failed in 8.2 now succeeds, driven through the same route on the same shape of scratch install, in WAL — the mode it was reproduced under.**
- [x] After it: the sale is gone, the grand total and the chain are back to the backup's state, all four chains verify, the deleted image serves again, and a `RESTAURATION` event is journalled.
- [x] Nothing is left in the data directory afterwards.
- [x] Production untouched.

### Status Record

**Status:** `COMPLETED` · **Completed:** 2026-09-06 · **Commit:** `52c66c0`
**Findings:** L-61 (**closed**), L-62 (**closed**). **Unblocks Batch 8.2's V-06.**

**Changes.** `src/lib/db.ts`: `globalForPrisma.prisma = db;` unconditionally, with the reasoning written where the next person will meet it. `src/lib/services/backup.ts`: `renameWithRetry(src, dest, deps)` exported, with `budgetMs`, `sleep` and **`rename` injectable** — the last one deliberately, so the retry logic is tested by driving the shipped function rather than a copy of it; `restoreBackup` calls it instead of `fs.rename`; and the `finally` around the swap unlinks both staged artifacts. `src/lib/services/restore-swap.test.ts` is new.

**Tests.** **934 pass, 0 fail** (923 before). 11 new cases. **Ten one-property reverts, every one caught, nothing passed under no revert.** They cover the root cause (R1 restores the `NODE_ENV` gate — L-61 itself; R2 drops the cache), the swap (R3 goes back to a bare `fs.rename`; R4 retries everything; R5 retries only `EPERM`; R6 drops the French message; R7 zeroes the budget) and the cleanup (R8, R9, R10). **R3 exists because the first draft had a hole**: every retry case passed while nothing asserted that `restoreBackup` *calls* it — a perfectly tested function nothing used.

**End to end, on the failure that started this.** Same scratch shape as 8.2, WAL, real routes: backup created, a sale rung, an image deleted, then `POST …/restore` → **`{"ok":true, images:{restored:144}, rewound:true, rewind:{receipt:1,fiscalEvent:1}}` in 2,6 s**, where 8.2 got `EPERM`. Afterwards: events **3 → 2 + 1 `RESTAURATION`**, grand total **6530 → 5480**, all four chains `ok`, uploads **138 → 139** with `Burger.png` served at **74 680 bytes** — its exact pre-deletion size — and the data directory holding only `backups custom.db fiscal-archives`, with **no `.restore-staged` and no plaintext `.tar.gz`**.

**Notes.**

1. **The first diagnosis was wrong and measurement is what caught it.** 8.2 recorded "`$disconnect()` does not release the Windows file handle", inferred from an exclusive-open test against the running server. That test proved *a* handle was held, not *whose*. An isolated probe released it in 4–9 ms in every configuration — WAL, rollback, `VACUUM INTO`, transactions — which is what turned the question from "does Prisma release handles" into "how many clients are there". The answer was two.

2. **A production-only guard hid it.** `NODE_ENV !== "production"` meant the defect could not appear in dev, in `bun test`, or in any unit test — only in a production build, and only in the one operation that needs the file closed. That is why six batches of tests over the backup subsystem never saw it, and why driving the real route on a real copy did.

3. **The retry is not the fix, and should not be read as one.** With two clients it would have retried for five seconds and still failed. It is there for the milliseconds between `$disconnect()` resolving and the OS releasing, and for a machine slower than this one.

4. **`rename` is injected for the tests, and that is a deliberate design choice.** The alternative — a retry loop reimplemented in the test file — would let every case pass while the shipped code was broken. `resolvePrinter(deps: { transport? })` set the precedent in Batch 1.3.

5. **What this does NOT fix.** The restore still requires the app to be running, so a database so broken the app will not start is still recovered by hand with `scripts/decrypt-backup.ts` — which is what that tool exists for and what its help text describes. **And it changes nothing about L-46**: the three real backups on production remain unrestorable because they predate seven fiscal tables. A fresh backup at commissioning is still required.

6. **Production untouched.** `db/custom.db` sha256 `c9f265163691c93e3402354616b1902f5898006657d47eb8da999e1c6813dceb`, 704 512 bytes, mtime 2026-09-06 17:49:32 — identical throughout. No `-wal`/`-shm`. Every write went to the scratchpad. A temporary `_probe61.ts` was used inside the repo to count client constructions, following Batch 0.2's `_baseline_check.ts` precedent, and deleted after each run; `git status` is clean of it. Servers on 3075 and 3076 stopped by PID, and a stale `next build` lock from an earlier run was cleared.
...
*Moved verbatim from `REMEDIATION_PLAN.md` lines 979–1044 (commit `b8ccb53`) on 2026-09-07. Nothing in this section has been rewritten; corrections, if any, are appended dated notes.*

**Status:** `COMPLETED` — reopened Stage 3 a fourth time and closed it the same day.

**Why now.** Batch 3.10 closed L-58's label half and deliberately left this one, recording that it "is the first thing to decide in any batch that opens a migration on `OrderItem`". The operator asked for it on 2026-09-07, **before the commissioning session** — which is the right moment and very nearly the last one: an `OrderItem` column is free while the only rows are development trading that Batch 8.0 deletes.

### L-58 — no per-line HT total is stored

**Status:** `COMPLETED` · Severity: LOW · Category: fiscal data in scope

**Problem.** BOFiP § 50 lists the data in scope for the fonctionnalité de caisse line by line, and among the article detail is **« total HT de la ligne »**. `OrderItem` stores `unitPrice` and `lineTotal` — both **TTC** — and `vatRate`. The HT figure exists nowhere: it is computed when the ticket's VAT block is rendered and thrown away.

**And it is not simply derivable, which is the whole finding.** The HT is taken on the line's net **after** the order's discount has been apportioned across the lines by largest remainder (`money.ts:120`). So a reader holding the stored columns cannot divide `lineTotal` by `1 + rate/100` and arrive at the figure the till used — they have to re-implement the apportionment. That is what makes "derivable" a weaker claim here than it sounds.

**Re-read of § 50, as the row instructs.** `docs/conformite-isca-recherche.md` § 9.6 renders § 50 as *« toutes les données liées à la réalisation d'une transaction … ainsi que … à la réception (immédiate ou attendue) du paiement »*, listed line by line, article detail including « total HT de la ligne ». **It governs the data the caisse records, not what the ticket prints** — § 30 and § 40 frame the fonctionnalité as memorising and recording. **The research's own caveat stands and is repeated here: that list is the research pass's rendering, not a verbatim quotation, and § 50 should be re-read at source before any attestation rests on it.**

**Consequence for this batch:** store the figure; **do not change the receipt**. The ticket already prints HT per rate (M-06) and no source read requires a per-line HT on paper. Changing a fiscal document to satisfy a data requirement would be a second, unasked change to an immutable artifact.

**Location.** `prisma/schema.prisma` (`OrderItem`), `src/lib/services/checkout.ts:168-213`, `src/lib/services/fiscal.ts` (the annual archive serialises `OrderItem` rows whole).

### Design decisions taken in this batch, with their reasons

1. **Two columns, not one: `lineNetTotal` and `lineHt`, both `Int?` cents.** The HT alone satisfies § 50's list, and would leave the same complaint one step further on: a reader still could not *check* it without re-running the apportionment. Storing the net TTC the HT was taken from makes the row self-verifying — `lineHt = round(lineNetTotal / (1 + vatRate/100))`, a division anyone can do. The VAT is `lineNetTotal − lineHt` and is not stored, because subtraction is not an algorithm.
2. **Nullable, and existing rows are NOT backfilled.** Null means "this sale was recorded before the figure was". Backfilling would mean computing figures for completed sales and writing them into the fiscal record as though the till had recorded them — inventing fiscal data, which is what this application forbids everywhere else. It is also unnecessary: the 20 affected rows are development trading that **Batch 8.0 deletes**. Same treatment, and the same reason, as L-57's `perpetualSalesTotal`.
3. **The values are written inside the checkout transaction, from the numbers the transaction already computed.** `lineNets[idx]` is the apportioned net; the HT is `splitVat(lineNets[idx], vatRate).ht` — the same helper the VAT breakdown uses. **No second implementation of the split**, and therefore no way for the stored figure to disagree with the `vatTotal` sealed on the same order.
4. **The annual archive's schema version goes 5 → 6.** `buildAnnualArchive` selects `items: true`, so the new columns appear in the file automatically. Free today: **zero archives exist** on production. A reader keyed on the number must not expect these fields in a 5.
5. **No third journal payload vintage, and that was checked rather than assumed.** `buildVentePayload` carries `itemCount` and no line detail (`sale-journal.ts`), so nothing hashed changes and the two-vintage rule in *Open Threads → D* still holds.

### Batch 3.11 — Validation Required

- Unit tests that a checkout stores `lineNetTotal` and `lineHt` on every line, and that **`Σ lineNetTotal` equals `order.total`** exactly — the apportionment property, now observable in the database rather than only in the VAT block.
- **The stored HT agrees with the order's sealed `vatTotal`**: `Σ (lineNetTotal − lineHt)` equals `order.vatTotal`, for a single-rate order and for a mixed-rate one. This is the assertion that would catch a second implementation of the split.
- A **discounted** order: the stored nets are the apportioned ones, not `lineTotal`, and they still sum to the discounted total.
- The zero-total (« Offert ») order: nets and HT are 0, not null, and nothing divides by zero.
- Existing rows keep `null`, and null is distinguishable from 0.
- The archive carries the fields and its `version` is 6.
- **The receipt is unchanged** — both snapshots byte-identical.
- **Prove the tests fail on the old code.** One property at a time, both directions, restored from a copy taken before each revert, and say which tests pass under no revert and why.
- **Migration rehearsal with a fingerprint diff** (*Methods*): out-of-band snapshot in `../db-snapshots/`, apply to a copy, diff every fiscal table before and after — row counts, `FiscalCounter`, `GrandTotal`, every event hash, both sealed Z rows, order lines, `integrity_check`, foreign-key errors, column order. **Only the two new columns and the `_prisma_migrations` row may differ.**
- **Drive the real route**: a sale through `POST /api/orders` on the migrated scratch copy, then read the two columns back out of the database.
- `bun run test`, `bun run typecheck`, `bun run lint`, `bun run build` all pass; the suite count moves only by the tests this batch adds, and `readme-counts.test.ts` keeps the README honest.
- Production untouched: sha256, size and mtime unchanged; no `-wal`/`-shm`; `db/fiscal-archives/` still absent; `db/backups/` still nine files.
- **[OPERATOR]** `bunx prisma migrate deploy` against production — prepared, rehearsed and handed over. **Claude cannot run it** (CLAUDE.md rule 4).

---

### Batch 3.11 — Status Record

**Status:** `COMPLETED`
**Completed:** 2026-09-07 — code, rehearsal, **and the operator's `prisma migrate deploy`, verified read-only the same day.**
**Changes:** `OrderItem` gains **`lineNetTotal`** and **`lineHt`**, both `Int?` cents. `lineNetTotal` is the line's TTC **after** the order's discount has been apportioned — `lineNets[idx]`, the array the checkout transaction already computes — and `lineHt` is `splitVat(lineNetTotal, vatRate).ht`, the **same helper the VAT breakdown uses**, so there is no second implementation of the split and the stored figure cannot drift from the `vatTotal` sealed on the same order. Both are written inside the checkout transaction beside the line they describe. The annual archive's schema `version` goes **5 → 6**: `buildAnnualArchive` selects `items: true`, so the columns reach the file without the archive code naming them. **The receipt is deliberately unchanged** — § 50 governs the data the caisse records, not what the ticket prints, and the ticket already carries HT per rate (M-06).
**Files:** `prisma/schema.prisma`, `prisma/migrations/20260907114611_order_item_line_ht/migration.sql` (new), `src/lib/services/checkout.ts`, `src/lib/services/fiscal.ts`, `src/lib/services/line-ht.test.ts` (new), `src/lib/services/archive.test.ts`, `README.md`.
**Tests:** `bun run test` — **976 pass, 0 fail** (967 + 9: eight new in `line-ht.test.ts`, one in `archive.test.ts`). `bun run typecheck`, `bun run lint`, `bun run build` — all PASS. The assertion that carries the batch is **`Σ (lineNetTotal − lineHt) === order.vatTotal`**, at one rate and at two; beside it, `Σ lineNetTotal === order.total` makes the apportionment observable in the database rather than only inside the VAT block, and a discounted order pins that the stored net is the **apportioned** figure and not the gross `lineTotal`. `readme-counts.test.ts` caught the README on its first real batch — 967 against a computed 976 — which is what it was built for.
**Reverts (seven, one property at a time, both directions).** *Which tests pass under NO revert and why:* one — "a row written before this batch keeps null" — survives R1 **by design**: with nothing stored, every row is already null and the assertion passes trivially. It is a control for the not-backfilled decision, not coverage of it, and is named as such. **R1** (store nothing — the pre-batch checkout): 7 of the 8 new tests fail. **R2** (store the gross `lineTotal` as the net — the finding itself): 3, the discount-sensitive ones. **R3** (a *second* implementation of the split, `Math.floor` instead of `splitVat`): 4, **including both agreement assertions** — the one revert this batch most needed to be caught, and it was. **R4** (store the HT but not the net, so the row stops being checkable by division): 7. **R5** (archive version left at 5): 2, one of them a **pre-existing** test. **R6** (*other direction* — `lineNetTotal || null`, collapsing a recorded 0 into "never recorded"): 1, isolated on the zero-total case. **R7** (*other direction* — apportion over the **gross** subtotal instead of the discounted total): 5, **including two pre-existing T-05 tests** that have guarded that arithmetic since Batch 6.1. *(A first attempt at R6 wrote `lineNets[idx] || 0`, which is a no-op on a value that is always a number; it was replaced rather than reported as a pass.)*
**Migration rehearsal (2026-09-07).** Out-of-band snapshot first, in the **sibling** `../db-snapshots/custom.db.pre-3.11.2026-09-07T11-53-44Z` (sha256 `58781596…`, identical to production at the time). The migration was **created against a scratch copy**, never against production — `DATABASE_URL` and `HIBAPOS_DATA_DIR` both overridden — and is two `ALTER TABLE … ADD COLUMN` statements with no table rebuild, which is the shape that cannot lose rows (the baseline migration's 30 `CREATE TABLE`s are what L-46 came from). Applied to a copy and diffed on a 305-line fiscal fingerprint covering every fiscal table's rows and column order, the counters, the grand total, every event hash, both sealed Z rows, every order line, `integrity_check`, `foreign_key_check` and the migration checksums. **After stripping the two new columns, exactly ONE line differs, and it is the `_prisma_migrations` row** — which is the only difference *Methods* permits.
**Real-route validation (2026-09-07).** On the migrated copy, marker (`SCRATCH-3.11 Admin`) read back from the pre-auth `GET /api/auth/profiles` before the first write; the prep script refused `db/custom.db` when pointed at it. A **discounted, mixed-rate** sale through `POST /api/orders` — 2× 5 Nuggets at 10 % and a Coca at 5,5 %, subtotal 1330, discount 137, total 1193 — then the columns read back read-only: nets **1058 + 135 = 1193 = `order.total`**; VATs **96 + 7 = 103 = `order.vatTotal`**; each line's HT reproduced by plain division. **82 legacy rows still `NULL`, 2 new rows filled** — null and zero distinguishable in the live data, not only in a test. Server stopped with `taskkill //PID <pid> //T //F`. **Production untouched**: `db/custom.db` still `58781596521e95c030c3535789479b57d05f5bbd61400679ebada10c57231c40`, 704 512 bytes, mtime 2026-09-07 12:40:47; no `-wal`/`-shm`; `db/fiscal-archives/` still absent; `db/backups/` still nine files.
**Migration applied by the operator, 2026-09-07, and verified read-only** (`bun:sqlite` `readonly: true`, no Prisma, no server — *Methods*; Batch 8.1's method). All four of runbook § 1b's checks pass. **(1)** `prisma migrate status` — *Database schema is up to date*, **10 migrations**, latest `20260907114611_order_item_line_ht`; the two columns are on `OrderItem`. **(2)** Counters unmoved — **20 / 3 / 2 / 2** — and so is `GrandTotal` in every field (5 480 / 2 / 502 / 5 480 / 0). **(3)** **All four chains `ok`**, recomputed with the application's own `verifyEvents` and `verifyCloses` rather than a reimplementation: 2 events checked, zero closes of any kind. **(4)** **82 existing order lines read `NULL` in both columns and 0 are filled** — not backfilled, exactly as decided. Beside those: `integrity_check` **ok**, `foreign_key_check` **empty**, journal byte 18 still `1`, every row count unmoved, and **the fingerprint diff against the pre-migration snapshot is ONE line — the `_prisma_migrations` row**, which is what the rehearsal predicted. *(The third time a migration in this project has done exactly that: 5.7a, 3.8, 3.11.)* Production sha256 `c9f26516…` → `58781596…` (the operator's Réglages save) → **`d09369c09dd9b4515c78af31118e8dc47516e74c0ab4d41e2bc4d93c5e54b16b`**, 704 512 bytes, mtime 2026-09-07 15:33:56. The only thing outside the fingerprint that moved is **one `AuditLog` row, a `SESSION_LOCKED`** at 14:32 — the app's own 30-minute inactivity lock on the operator's session, unrelated to the migration and checked rather than assumed.
**Commit:** `0996622` (code, migration and rehearsal) + `b8ccb53` (sha) + this closure.
**Notes:**
1. **Two columns, not one, and that is the decision this batch exists to make.** The HT alone satisfies § 50's list and would have left the finding's actual complaint one step further on: a reader still could not *check* the figure without re-implementing `apportion`. Storing the net it was taken from makes the row self-verifying by division. The VAT is `net − HT` and is not stored, because subtraction is not an algorithm.
2. **Existing rows are NOT backfilled, deliberately.** Null means "recorded before the till kept this figure". Computing HT figures for completed sales and writing them in would be inventing fiscal data — the thing this application refuses everywhere else. It is also unnecessary: all 82 affected rows are development trading that **Batch 8.0 deletes**. Same treatment and same reason as L-57's `perpetualSalesTotal`.
3. **No third journal payload vintage, and that was checked rather than assumed.** `buildVentePayload` (`sale-journal.ts`) carries `itemCount` and no line detail, so nothing hashed changes and *Open Threads → D*'s two-vintage rule still holds. This was the first thing verified, because it is the one way a column on `OrderItem` could have been expensive.
4. **The archive version was free, and it is the last of the four that was.** 5 → 6 costs nothing because **zero archives exist on production** — re-verified read-only 2026-09-07. The pin in `archive.test.ts` is what surfaced the change at all: the archive code never names these columns, so without that assertion the file's shape would have moved silently.
5. **The archive suite had never exercised an archived order LINE.** `anOrder` creates orders with no `OrderItem` rows, so `parsed.orders.flatMap(o => o.items)` was empty and the new assertion failed on its first run for that reason rather than a defect. The test now seeds its own line. Small coverage gap, closed in passing.
6. **§ 50 was re-read before acting, as L-58's row instructs**, and the research's own caveat is repeated rather than dropped: `docs/conformite-isca-recherche.md` § 9.6 **renders** the list, it does not quote it verbatim, and § 50 should be re-read at source before any attestation rests on it. What the reading settles for this batch is narrower and sufficient: § 30 and § 40 frame the fonctionnalité as *memorising and recording*, so the requirement is about data held. **The ticket was therefore not touched** — changing an immutable fiscal document to satisfy a data requirement would have been a second, unasked change.
7. **`bunx prisma generate` failed `EPERM` and was correct to.** The operator's own server was live on port 3000 holding `query_engine-windows.dll.node` (warning 9's exact symptom). **It was not killed** — Claude stops only servers Claude started. The client types regenerated anyway; only the engine binary rename failed, and the engine is version-tied to Prisma rather than to the schema, so an additive column change needs no new binary. Typecheck, the suite and the real route all confirmed it.

---

## Batch 3.12 — VAT does not vary by order type, and the drinks are rated as if it did (L-68)

**Status:** `COMPLETED` · **Opened and completed:** 2026-09-09, on the operator's ruling, out of the combo analysis that preceded it.

**Stage 3 reopens for a fourth time.**

### The finding

**The operator's ruling, 2026-09-09, and it is the specification:** a sealed bottle or can is **10 % sur place** and **5,5 % à emporter et en livraison**. Everything else in this catalogue is 10 % whatever the order type — hot food for immediate consumption. **It is not a blanket takeaway rule**; it is a property of the item, and that is what makes it a per-category setting rather than a global switch.

**What the code does instead.** `resolveVatRate(product)` — `services/pricing.ts:56` — takes the product and nothing else. Nothing anywhere links VAT to `orderType`: not `pricing.ts`, not `checkout.ts`, not `money.ts`. **The rate is therefore the same for all three order types.**

**What the catalogue does.** **17 products are rated 5,5 % unconditionally** — 4 in *Bouteilles*, 13 in *Canette*, both categories carrying `vatRate = 5.5`. Every other product resolves to 10 %.

**So every one of those 17 drinks sold SUR PLACE books at 5,5 % where the ruling says 10 %** — an under-declaration, across a whole product class, on every dine-in drink. It is not confined to the combos that exposed it.

**Nothing wrong has been recorded.** The application has never traded: the trading data on production is the developer's own and § 6 deletes it before the first real sale, and the three-day client trial runs with the **old till as the system of record and FACTICE on**. This is not a correction of history — it is a defect to fix before real trading, and the deadline is the end of that trial, not the end of the feature list.

**Why the fix is small, which was not obvious from the outside.** `resolveVatRate` has **exactly three call sites**: `effectiveVatRate` in the two catalogue product routes, which is **display only**, and `orders/route.ts:251`, which is **the fiscal one**. That line already sits in a handler where `orderType` is destructured (`:178`) and already passed to `computeLinePricing(itemIntent, product, orderType)` (`:234`) — an exact precedent for the signature this batch needs. **The rate is snapshotted server-side on purpose** (L-16/L-17, Batch 3.1c), so the client's `CartItem.vatRate` is display-only and cannot influence what is booked.

### L-68 — the reduced rate is not applied by order type

| Item | Status | What |
|---|---|---|
| **L-68a** | `IMPLEMENTED` | A second rate on the category — the à-emporter rate — beside the existing one. **Nullable, and null means "the same rate whatever the order type"**, so every product that is 10 % today is untouched by construction. Migration required. |
| **L-68b** | `IMPLEMENTED` | `resolveVatRate(product, orderType)`: `DINE_IN` takes the existing rate; `TAKEAWAY` and `LIVRAISON` take the à-emporter rate when one is set, and fall back to the existing rate when it is not. |
| **L-68c** | `IMPLEMENTED` | The fiscal call site passes `orderType`. The display call sites resolve for the cart's current order type, so what the cart shows agrees with what will be booked. |
| **L-68d** | `DEFERRED` | **Dropped after measurement, not written.** Nothing displays VAT before checkout — not the cart panel, not the payment dialog — and `orders/route.ts` rebuilds the rate from the product and ignores what the client sends. So this would have changed no figure anyone can see and none that is booked. Record → note 6. |
| **L-68e** | `IMPLEMENTED` | The category editor can set the à-emporter rate. |
| **L-68f** | `COMPLETED` (operator, 2026-09-09) | **[OWNER]** Set 10 % / 5,5 % on *Bouteilles* and *Canette* — **a SWAP, not an addition**: both carry `vatRate = 5.5` today and that column now means *sur place*, so setting only the new rate would leave dine-in wrong. A write to real menu data, so it is the operator's (warning 9), **after** the migration is applied. |

### Validation Required

1. **Unit tests on `resolveVatRate`** across all three order types, with the à-emporter rate set and absent, and at product / category / parent-category level — including the fallback that keeps every 10 % product at 10 % under every order type.
2. **A route-level test on `POST /api/orders`** proving the booked `OrderItem.vatRate` follows the order type. **The revert protocol must fail this, not only the unit tests** — that is Batch 5.8's lesson, where the rule was proved and the wiring was not.
3. **The revert protocol** — *Methods → Prove the test fails on the old code*: one property at a time, both directions, and say which tests pass under no revert and why.
4. **Migration rehearsal with a fingerprint diff** — *Methods*: snapshot into `../db-snapshots/`, a **SIBLING** of the repo; apply to a copy; diff row counts, `FiscalCounter`, `GrandTotal`, every event hash, both sealed Z rows, order lines, `integrity_check`, foreign-key errors and column order. **Only the new column and the `_prisma_migrations` row may differ.** Then hand the operator the exact `bunx prisma migrate deploy`.
5. **A worked example on a scratch copy**, end to end: the same can rung `SUR PLACE` and `À EMPORTER`, with the ticket's *Détail TVA* showing 10 % on one and 5,5 % on the other.
6. `bun run test`, `bun run typecheck`, `bun run lint`; README count re-pinned if it moves.
7. **Production untouched by this session**, demonstrated rather than asserted.

### Deliberately NOT in this batch

- **The combo feature.** It waits on this and not the other way round: a menu price cannot be split across rates until the rates themselves are right.
- **Re-rating the 17 drinks.** That is L-68f, the operator's, after the migration.
- **Alcohol at 20 %.** The restaurant does not sell it, and inventing the case would be guessing.
...
**Changes.** `Category` gains **`vatRateTakeaway Float?`** — one nullable column, migration `20260909101500_category_vat_rate_takeaway`, a single additive `ALTER TABLE`. **Null means "the same rate whatever the order type"**, which is the property that made the column safe to add: every category in the catalogue resolves exactly as before until one is set. `resolveVatRate(product, orderType)` resolves **one governing level and reads both rates off it** — `DINE_IN` takes `vatRate`, `TAKEAWAY` and `LIVRAISON` take `vatRateTakeaway` and fall back to `vatRate` when it is unset. The order type defaults to `DINE_IN`, deliberately: defaulting to the reduced rate would let a caller that forgot quietly under-declare. **`orders/route.ts:251` — the one call site that decides what is booked — now passes `orderType`.** The two `effectiveVatRate` call sites are display only and unchanged. `categorySchema` gained the field behind the **same** `vatRateField` guard as `vatRate`, so a bad value is refused by one rule rather than two that could drift; both write paths spread `parsed.data`, so nothing else in the routes changed. The category editor gained a second selector, and **two labels reading « sur place et à emporter » were corrected** — the operator's ruling falsified them.

**Files.** `prisma/schema.prisma`, `prisma/migrations/20260909101500_category_vat_rate_takeaway/migration.sql` (new), `src/lib/services/pricing.ts`, `src/app/api/orders/route.ts`, `src/lib/validation.ts`, `src/app/api/catalog/categories/route.ts`, `src/types/api.ts`, `src/features/catalog/categories-view.tsx`, `src/features/catalog/products-view.tsx`, `src/lib/services/vat-inheritance.test.ts` (+8), `src/app/api/orders-vat-by-order-type.test.ts` (new, 5), `README.md`, `src/lib/plan-freshness.test.ts`.

**Tests.** 1044 pass, 0 fail — 13 added. **The revert protocol is why there are two test files.** R1 — the route stops passing `orderType`, the rule left correct — **all 20 unit tests pass and 2 of the 5 route tests fail.** That is Batch 5.8's lesson reproduced on purpose: a rule can be right while nothing consults it, and `OrderItem.vatRate` is what every report reads. R2 (takeaway branch removed) → unit 18/2 **and** route 3/2. R3 (the two rates resolved independently instead of from one governing level) → unit 18/2. R4 (default flipped to `TAKEAWAY`) → unit 19/1. Restored → 20 and 5. **Three tests pass under every revert and are named rather than counted**: the `DINE_IN` case, which is correct either way; the pizza control, which pins that a category with no à-emporter rate is untouched; and the client-tamper assertion, a regression pin on L-16/L-17's server-side snapshot.

**Notes.**

1. **The ruling refines Batch 3.1c rather than contradicting it.** 3.1c moved the 17 cans and bottles from 10 % to 5,5 % on the operator's determination; that determination was **incomplete, not wrong** — it is the container *and* the consumption that decide. The tests live in 3.1c's own file for that reason.
2. **The migration rehearsal found nothing, which is the result it should produce.** Snapshot to `../db-snapshots/pre-3.12-20260909T001857.db`, a **sibling** of the repo; applied to a copy; fingerprint diffed across 31 tables. **Exactly three differences: the new `Category` column at index 10, `_prisma_migrations` 10 → 11, and the new migration name.** No row count, no `FiscalCounter`, no `GrandTotal`, no event hash, no sealed Z row moved. `integrity_check` ok, 0 foreign-key errors, and every category kept its `vatRate` with `vatRateTakeaway` null.
3. **THE WORKED EXAMPLE CAUGHT A WRONG RESULT, and the catch is the point.** Rung on a copy of the real catalogue, the first pair came back **10 % on both** order types. The cause was not the code: `.next` had been built at 22:27 and `pricing.ts` edited at 00:15, so the server was running the **pre-fix** build. Rebuilt and re-rung, and the four sales on identical data read: **#30 DINE_IN 10 %, #31 TAKEAWAY 10 % (pre-fix — the defect itself, on production data), #32 DINE_IN 10 %, #33 TAKEAWAY 5,5 %** with `lineHt` 136 → 142. *Methods → Run the thing*: the arithmetic said the takeaway VAT should be 8 and the response said 14, and only the number exposed a stale build that every test had passed around.
4. **A deploy of this needs `bun run build`.** Note 3 is that sentence with a cost attached. `CHANGES-LOG` rule 8 already says it; this batch demonstrates the failure it prevents.
5. **`prisma generate` failed `EPERM` and it did not matter — checked, not assumed.** The operator's own dev server on port 3000 holds `query_engine-windows.dll.node`, exactly as warning 9 describes; Claude does not kill the operator's processes. Only the native engine **rename** failed. The TypeScript client had already regenerated (59 references to the new field), and the engine binary is schema-agnostic, so the field was usable throughout. **A session that hits this on a machine with no running server should still expect it to matter.**
6. **`CartItem.vatRate` is vestigial and was left alone.** Nothing displays VAT before checkout — not the cart panel, not the payment dialog — and `orders/route.ts` rebuilds the rate from the product and ignores what the client sent. So the planned item L-68d (re-resolving the cart's VAT on an order-type change) **would have changed no figure anyone can see and none that is booked**, and it was dropped rather than written. Measured before dropping.
7. **L-68f is a SWAP, not an addition.** *Bouteilles* and *Canette* carry `vatRate = 5.5` today, and that column now means **sur place**. Setting only the new rate would leave dine-in wrong at 5,5 %. Both categories need `vatRate = 10` **and** `vatRateTakeaway = 5.5`.
8. **Production was read, never written, by this session, and it is demonstrated.** The migration is **not** applied there (`Category` has no `vatRateTakeaway`), no `SCRATCH%` marker exists, both drink categories still read 5,5 %, and no `-wal`/`-shm` appeared beside the file. Read-only inspection used `bun:sqlite` with `readonly: true`. Scratch servers on port 3088 were stopped with `taskkill //PID <pid> //T //F` each time.
9. **Verified on the production till, by the operator, after both [OWNER] steps.** *Bouteilles* and *Canette* now read **sur place 10 % / à emporter et livraison 5,5 %**, and the journal shows the same product taxed two ways: **#37 `DINE_IN` 7 Up — `vatRate` 10, `lineHt` 136, TVA 0,14 €** and **#38 `TAKEAWAY` 7 Up — `vatRate` 5,5, `lineHt` 142, TVA 0,08 €**.
10. **The trap in L-68f sprang exactly as written, and cost one round trip.** After applying the migration the operator rang two sales and both still showed 0,08 €. Nothing was broken: the categories still held `vatRate = 5.5` with `vatRateTakeaway` unset, and unset means *the same rate whatever the order type*. **The old 5,5 was sitting in the column that now means sur place.** The item said "a SWAP, not an addition" and the note above it said so twice; it still had to be diagnosed from the data before it was believed. **A fallback that is safe by design is also silent by design** — worth remembering the next time a nullable column is added to spare existing rows.
...
**Status:** `IN PROGRESS` · **Opened:** 2026-09-08, at the operator's instruction, from a duplicated « Sauces » group visible on every Croustillant in the POS.

**Stage 5 reopens for it**, as Stages 2 and 3 have each reopened before. 5.7d's "this batch completes Stage 5" was true when written.

### The defect

Three facts, each read from the code rather than inferred:

1. **The read merges and does not mark.** `options: [...categoryOptions, ...productOptions]` — `src/app/api/catalog/products/route.ts:142` and `src/app/api/catalog/products/[id]/route.ts:139`. Nothing in `ProductDto` says which groups came from the category, so no client can tell them apart.
2. **The editor round-trips that merged list.** `products-view.tsx` builds `cleanGroups` from the form state it loaded from the DTO and sends it as `options` (`:471`).
3. **The write persists everything it receives as product-level.** `PUT /api/catalog/products/[id]` does `deleteMany({ where: { productId } })` and then creates one `OptionGroup` per received group with `productId` set (`:209-221`).

**Therefore: opening any product whose category provides option groups and saving it copies those groups down onto the product.** The category keeps its own, and the POS then shows both. It is not a one-off data accident; it reproduces on the next save.

**Measured 2026-09-08.** `bun scripts/fix-duplicate-product-options.ts` (dry run) reports **12 affected products, all Croustillants, one duplicate « Sauces » group each** — exactly the twelve edited that afternoon to change their photographs, and no others.

**Why it is not cosmetic.** The Croustillants category's « Sauces » group is `required: true`, so the POS presents **two required groups**: a sauce must be chosen twice before the product can be added to the basket, and the choice then appears twice on the line and on the kitchen ticket. Option choices carry `priceModifier`, `pickupPriceModifier` and `deliveryPriceModifier`, and `recalculateUnitPrice` sums the modifier of **every** selected option — so a duplicated group whose choices carry a non-zero modifier applies it twice. **Every Croustillants sauce is zero, so no price on production is wrong today**; the mechanism is not limited to that category.

**Why the repair script is not the fix.** `scripts/fix-duplicate-product-options.ts` exists, is dry-run by default, and has plainly been needed before. It treats a recurring corruption as an event. This batch removes the cause; the script stays, because the data already corrupted still has to be repaired.

**The rule is inherited, not invented here.** That script already encodes it: when `inheritCategoryGlobals` is true, a product-level group whose name matches an inherited one — trimmed, lower-cased, against `category.parent ?? category` — is an error and is deleted. This batch reuses that rule **exactly**, so the guard and the repair agree by construction rather than by coincidence.

### L-67 — Inherited option groups are persisted as product-level groups on every save

| Item | Status | What |
|---|---|---|
| **L-67a** | `NOT STARTED` | One tested module owning the merge and the inheritance rule. Both routes carry their own copy today; this is `services/ticket-layout.ts`'s rule — one home or none. |
| **L-67b** | `NOT STARTED` | `ProductDto` marks inherited groups, so a client can tell them from the product's own. |
| **L-67c** | `NOT STARTED` | `PUT` refuses to persist a group duplicating an inherited one. **Server-side, so the data is protected whatever the client sends** — including the client that caused this. |
| **L-67d** | `NOT STARTED` | The admin form stops sending inherited groups back. |
| **L-67e** | `NOT STARTED` | **[OWNER]** Repair the 12 already-corrupted products. A write to real menu data, so it is the operator's command (warning 9), with a backup first (warning 4). |

### Validation Required

1. **Unit tests on the extracted module**, covering: an inherited group and a product group with the same name; differing case and surrounding whitespace; `inheritCategoryGlobals` false (nothing is inherited, so nothing is a duplicate and the product keeps its own); a sub-category inheriting through `category.parent`; and a product-level group with no inherited counterpart, which must survive.
2. **The revert protocol** — *Methods → Prove the test fails on the old code*: one property at a time, both directions, and **state which tests pass under no revert and why**.
3. `bun run test`, `bun run typecheck`, `bun run lint` all green; the README count re-pinned if it moves.
4. **Manual proof against the real PUT, on a scratch copy** — *Methods → Scratch copy*: marker proved through the pre-auth `GET /api/auth/profiles` **before** the first write, then a Croustillant edited and saved through the running app, then the copy re-inspected to show **no product-level « Sauces » group was created**. The same edit on the pre-fix code must create one.
5. **Production untouched**: `db/custom.db` sha256 and mtime unchanged, no `-wal`/`-shm` beside it, `db/backups/` and `db/fiscal-archives/` untouched.
*Moved verbatim from `REMEDIATION_PLAN.md` lines 1520–1562 (commit `6994e5f`) on 2026-09-08.*

### Status record

**Status:** `COMPLETED` · **Completed:** 2026-09-08 · **Commit:** `6994e5f` · **Findings:** L-67 (**closed**) · **Decisions:** none — the rule was already the repair script's and is reused rather than chosen.

**Changes.** One module now owns the option-group inheritance rule, `src/lib/services/product-options.ts`: `normalizeGroupName` (trim + lower-case), `inheritedCategoryGroups` (`parent ?? category`), `inheritedGroupNames` (empty when `inheritCategoryGlobals` is false) and `splitOwnFromInherited`. Its rule is `scripts/fix-duplicate-product-options.ts`'s, reproduced exactly, so the guard and the repair cannot drift. **L-67c, the fix that matters:** `PUT /api/catalog/products/[id]` now loads the category being saved to (with its parent), computes the inherited names and persists only `own` — so a group the product already inherits is never stored again, whatever the client sends. It is silent rather than a 400, because the client is returning what the GET gave it; failing the save would stop an operator editing the price of any product with inherited options. **L-67b:** `OptionGroupDto` gained `inherited: boolean`, set `true` on category-sourced groups and `false` on product-sourced ones in both serialisers, so the merged list is no longer ambiguous. **L-67d:** the product form filters `inherited` groups out of its editable list — they are edited on the category, and after L-67c offering them would silently discard the edit. **The size group is deliberately not filtered there**: `existingSizes` reads it from the merged list and it drives the product's own base price, which does save. **L-67e was completed by the operator**, who ran the repair after backing the database up.

**Files.** `src/lib/services/product-options.ts` (new), `src/lib/services/product-options.test.ts` (new, 14), `src/app/api/catalog/products-inherited-options.test.ts` (new, 6), `src/app/api/catalog/products/[id]/route.ts`, `src/app/api/catalog/products/route.ts`, `src/types/api.ts`, `src/features/catalog/products-view.tsx`, `README.md` (count re-pinned 1007 → 1031), `src/lib/plan-freshness.test.ts` (open-finding count).

**Tests.** 1031 pass, 0 fail — 20 added. **The revert protocol found the batch's own hole and is the reason there are two test files rather than one.** R2 (normalisation removed) → 4 fail; R3 (parent resolution removed) → 2 fail; R4a (`inheritCategoryGlobals` ignored) → 2 fail; R4b (the same flag inverted) → 8 fail; restored → 14 pass. **R1, reverting the route's single `splitOwnFromInherited` call, was survived by all 1025 tests** — the unit tests proved the rule and nothing proved the route used it, which is *Methods → Prove the test fails on the old code* verbatim: "the test proves less than it claims". `products-inherited-options.test.ts` was written for that, driving the real `PUT` through `route-harness.ts`; under R1 **4 of its 6 fail**. The two that survive R1 are named as such: the `inheritCategoryGlobals: false` case is a **control** that must pass either way, and the `options`-omitted case is a **regression assertion** pinning C-24 (Batch 4.6), which this batch's guard sits inside.

**Notes.**

1. **The cause was a round-trip, not a data accident.** The GET merged the category's groups with the product's own and marked neither; the editor loaded that merged list; the PUT stored all of it with `productId` set. Every save copied the category's groups onto the product. `scripts/fix-duplicate-product-options.ts` existed and had evidently been needed before — it repaired the symptom on a schedule nobody had written down.
2. **Measured before and after, read-only.** The dry run reported **12 affected products, all Croustillants, one duplicate « Sauces » each** — exactly the twelve edited that afternoon to change their photographs, and no others. After the operator's repair the same dry run reports **0**.
3. **Not cosmetic, and the reason is `required`.** The Croustillants category's « Sauces » is `required: true`, so the POS presented two required groups: a sauce had to be chosen twice before the product could enter the basket, and it printed twice. `recalculateUnitPrice` sums the modifier of every selected option, so a duplicated group with non-zero modifiers would apply it twice — **every Croustillants sauce is zero, so no price on production was ever wrong**.
4. **The pizzas were the near-miss.** The Pizzas category owns a « Taille » group, and the product form rebuilds a size group from the merged list and sends it on every save — so the guard drops it. That is safe, and it was checked rather than assumed: `product-options-dialog-v2.tsx` selects the size group with `options.find(...)`, and the merged list puts the category's first, so a product-level « Taille » has never been what the till renders. The base price still saves through the form's separate `sizes` path. **Had the guard been narrowed to spare it, the same corruption would have been left open for every size group.**
5. **A test-isolation fault that only the full suite could show.** The route file's first `wipe()` did `deleteMany` over `User`, `Product` and `Category`; six tests passed alone and all six failed together, because the runner shares one temp database and other files leave rows referencing `User`. The cleanup is now scoped to the rows this file creates. It also deleted the wrong model — **category groups are `CategoryOptionGroup`; `OptionGroup` carries only `productId`** — which the first scoped version got wrong too.
6. **The override path is unchanged and is the whole answer to "what if a product really wants its own version".** With `inheritCategoryGlobals` false the category contributes nothing, so an identically-named group is the product's own and survives; a route test pins it. **That hatch is all-or-nothing** — see *Left open*.
7. **Criterion 4 was re-derived rather than run as written.** It asked for a manual walkthrough against the real PUT on a scratch copy. What replaced it is stronger for this property and weaker for one other, and both halves are stated: `products-inherited-options.test.ts` drives the **real** handler against a real database and covers the sub-category and opt-out cases a walkthrough would not have reached, and R1 proves it catches the defect; what it does **not** cover is the admin form itself (L-67d), which remains an [OWNER] check on the till. *Methods → A batch's Validation Required may predate its answer.*
8. **Production was read, never written, by this session.** Only `inspect-options.ts`, `inspect-product.ts` and the repair script's **dry run** were run, all read-only or `--apply`-gated. `db/custom.db` is `ba713d346292d3ee34ac441a9039bc68bc9b484e61041df4745d91e1bc2357bf`, 749 568 bytes, with no `-wal`/`-shm` beside it. **The change from the 704 512 bytes of the change log's baseline is the operator's own work** — the new Croustillants photographs and their repair — not this batch's.


### Appended 2026-09-08 — criterion 4 run after all, on the real UI

*Appended, not edited: the record is append-only and the batch above was already `COMPLETED`. Note 7 said criterion 4 had been re-derived because a route test covered the mechanism and the admin form remained an [OWNER] check. **The operator asked for that check to be closed, and it now is.** Nothing in the batch changed as a result — this records the evidence.*

**How.** *Methods → Scratch copy*, in full. `db/custom.db` copied to the session scratchpad; a marker user `SCRATCH-COPY-5.8` and a PIN Claude chose written to **the copy only**, by a script guarded on the target path that refuses any `DATABASE_URL` outside the scratchpad or containing `custom.db`; `bun run build` and `bunx next start -p 3088` both with **`DATABASE_URL` and `HIBAPOS_DATA_DIR`** overridden. **The marker was proved through the pre-auth `GET /api/auth/profiles` before the first write** — it answered with `SCRATCH-COPY-5.8` present.

**What the form does now.** The editor for **5 Tenders**, a Croustillant, offers *« Hériter des options & suppléments globaux — Applique les réglages définis dans la catégorie « Croustillants » »* and, under *Options de personnalisation*, **« Aucune option. »** The « Sauces » group is no longer presented as the product's own to edit, which is L-67d on screen. **Saving that product through the real editor** then left it with exactly `[{ name: "Sauces", inherited: true }]` — one group, the inherited one. That same save is what used to create the duplicate.

**The pizza, which note 4 called the near-miss.** **Calzone**'s editor still shows *Tailles multiples* with its size rows, inheriting from « Sauce Tomate ». Saved through the real editor, the product came back **byte-identical**: `price` 890, `pickupPrice` 890, `deliveryPrice` 990, « Taille » still `inherited: true` with Junior 0 / Senior 300 / Mega 700, and **zero product-level groups**. So filtering inherited groups out of the form did not disturb the size path, and the guard did not eat a size group.

**Browser driving.** Synthetic clicks did not land — the shape *Methods → Browser driving* and L-47 describe. Every interaction was dispatched through the DOM instead, and that is said here rather than left implied. A screenshot also timed out with the pane hidden; `get_page_text` and `javascript_tool` carried the walkthrough.

**Production.** **This session never wrote to it, and that is demonstrated rather than asserted:** `SCRATCH-COPY-5.8` **does not exist** in `db/custom.db` — its only users are `admin` and `manager` — and `5 Tenders` and `Calzone` both have **0** product-level option groups there. No `-wal`/`-shm` appeared beside it; the copy grew its own, as *Methods* says a scratch copy does. **Its sha256 DID move during the session** — `26e8f0a4…` when the copy was taken, `5f6895de…` afterwards, size unchanged at 749 568 bytes — and the reason is **not this session**: the operator's own server is listening on port 3000 against the production database and they were working in it throughout. Recorded plainly because "production untouched" would have been the convenient claim and the hash does not support it. Read-only inspection used `bun:sqlite` with `readonly: true`, per *Methods*. The scratch server was stopped with `taskkill //PID 1848 //T //F`.
...
*Moved verbatim from `REMEDIATION_PLAN.md` lines 1722–1781 (commit `84ca622`) on 2026-09-07. Nothing in this section has been rewritten; corrections, if any, are appended dated notes.*

**Status:** `COMPLETED` — reopened Stage 7 and closed it the same day.

**Why it exists.** Seven rows in *Newly Discovered Issues* said `NO BATCH OWNS THIS`. The operator asked on 2026-09-07 for the small ones to be cleared. **Three of them are code and are in this batch. Two are not code and are excluded, with reasons** — saying so is the point of this section, because "do the five" would otherwise be answered with three fixes and two silences.

### In this batch

**L-22 — validation errors reach the French UI as untranslated English zod messages.**
Reproduced verbatim 2026-09-07: `z.number().int().min(32).max(48)` on `receiptWidth` yields **`"Too big: expected number to be <=48"`** — the exact string the operator saw as L-20. Measured: **24 API routes** return `parsed.error.issues[0]?.message` straight to the client, and `validation.ts` declares **89** zod fields of which only **19** carry a custom French message. So ~70 fields can emit English into a French UI, on every screen that saves.

**L-36 / DOC-13 — `ApprovalPayload.amount` is documented as euros and has always carried cents.** The *same defect, filed twice*: DOC-13 against Batch 1.1 and L-36 against 4.4c. `approvals.ts:23` reads `amount: number | null; // euros`; every caller binds cents (`step-up/route.ts` declares `z.number().int()`, `refund/route.ts` passes `parsed.data.amount`, `orders-view.tsx` passes `amountCents` **by that name**). Nothing is mis-computed — both sides agree — and the row's own instruction is **fix by correcting the comment, not the code**.

**DOC-14 — vestigial euros-era rounding in a money path.** `product-options-dialog-v2.tsx:93` computes `Math.round((unitPrice + addonsTotal) * qty * 100) / 100`. `productUnitPrice()` returns integer cents and add-on prices are cents, so `Math.round(cents × qty × 100) / 100` **is exactly `cents × qty`** — the displayed figure is right and the arithmetic reads like the cents/euros confusion that produced C-01 and C-02.

### NOT in this batch, and why

**L-14 — archived 80-column receipts.** Re-measured 2026-09-07: all **20** stored `Receipt.content` rows are 80 columns. **There is nothing to fix.** An archived receipt is an immutable fiscal artifact and re-rendering one is forbidden — the row says so and Batch 3.6's constraint repeats it. The population is development trading that **Batch 8.0 deletes**, so on this install the finding empties rather than closes. Adding a warning to the reprint path would be new behaviour, not a fix, and a decision nobody has been asked for (safety rule 11).

**L-47 — the login screen renders over a valid session in the in-app browser pane.** **Cause not established, and not for want of trying**: Batch 6.3 attempted a reproduction, could **not** reproduce it, and falsified its own hypothesis. The row's own instruction is to **re-open on the next occurrence rather than assign it to a batch that would have nothing to reproduce**, and that instruction is still right. A third speculative attempt would be work with no artifact.

### Design decisions taken in this batch, with their reasons

1. **L-22 is fixed with zod's own French locale, once, not with ~70 per-field annotations.** zod 4.3.5 ships `z.locales.fr()`; `z.config(z.locales.fr())` makes every message French for every schema and every route. Verified: the `receiptWidth` message becomes `"Trop grand : number doit être <=48"`. **Per-field custom messages still win** — zod prefers them — so the 19 already-good French messages are untouched and the fix cannot regress them.
2. **The config lives in its own side-effecting module** (`src/lib/zod-locale.ts`) imported by **both** `src/lib/validation.ts` and `src/instrumentation.ts`. Neither alone is enough: **13 API routes declare inline schemas and do not import `validation.ts`** (measured), so the shared-schema import misses them; and `instrumentation.ts` is server-only, so it misses the client forms `validation.ts` says it also serves.
3. **It is proved through the real route, not the module.** A global mutable config that the wrong module instance reads would look correct in a unit test and be wrong in the app. The criterion is an HTTP response.

### Batch 7.5 — Validation Required

- The English message is **reproduced before it is fixed**, and the count measured: 24 routes, 89 fields, 19 with custom messages.
- After the fix, `settingsSchema` rejecting `receiptWidth: 80` yields a **French** message, and the string `"Too big"` appears in no response.
- **The 19 existing custom French messages are unchanged** — a per-field message still beats the locale.
- Inline route schemas are covered: a schema declared in a route that does **not** import `validation.ts` also answers in French.
- **Driven through the real HTTP route**: `PUT /api/settings` with an out-of-range value on a scratch copy returns French, and so does one of the 13 inline-schema routes.
- `approvals.ts` documents `amount` as **cents**, and a test pins that the comment and the callers agree, so the next caller cannot read the old lie.
- `product-options-dialog-v2.tsx`'s line total is asserted to equal `cents × qty` exactly, before and after, for a case with add-ons and a quantity above 1.
- **Prove the tests fail on the old code.** One property at a time, both directions, restored from a copy taken before each revert, and say which tests pass under no revert and why.
- `bun run test`, `bun run typecheck`, `bun run lint`, `bun run build` all pass; `readme-counts.test.ts` keeps the README's figure honest.
- Production untouched: sha256, size and mtime unchanged; no `-wal`/`-shm`; `db/backups/` still nine files.

---

### Batch 7.5 — Status Record

**Status:** `COMPLETED`
**Completed:** 2026-09-07
**Changes:** **L-22** is fixed once rather than seventy times. zod 4.3.5 ships an official French locale, so `src/lib/zod-locale.ts` calls `z.config(z.locales.fr())` as a module side effect and every schema in the codebase answers in French — including the ones nobody has written yet. **It is imported in two places and both are load-bearing**: `src/lib/validation.ts` covers the shared schemas and the client forms it also serves, and `src/instrumentation.ts` covers the **13 API routes that declare inline schemas and never import `validation.ts`** (measured, not assumed). **Per-field messages still win** — zod prefers them — so the 19 French messages already in `validation.ts` are untouched, which is the property that makes a global change safe. **L-36 / DOC-13**: `approvals.ts`'s `amount` is documented as **integer cents**, which is what every caller has always bound; the comment said `// euros` from Batch 1.1 until today, and the row's instruction was to fix the comment rather than the code. **DOC-14**: the vestigial `Math.round((unitPrice + addonsTotal) * qty * 100) / 100` is now `(unitPrice + addonsTotal) * qty` — identical on integer cents, which is why the displayed figure was never wrong, and no longer reading like the cents/euros confusion that produced C-01 and C-02.
**Files:** `src/lib/zod-locale.ts` (new), `src/lib/validation.ts`, `src/instrumentation.ts`, `src/lib/approvals.ts`, `src/components/pos/product-options-dialog-v2.tsx`, `src/lib/small-findings.test.ts` (new), `README.md`.
**Tests:** `bun run test` — **986 pass, 0 fail** (976 + 10). `bun run typecheck`, `bun run lint`, `bun run build` — all PASS. `readme-counts.test.ts` caught the README again, which is twice out of two batches since it was built.
**Reverts (seven, one property at a time, both directions).** *Which tests pass under NO revert and why:* three, all controls. "**the line total is exactly cents × quantity**" survives R5 **by design** — the old and new expressions are mathematically identical on integer cents, and that is the whole point of DOC-14: the assertion exists to prove the figure did **not** change, not to cover the removal. "**the callers really do bind cents**" and "**the type still admits null**" survive R4 for the same reason: they prove the corrected comment is *true*, which is what stops a comment fix from making the file more wrong. **R1** (no locale — the pre-batch English): 3 fail. **R2** (locale in `validation.ts` only): **1 fail, and only the wiring assertion** — see note 2, this is the batch's most useful revert. **R3** (locale in `instrumentation.ts` only): 4. **R4** (comment says `euros` again): 1, isolated. **R5** (vestigial rounding back): 1, isolated. **R6** (*other direction* — `z.locales.en()` instead of `fr()`): 4. **R7** (*other direction* — strip a field's own French message so the locale answers instead): 1, isolated on "does not disturb the messages that were already French", which pins the per-field-wins property.
**Real-route validation (2026-09-07).** Scratch copy under the scratchpad, both `DATABASE_URL` and `HIBAPOS_DATA_DIR` overridden, marker (`SCRATCH-7.5 Admin`) read back pre-auth before the first write, prep script demonstrably refusing `db/custom.db`. Authentication gates validation, so the walkthrough logs in first. **Both halves of the wiring proved on the wire, which is the only place they can be**: (1) a **shared** schema — `PUT /api/settings` with `receiptWidth: 80` → `{"error":"Trop grand : number doit être <=48"}`, the exact message that read `"Too big: expected number to be <=48"` before this batch; (2) an **inline** schema in a route that never imports `validation.ts` — `POST /api/reports/z` with a string `closingFloat` → `{"error":"Entrée invalide : nombre attendu, string reçu"}`. Server stopped with `taskkill //PID <pid> //T //F`. **Production untouched**: `d09369c09dd9b4515c78af31118e8dc47516e74c0ab4d41e2bc4d93c5e54b16b`, 704 512 bytes, mtime 2026-09-07 15:33:56; `db/backups/` still nine files.
**Commit:** `6e498e6`
**Notes:**
1. **The English message was reproduced before it was fixed, and the scale measured.** `"Too big: expected number to be <=48"` — the very string the operator saw as L-20 — on the screen that saves every other setting. **24 API routes** hand `parsed.error.issues[0]?.message` straight to the client and **89** zod fields exist against **19** French messages, so about seventy fields could answer a French operator in English.
2. **R2 is the finding of this batch, and it is about the test rather than the code.** Removing the `instrumentation.ts` import — the half that covers the 13 inline-schema routes — failed **only the assertion that reads the two files as text**. No behavioural test caught it, and none could: in `bun test`, any test that imports `validation.ts` configures zod for the whole process, so the inline-schema case passes whether instrumentation is wired or not. **A unit test cannot prove this fix.** That is why the batch's criterion was an HTTP response from `/api/reports/z`, and why the text assertion is kept rather than dismissed as a lint.
3. **A locale, not seventy annotations.** Annotating fields one at a time fixes the fields somebody remembered — the half-fix shape that L-21 turned out to be and that L-63 then had to finish. One config covers every schema including future ones, and cannot regress the existing French messages because zod prefers a field's own.
4. **The two one-line corrections were verified as still-open before being fixed, not taken on the register's word.** `approvals.ts` really did read `// euros`, and `product-options-dialog-v2.tsx` really did carry the `* 100 / 100`. Both had been recorded since 2026-09-03 and outlived Batches 7.1 and 7.2.
5. **L-36 and DOC-13 are one defect filed twice** — DOC-13 against Batch 1.1, L-36 against 4.4c — which is a plausible reason it outlived 7.1: each row looked like the other's problem. Both close here.
6. **Two of the five findings the operator named are NOT code and were excluded, with reasons stated in the batch rather than passed over in silence.** **L-14**: all 20 archived receipts are 80 columns (re-measured today), an archived receipt is immutable and must not be re-rendered, and Batch 8.0 deletes the population — so it empties rather than closes, and adding a reprint warning would be new behaviour nobody asked for (safety rule 11). **L-47**: Batch 6.3 attempted a reproduction, could **not** reproduce it, and falsified its own hypothesis; the row's instruction is to re-open on the next occurrence rather than assign it to a batch with nothing to reproduce, and that is still right.

---

## Batch 7.6 — Touch targets and label association (L-09, L-10)

*Moved verbatim from `REMEDIATION_PLAN.md` lines 1735–1804 (commit `5783d69`) on 2026-09-07. Nothing in this section has been rewritten; corrections, if any, are appended dated notes.*

**Status:** `COMPLETED` — reopened Stage 7 a second time and closed it the same day.

**Why now.** Both were `DEFERRED` — "revisit after Stage 8" — and the operator pulled them forward on 2026-09-07. The reason is sound: the commissioning session is the **first time anyone will use this app as a touchscreen all day**, and these are the two findings about exactly that.

### L-09 — touch-target regressions from the Phase 10 pass

**Status:** `COMPLETED` · Severity: LOW · Category: UI ergonomics

**Measured 2026-09-07, and both cited sites are real** — with the line numbers drifted, as usual here:

| Site | Cited | Actual | What it is |
|---|---|---|---|
| `payment-dialog.tsx` | `:340` | **`:410`** | `h-9 min-h-[48px] w-9` — 48 px tall and **36 px wide**. The `min-h` rescues the height and nothing rescues the width. |
| `pos-view.tsx` | `:451` (wrong path) | **`src/features/catalog/pos-view.tsx:504`** | `<Button size="sm" className="h-7">` — **28 px** — on the *blocking* no-shift banner, i.e. the one control a cashier must hit to start trading. |
| — | not cited | **`products-view.tsx:964`** | `h-7` — 28 px — "Ajouter un choix". Found by sweeping rather than by reading the row. |

**And the root, which the finding does not name.** `src/components/ui/button.tsx`'s size variants are **all** under 44 px: `default h-9` (36), `sm h-8` (32), `lg h-10` (40), `icon size-9` (36). Every button that does not override with `min-h-[48px]` or `h-11`/`h-12` is undersized for touch. The Phase 10 pass evidently knew this — `min-h-[48px]` appears throughout `cart-panel.tsx` and the error boundaries — which is what makes the three above *regressions* rather than an absent convention.

### L-10 — `aria-label` coverage

**Status:** `COMPLETED` · Severity: LOW · Category: accessibility

**⚠ The finding's premise is substantially wrong, and measuring it is this batch's most useful output.** L-10 says coverage is "absent from shifts, reports, orders, dashboard, settings, logs, audit and four dialogs". Measured with a parser that reads whole JSX elements rather than lines:

- **Icon-only `<Button size="icon">`: 21 of 22 carry an `aria-label` or `title`.** The 22nd is `ui/calendar.tsx`'s day button, which takes its name from the day number react-day-picker passes through `{...props}` — **not a defect**. So the real count of nameless icon buttons is **zero**.
- **`<Label>`: 54 of 71 already carry `htmlFor`.** **17 do not**, across seven files.
- **`<Input>`: 5 have neither a name, an `id`, nor a placeholder.** Two of those have no visible label at all; three sit beside a `<Label>` that is not associated.

So the real gap is **label association**, which L-10 does not mention, and it is a quarter the size the row implies. *Two earlier measurements in this session got this wrong* — a line-based grep missed `aria-label` on an adjacent line and reported 10 nameless icon buttons; a regex that stopped at the first `>` was defeated by `onChange={(e) => …}` and reported 33 nameless inputs. Both numbers were wrong and both were checked by eye before anything was written.

### Design decisions taken in this batch, with their reasons

1. **The three measured touch targets are fixed; the Button primitive's base sizes are NOT changed.** Raising `default` from `h-9` to `h-11` would grow every button in the application by 8 px, in dense admin tables and dialogs I cannot see rendered, on the eve of the commissioning session. The three sites are the measured defect; the base sizes are a **design decision for the operator**, recorded as **L-64** rather than taken here (safety rule 11).
2. **`htmlFor`/`id` pairs rather than `aria-label`.** Where a visible label exists, associating it is correct — `aria-label` on a control that already has visible text creates two names, and a screen reader announcing something different from what is on screen is worse than the gap. `aria-label` is used only for the two inputs that have no visible label.
3. **The guard is a static test over the source**, in the shape this codebase already uses (`deployment.test.ts` reads PowerShell as text, `role-model.test.ts` greps `src/`). It asserts the invariant, so the next `h-7` or unassociated `<Label>` fails the build rather than waiting for another audit.
4. **The parser used by that test reads whole JSX elements**, tracking braces, quotes and nesting. A line-based or non-greedy regex is what produced both wrong numbers above; shipping one inside the guard would bake the same error in.

### Batch 7.6 — Validation Required

- The defects are **measured before they are fixed**, and the finding's premise is re-measured rather than taken on trust.
- Every interactive element in `src/` is at least **44 px** in both dimensions, asserted over the source.
- Every `<Label>` carries `htmlFor`, and every `<Input>` carries an `id`, an `aria-label` or a placeholder — asserted over the source.
- The `id`s introduced are **unique within their file**, so an association points at one control.
- **Prove the tests fail on the old code.** One property at a time, both directions, restored from a copy taken before each revert, and say which tests pass under no revert and why.
- `bun run test`, `bun run typecheck`, `bun run lint`, `bun run build` all pass; the suite count moves only by the tests added.
- **No visual regression is claimed.** The three touch-target fixes change rendered size and this session cannot see the application. What is asserted is the measurement; **confirming the layout still reads well is an `[OWNER]` check at the commissioning session**, added to the runbook.
- Production untouched.

---

### Batch 7.6 — Status Record

**Status:** `COMPLETED`
**Completed:** 2026-09-07
**Changes:** **L-09** — **eleven** undersized touch targets raised to at least 44 px, not the two the row names. The two cited sites are real (`payment-dialog.tsx`'s 48×**36** icon button, and the **28 px** button on the *blocking* no-shift banner — the one control a cashier must hit to start trading); a third was found by sweeping (`products-view.tsx`, 28 px), and **eight more were found by the standing guard once it existed**, because they declare `className` on a different line from `<Button` and a line-based grep cannot see them. **L-10** — every `<Label>` in `src/` is now associated: **12 by `htmlFor`/`id`** where a single control exists, and **5 by `id` + `role="group" aria-labelledby`** where the label names a set of buttons or cards and `htmlFor` would point at nothing. Two price inputs in the repeated size row, which have no visible label at all, got `aria-label`. **`src/lib/touch-and-labels.test.ts`** is the standing guard: it parses whole JSX elements and asserts no target under 44 px, every Label associated, every Input named, every icon-only Button named, and every id unique within its file.
**Files:** `src/lib/touch-and-labels.test.ts` (new); `src/components/pos/payment-dialog.tsx`, `cart-panel.tsx`, `customer-picker-dialog.tsx`; `src/components/shared/topbar.tsx`; `src/features/catalog/pos-view.tsx`, `products-view.tsx`, `categories-view.tsx`; `src/features/tables/tables-view.tsx`; `src/features/media/media-view.tsx`; `src/features/orders/orders-view.tsx`; `src/features/shifts/cash-movement-dialog.tsx`; `README.md`.
**Tests:** `bun run test` — **992 pass, 0 fail** (986 + 6). `bun run typecheck`, `bun run lint`, `bun run build` — all PASS.
**Reverts (seven, one property at a time, both directions).** *Which tests pass under NO revert and why:* one — "parses enough Buttons for the check above to mean something" — is a **vacuity control**, there so the loop cannot pass over an empty set the way `plan-freshness.test.ts` twice did; it is not coverage of any fix. **R1** (the 36 px-wide icon button back): 1. **R2** (the 28 px banner button back): 1. **R3** (one `htmlFor` removed): 1. **R4** (one input `aria-label` removed): 1. **R6** (*other direction* — an id reused, so an association points at two controls): 1. **R7** (*other direction* — the parser degraded to stopping at the first `>`, the exact bug that produced this batch's two wrong measurements): 2. **R5 was NOT CAUGHT, and that is the batch's most useful result** — see note 3.
**Not claimed:** **no visual verification.** Eleven controls got taller and this session cannot see the application render. What is asserted is the measurement over the source; **that the till still reads well is an `[OWNER]` check**, added to the runbook at § 2.
**Production untouched**: `d09369c09dd9b4515c78af31118e8dc47516e74c0ab4d41e2bc4d93c5e54b16b`, 704 512 bytes, mtime 2026-09-07 15:33:56.
**Commit:** `1439628`
**Notes:**
1. **L-10's premise was substantially wrong, and measuring it was worth more than fixing it.** The row says `aria-label` coverage is "absent from shifts, reports, orders, dashboard, settings, logs, audit and four dialogs". Measured: **21 of 22 icon-only buttons already carried a name** (the 22nd, `ui/calendar.tsx`'s day button, takes its name from the day number react-day-picker passes through `{...props}` — not a defect), and **54 of 71 Labels already carried `htmlFor`**. The real gap was **label association**, which L-10 never mentions, and it was a quarter the size the row implies.
2. **Two measurements in this batch were wrong before anything was written, both from cheap parsing.** A line-based grep reported **10** nameless icon buttons — it could not see `aria-label` on the line below `size="icon"`; the real number was **0**. A non-greedy regex reported **33** nameless inputs — it stopped at the first `>`, which inside `onChange={(e) => …}` is the arrow; the real number was **5**. Both were caught by opening the files and looking. The guard therefore parses whole elements, tracking braces, quotes and nesting, and **R7 exists to prove that matters**: degrading it to the first `>` breaks two assertions immediately.
3. **R5 was a revert nothing caught, and it exposed a hole in the guard rather than a no-op.** Stripping `role="group" aria-labelledby=…` off a container left the label's `id` in place — the association broken, and every assertion still green, because the Label test accepted an `id` as sufficient. It now requires that the id be **pointed at** by some `aria-labelledby` in the same file. R5 fails against the strengthened test. *(This is the third revert in this project to have found a weak assertion rather than a weak fix — 4.4c's two and 3.10's R7b were the others.)*
4. **`htmlFor` was preferred to `aria-label` wherever a visible label exists.** An `aria-label` on a control that already shows text creates a second name, and a screen reader announcing something different from what is on screen is worse than the gap it closed. `aria-label` was used only for the two size-price inputs, which have no visible label at all.
5. **The Button primitive's base sizes were deliberately NOT changed, and that is recorded as L-64 rather than decided here.** `ui/button.tsx` declares `default h-9` (36 px), `sm h-8` (32), `lg h-10` (40), `icon size-9` (36) — so every button that does not override is under 44. Raising the base would grow every button in the application by up to 8 px, in dense admin tables and dialogs nobody in this session can see rendered, on the eve of commissioning. It is a design decision for the operator (safety rule 11), and the guard above will hold the line at the call sites meanwhile.

---

## Batch 7.7 — The Button primitive's base sizes (L-64)

*Moved verbatim from `REMEDIATION_PLAN.md` lines 1748–1785 (commit `fdd2459`) on 2026-09-07.*

**Status:** `COMPLETED` — reopened Stage 7 a third time and closed it the same day.

**Why it exists.** Batch 7.6 opened L-64 and explicitly declined to take it: raising the base sizes grows every button in the application, and **no session had ever seen this application render**. The operator took the decision on 2026-09-07. The second reason did not go away with the first, so this batch went and looked.

### L-64 — every size variant was under 44 px

**Status:** `COMPLETED` · Severity: LOW · Category: UI ergonomics

`src/components/ui/button.tsx` declared `default h-9` (**36 px**), `sm h-8` (**32**), `lg h-10` (**40**) and `icon size-9` (**36**). Measured 2026-09-07: **103 of 144 `<Button>` elements declare no height of their own** and take the variant's — so Batch 7.6 could fix eleven call sites and leave the large majority undersized, **and its guard could not see them**, because that guard reads the call site's own classes. 7.6's note that it "will hold the line at the call sites meanwhile" was true only of sites that declare a height, and this batch is the correction.

Now `default` and `sm` are **44 px**, `lg` is **48**, `icon` is **44**. `sm` keeps its tighter padding, smaller gap and type: on a till where every control is touched, "small" cannot mean "below the floor" — it means "takes less room across".

### Batch 7.7 — Validation Required

- The scope is **measured** — how many buttons take the variant's height rather than their own — before anything is changed.
- Every size variant is at least 44 px, asserted over `button.tsx` itself, beside the existing call-site assertion.
- **The application is looked at.** The reason 7.6 gave for deferring was that nobody could see it render; that has to be answered rather than repeated.
- **Prove the tests fail on the old code**, one property at a time.
- `bun run test`, `bun run typecheck`, `bun run lint`, `bun run build` all pass.
- Production untouched.

### Batch 7.7 — Status Record

**Status:** `COMPLETED`
**Completed:** 2026-09-07
**Changes:** `default` 36 → **44 px**, `sm` 32 → **44**, `lg` 40 → **48**, `icon` 36 → **44**. One new assertion in `touch-and-labels.test.ts` reads `button.tsx` itself, so the variant level is now guarded as well as the call sites.
**Files:** `src/components/ui/button.tsx`, `src/lib/touch-and-labels.test.ts`, `README.md`.
**Tests:** `bun run test` — **993 pass, 0 fail** (992 + 1). `bun run typecheck`, `bun run lint`, `bun run build` — all PASS. **R1/R2/R3** put `default`, `icon` and `sm` back one at a time; each fails the new assertion alone.
**Visual verification — and this is the part 7.6 could not do.** The application was started on a scratch copy (`DATABASE_URL` and `HIBAPOS_DATA_DIR` both overridden, a known PIN written into the copy by a guarded script) and **driven in the browser pane through six screens**: the profile picker, the PIN pad, the dashboard, the **POS**, the **product-options dialog**, the **Catégories list** and the **category editor dialog** — the last two being the densest surfaces and the ones carrying 7.6's edits. **Nothing overlapped, nothing was clipped, no row broke.** The colour-swatch group 7.6 gave `role="group"` renders correctly. Server stopped with `taskkill //PID <pid> //T //F`; production `d09369c0…` unchanged.
**Commit:** `1379e93`
**Notes:**
1. **L-47 did not reproduce, and this is the second failed reproduction.** The row says the in-app browser pane "renders its login screen even with a valid session, so no browser walkthrough can reach any authenticated view", and instructs re-opening on the next occurrence rather than assigning it a batch. This session **selected a profile, typed a PIN and reached the dashboard, the POS and two dialogs** — synthetic clicks landed, and the authenticated views rendered. Batch 6.3 also failed to reproduce it and falsified its own hypothesis. **The row is amended rather than closed**: a defect nobody can reproduce twice is not proven absent, but "no browser walkthrough can reach any authenticated view" is now contradicted by one that did.
2. **The reason 7.6 gave for deferring was answered, not repeated.** It deferred on two grounds — the decision was the operator's, and nobody could see the application. The operator settled the first; the second was settled by going and looking, which turned out to be possible. **A constraint recorded once is worth re-testing before it is inherited a third time.**
3. **`sm` is now the same height as `default`, deliberately.** It stays smaller in padding, gap and type. The alternative — leaving `sm` at 40 px — would have kept a variant that fails the floor by name, which is the shape of defect this batch exists to remove.
...
|---|---|---|---|---|
| **3.12** | `COMPLETED` | 2026-09-09 | `004c112` | L-68: VAT did not vary by order type, so all 17 drinks booked 5,5 % including sur place; the rate now resolves per order type and the operator swapped the categories to 10 % / 5,5 %. |
| **5.8** | `COMPLETED` | 2026-09-08 | `6994e5f` | L-67: the catalogue editor was writing every inherited option group back onto the product on each save, so the POS showed each one twice; the rule now lives in one tested module and `PUT` refuses the duplicates. |
...
| 8.1 | COMPLETED | 2026-09-06 | `ce500ac` | V-04 and V-05 — the live database verified read-only (`bun:sqlite`, `readonly: true`, no Prisma, no server), with the four chains recomputed by the app's **own** `verifyEvents`/`verifyCloses` rather than a reimplementation. **27 checks pass, 1 flagged.** No counter drift — V-04's own suspicion, not borne out: every counter equals its maximum exactly (20/3/2/2). No orphans in ten checks, including the three plain-id columns a `foreign_key_check` cannot see. Nine migrations applied, nine on disk, no pending and no ghost. **`GrandTotal`, `FiscalCounter` and the chain's `lastSequence` are unchanged from the Batch 0.2 baseline in every field** — three months and nine migrations later. Every other difference explained: +11 audit rows, +2 technical logs, two tables dropped by 5.7a, two added by 5.5 and 3.8. **L-60 recorded**: 18 of 20 orders predate the fiscal journal (commit `d7ceb18`, 2026-08-29), so the Z reports read 36 300 and the grand total 5 480 and both are right — 8.0 deletes all of it. Production byte-identical throughout.
| 2.5 | COMPLETED | 2026-09-06 | `52c66c0` | **L-61 (HIGH) and L-62** — the fix for what 8.2's rehearsal found. **The first diagnosis was wrong and measurement corrected it**: `$disconnect()` releases the handle in 4–9 ms in every configuration probed (WAL, rollback, `VACUUM INTO`, transactions). The server simply had **two PrismaClients** — `db.ts` cached on `globalThis` only when `NODE_ENV !== "production"`, and Next bundles server code per entry point, so `instrumentation.ts` built one and the route handlers another. Instrumented and counted on the real production build: one process, two constructions. So `restoreBackup` disconnected its own client while the other held the file, and every restore died `EPERM`. **A production-only guard meant the defect could not appear in dev, in `bun test`, or in any unit test.** Fixed by caching unconditionally, plus `renameWithRetry` (EPERM/EACCES/EBUSY only, 5 s budget, French message saying the data was NOT modified) and a `finally` that unlinks the staged database and the decrypted 47,6 MB media archive (L-62). **Ten one-property reverts, all ten caught**; R3 exists because the first draft tested the retry thoroughly and never asserted that `restoreBackup` calls it. End to end on the failure that started it: `ok` in **2,6 s**, events 3 → 2 + `RESTAURATION`, grand total 6530 → 5480, four chains ok, 144 images restored, nothing left on disk. **Unblocks 8.2's V-06.** 923 → **934/0**.
| 1.3b | COMPLETED (code half; 1.3's `[HW]` print outstanding) | 2026-09-07 | `57f109e` | **L-21** — `renderReceipt()` centres and never wrapped, so the restaurant's **56-character** address went onto **48-column** paper on every ticket. Measured read-only from the live `Setting` rows first: one line over, by eight characters. Fixed at render time, which is the only place it can be — the stored `Receipt.content` is the fiscal artifact and `buildPrintJob()` is forbidden from reflowing it. `wrapToWidth` word-wraps, hard-breaks an over-long token, and **returns a fitting string byte-identical**, which is what let the existing snapshot stay untouched (**+33 / −0**) and be the proof the fix cannot alter a correct ticket. Every centred line routes through one helper, so the guarantee is an invariant over the block, not a list of three fields. **Eight reverts, both directions**: under the full revert the whole suite is **936 pass / 7 fail and all seven failures are this batch's** — 934 tests caught nothing, which is why L-21 lived through eight batches that edited this file; over-applying the wrap (R8) broke **both snapshots and eight pre-existing tests**, proving the snapshot is a real opt-in gate. Driven through the real route rather than the service: a sale rung through `POST /api/orders` on a scratch copy, `Receipt.content` read back (29 lines, widest 48, zero over), then `POST .../print` and `.../reprint` to a mock ESC/POS printer whose **raw bytes decoded as CP1252** gave 30 and 31 printed lines, widest 48, zero over, zero stray control bytes. **L-63 opened**: `leftRight()` overflows by a different mechanism, the option/add-on lines are not laid out at all, and the same `center()` helper is copied into two more renderers — latent on today's catalogue, one operator edit away. 934 → **943/0**; production byte-identical.
| 1.3c | COMPLETED (code half; 1.3's `[HW]` print outstanding) | 2026-09-07 | `e2b14ed` | **L-63** — the receipt's *other* overflow and the two renderers carrying a copy of the first. `leftRight()` clamped the **gap** to one space, so an over-wide pair went out as `left + " " + right` with the amount past the edge; the option, add-on and change sub-lines were raw pushes with no width arithmetic near them; and `renderDayCloseTicket()` and `renderTestPage()` each had their own copy of the receipt's `center()`, defect included. All three now share `services/ticket-layout.ts`. **Nothing is truncated** — BOFiP § 50 lists the article's libellé among the data in scope — so the label wraps and the amount is right-aligned on its **last** line, keeping one amount per article; option continuations are indented past the marker. Every primitive returns a fitting line **byte-identically**, which for `leftRight` falls out of the arithmetic rather than a special case, and **both receipt snapshots are unchanged**. **Ten reverts, both directions**: under the broadest the whole suite is **956 pass / 7 fail with every failure this batch's own** — as in 1.3b, no pre-existing test caught it; centring with `ceil` instead of `floor` breaks both snapshots, which pins the centring to the column. **R9 was a no-op and is reported as one** (`centred(s, w)` is the identity when `s.length === w`, so routing the ruler through it changes nothing — the "do not lay out the ruler" comment states an intention no test distinguishes). **The best finding came from reading the ticket, not the tests**: every unit test passed while the renderer tore `(1,50 €)` in half, because JavaScript's `\s` includes the NO-BREAK SPACE `formatEuro` emits to forbid exactly that break; the wrap now breaks only at ordinary whitespace, and both invisible characters are written as escapes. Two more defects found by tests: the batch's own guard branch returned a line one character over the paper, and `role-model.test.ts` caught a test constant carrying the removed role's name — neither guard weakened. Driven through the real routes on a scratch copy with over-long catalogue names: `POST /api/orders` → `Receipt.content` at 32 lines / widest 48 / zero over; `POST .../print` decoded from the raw CP1252 bytes at 33 lines, widest 48; `PUT /api/settings` + `POST /api/print/test` wrapping a 61-character name while the **ruler stayed exactly 48, unwrapped**; and `POST /api/fiscal/close-day` sealing a real `DailyClose` whose slip is clean at 48 and 32. 943 → **963/0**; production byte-identical. |
| 3.11 | COMPLETED | 2026-09-07 | `0996622` | **L-58's open half — the stored per-line HT**, and with it L-58 closes entirely. BOFiP § 50 lists « total HT de la ligne » and `OrderItem` held only TTC figures. **"Derivable" was the weaker claim it sounded**: the HT is taken on the line's net AFTER a largest-remainder apportionment of the discount, so a reader had to re-implement `apportion` to check it. `lineNetTotal` and `lineHt` are now written inside the checkout transaction from the numbers it already has, using **the same `splitVat` the VAT breakdown uses** — so `Σ (net − HT)` equals the order's sealed `vatTotal`, and **R3, which swapped in a floor division, was caught by four tests including both agreement assertions**. Two columns rather than one on purpose: the net makes the row checkable by division instead of by re-running the algorithm, which was the finding's actual complaint. **Existing rows are NOT backfilled** — null means "recorded before the till kept this figure", and inventing it for completed sales is what this application refuses everywhere else. Archive schema 5 → 6, surfaced only because `archive.test.ts` pins the number; **no third journal payload vintage**, checked in `buildVentePayload` rather than assumed. Seven reverts, both directions. **Rehearsed then applied**: a 305-line fiscal fingerprint diffed one line — the `_prisma_migrations` row — and the operator's `migrate deploy` reproduced exactly that on production, verified read-only (10 migrations, counters 20/3/2/2, all four chains `ok`, 82 legacy lines `NULL`). Also closed a second hole in `plan-freshness.test.ts`: its batch-id regex was `[0-9]\.[0-9]`, so **Batches 3.10 and 3.11 were invisible to it**. 967 → **976/0**. |
| 7.6 | COMPLETED | 2026-09-07 | `1439628` | **L-09 and L-10**, pulled forward from *Deferred* because the commissioning session is the first day anyone uses this application as a touchscreen. **Eleven** undersized controls raised to at least 44 px — not the two L-09 names: the two cited sites are real (a 48×**36** icon button, and a **28 px** button on the *blocking* no-shift banner, which is the one control a cashier must press to start trading), a third came from sweeping, and **eight more from the standing guard once it existed**, because they declare `className` on a different line from `<Button` and no line-based grep can see them. **L-10's premise was substantially wrong and measuring it was worth more than fixing it**: 21 of 22 icon-only buttons and 54 of 71 Labels were already correct, and the real gap — label association — is not what the row describes. All 17 unassociated Labels now carry `htmlFor`, or an `id` plus `role="group" aria-labelledby` where they name a set of buttons rather than one control. **Two measurements were wrong before anything was written**, both from cheap parsing (a line grep said 10 nameless icon buttons; the real number was 0 — a non-greedy regex said 33 nameless inputs, defeated by the `>` in `(e) =>`; the real number was 5), and both were caught by opening the files. `touch-and-labels.test.ts` is the standing guard, parsing whole JSX elements. Seven reverts, both directions; **R5 was caught by nothing and exposed a hole in the guard** — a group label's `id` was accepted without anything pointing at it — now closed and re-proved. **L-64 opened**: every `Button` base size is under 44 px, so the convention lives at the call sites; raising it is a design decision for the operator. **No visual verification is claimed** — an `[OWNER]` check was added to runbook § 2. 986 → **992/0**. |
...
|---|---|---|---|---|---|
| **L-68** | 2026-09-09 | Analysing the combo/menu feature, when the operator ruled on the drinks rate | **VAT does not vary by order type anywhere in the application, and 17 drinks are rated 5,5 % unconditionally.** `resolveVatRate(product)` (`services/pricing.ts:56`) takes the product alone; nothing in `pricing.ts`, `checkout.ts` or `money.ts` reads `orderType` for VAT. *Bouteilles* (4) and *Canette* (13) both carry `vatRate = 5.5`. **Operator's ruling 2026-09-09: a sealed bottle or can is 10 % sur place, 5,5 % à emporter et en livraison**, while everything else stays 10 % whatever the order type. So **every dine-in drink books at 5,5 % where it should book at 10 %** — an under-declaration across a whole product class, and not confined to the combos that exposed it. **No history to correct**: the app has never traded, § 6 deletes the development data, and the client trial runs on the old till with FACTICE on. The fix is contained — `resolveVatRate` has three call sites, two display-only, and the fiscal one already has `orderType` in scope. | **HIGH** — a VAT under-declaration on every dine-in drink, and it must be right before the first real sale | **Batch 3.12** — opened 2026-09-09 on the operator's ruling |
| **L-67** | 2026-09-08 | The operator reporting a duplicated « Sauces » group on every Croustillant, hours before the trial copy was due | **Saving a product in the admin editor copies its category's option groups onto the product, so the POS then shows every inherited group twice.** The read merges without marking (`products/route.ts:142`, `products/[id]/route.ts:139` — `[...categoryOptions, ...productOptions]`, nothing says which is which), the editor round-trips that merged list (`products-view.tsx:471`), and the write persists all of it as product-level groups (`products/[id]/route.ts:209-221`). **It recurs on every save**, so `scripts/fix-duplicate-product-options.ts` — which exists, and has evidently been needed before — repairs the symptom and not the cause. **Measured**: 12 affected products, all Croustillants, exactly the twelve edited that afternoon to change their photographs. **Not cosmetic**: the group is `required: true`, so a sauce must be chosen twice before the product can be added and prints twice on the ticket; and because `recalculateUnitPrice` sums the modifier of every selected option, a duplicated group with non-zero modifiers would apply it twice. Every Croustillants sauce is zero, so **no price on production is wrong today**. | **HIGH** — it silently corrupts the catalogue, which warning 4 calls real and irreplaceable, on every edit | **Batch 5.8** — opened 2026-09-08 at the operator's instruction |
| **L-66** ✅ **RESOLVED in Batch 1.4c** (2026-09-08 — found and fixed the same day, answering how the app reaches the till) | 2026-09-08 | Batch 1.4c, reading `install-windows.ps1` and `hibapos-server.ps1` for the operator's question | **The launcher does not refuse a repository that was never built.** `bun run start` is `next start`, which requires `.next/BUILD_ID`. A till cloned, installed and rebooted without `bun run build` comes up dead — and the launcher, which refuses a missing database (L-59), a pending migration, a missing `SESSION_SECRET` and, since 1.4b, a bun it cannot see (L-65), said nothing at all: `server.log` recorded a non-zero exit and no cause. **The same shape of silence as L-65, on the day after it**, and reachable by the ordinary path because **no document had a build step** (DOC-17). **Fixed** by refusal 5, grouped with refusal 4 because both answer *is this install finished?*, both are instant local checks and both were silent; it names `bun install` / `bunx prisma generate` / `bun run build` and `build.ps1`, and logs the BUILD_ID it found on success. **It checks `BUILD_ID` and not the `.next` DIRECTORY**, because `next dev` creates `.next` with no BUILD_ID and so does a build that failed halfway — revert R4 weakens it to the directory and a test fails. | MEDIUM (operational — a dead till on the first boot, with no cause in the log) | **Batch 1.4c** |
| **DOC-17** ✅ **RESOLVED in Batch 1.4c** (2026-09-08) | 2026-09-08 | Batch 1.4c, from the operator asking whether an installer exists | **Neither deployment document said how the application gets onto the machine, and one of them was wrong about it.** `grep` for `bun install`, `bunx prisma generate`, `bun run build`, `git clone` and `node_modules` across `docs/mise-en-service.md` returned **nothing** before this batch; `.zscripts/README-windows.md` § 1 listed only bun, Edge and a `.env` from `.env.example`. Both compressed the work into one prerequisite line. Two orderings inside the gap are load-bearing and were therefore unrecorded: **`.env` before `bun run build`** (`next build` throws at import time without `SESSION_SECRET`, and blames the import) and **the build before § 2's reboot** (which is L-66). Worse, `README-windows.md` § 5 asserted *"`install-windows.ps1` is the only path that puts one in place"* about the database — the installer **relocates** a database already in the install directory and prints `skip (absent)` when there is none, so on a fresh clone that sentence sends the reader hunting for a script that will make them a database, which is precisely what L-59 forbids anything here from doing. **Fixed** by a new **§ 0b** — git versus the ~705 KB carried by hand with sizes, the sequence, the two orderings as checkboxes, and the unresolved internet question with its 880 MB USB fallback — plus § 0's row repointed, a *Step 0* in `README-windows.md`, its prerequisites corrected, and § 5's claim corrected with a dated note. | MEDIUM (it is the first hour of the commissioning session, over a remote connection) | **Batch 1.4c** |
| **L-65** ✅ **RESOLVED in Batch 1.4b** (2026-09-07 — found and fixed the same day, while checking the runbook prerequisite it belongs to) | 2026-09-07 | Batch 1.4b, reading `hibapos-server.ps1` for § 0a | **The launcher's one silent refusal, and it is the likeliest of the four.** `hibapos-server.ps1` states its own rule — *"A refusal is loud: it writes to the log and exits non-zero, so the Task Scheduler entry shows a failure instead of a green tick over a dead till"* — and refusals 1 (no database), 2 (pending migrations) and 3 (no `SESSION_SECRET`) all obey it. A **bun that the account cannot see** did not. `$ErrorActionPreference = "Stop"` is set at the top; `& bunx prisma migrate status` throws `CommandNotFoundException`, which is terminating and is not caught, so the script died **before `$statusCode` was assigned** and refusal 2's `Fail` was unreachable. **Measured** against the real script with `Roaming\npm` off `PATH` and a scratch copy of the database: stopped at `hibapos-server.ps1:117`, exit 1, and the log's last line was `Checking migration status...`. The stderr went to a `-WindowStyle Hidden` Scheduled Task, which discards it. **Fixed** by a fourth refusal ahead of every use of bun, naming the account, both commands and the installer's three ways out — and logging which bun it found on success, so the log answers either way. | MEDIUM (operational — the failure mode is a till that does not start and a log that does not say why) | **Batch 1.4b** |
| **DOC-16** ✅ **RESOLVED in Batch 1.4b** (2026-09-07 — one commit old when found; `HEAD~1` was clean) | 2026-09-07 | Batch 1.4b, reading § 0 and § 0a to run them | **Commit `4ab1eef` corrupted both of the warnings it was written to add.** Backslash escapes in `docs/mise-en-service.md` were interpreted rather than written literally. **(1)** `%APPDATA%\npm` and `AppData\Roaming\npm` each became `…` + a real newline, splitting the § 0 bun row — the runbook's own *"most likely way this session goes wrong"* — across **three physical lines**. Markdown ends a table at the first non-row line and neither orphan contains a `\|`, so **the table ended after its first row** and the warning rendered only as far as `%APPDATA%`. **(2)** `C:\HibaPOS\bin` held a literal BACKSPACE (U+0008) where `\b` had been, inside the sentence naming the installer's third way out. **(3)** § 0a's `Copy-Item` and `cd` held a literal BEL (U+0007) where `\a` had been, in `C:\HibaPOS-rehearsal\app` — commands the operator was to paste that evening. **(4)** `REMEDIATION_PLAN.md`'s *Next Batch* carried the same `\n` split. A 263-file sweep for BEL, BACKSPACE, FORMFEED, VTAB, ESC and NUL found no others; the `CR`s in `next-env.d.ts` and `src/lib/db.ts` are CRLF line endings. **No guard in this repo could have caught it**: `deployment.test.ts` asserts the `.ps1` files are pure ASCII and **BEL and BACKSPACE are ASCII**, `plan-freshness.test.ts` does not read `docs/`, and nothing lints Markdown. | MEDIUM (delivery-blocking for a document a human reads at 07:00) | **Batch 1.4b** |
...
| **L-36** ✅ **RESOLVED in Batch 7.5** (2026-09-07 — the same defect as DOC-13, filed twice; both closed together) | 2026-09-04 | Batch 4.4c | **`ApprovalPayload.amount` is documented as euros and has always carried cents.** `approvals.ts:17` declares `amount: number | null; // euros`, but every caller binds cents: `refund/route.ts` passes `parsed.data.amount` (cents, per the `refundSchema` comment), `orders-view.tsx` passes `amountCents` by that name, and `payment-dialog.tsx` passes `discountTotal`. The HMAC therefore binds a cent figure while the type says otherwise, and the `tolerance ?? 0.001` in `verifyApprovalToken` reads as a floating-point euro guard when it is in fact an exact-integer-cent comparison. Nothing is mis-computed today — both sides agree — so this is a comment and a type-doc defect, not a money defect. It matters because Batch 4.4c's step-up now binds amounts through the same field, and the next person to add a caller will read the comment. Recorded rather than fixed: `approvals.ts` is not this batch's file (safety rule 10). **Fix by correcting the comment, not the code.** | LOW (documentation contradicts the implementation in a money path) | ⚠ **NO BATCH OWNS THIS** — 7.1 completed 2026-09-05 without it; its scope was the DOC table and four named comments, and this is a fifth. A one-line comment correction in a money path, still open. |
| **L-64** ✅ **RESOLVED in Batch 7.7** (2026-09-07 — the operator took the decision; verified by driving the app across six screens) | 2026-09-07 | Batch 7.6 | **Every `Button` size variant is under 44 px, so a button is touch-sized only when its call site says so.** `src/components/ui/button.tsx` declares `default h-9` (**36 px**), `sm h-8` (**32**), `lg h-10` (**40**) and `icon size-9` (**36**). The Phase 10 pass evidently knew — `min-h-[48px]` is scattered through `cart-panel.tsx` and the error boundaries — which is why L-09's eleven sites were *regressions* rather than an absent convention: the convention lives at the call sites, where it has to be remembered each time. **Batch 7.6 held the line at the call sites with a standing guard** (`touch-and-labels.test.ts`) rather than moving the base, and deliberately: raising `default` to `h-11` grows **every** button in the application by 8 px, in dense admin tables and dialogs, and **no session so far has been able to see this application render**. Measured 2026-09-07: 11 call sites needed rescuing, and the guard now refuses a twelfth. | LOW (ergonomics on a touchscreen; the guard prevents new instances, but every new button still starts undersized) | ⚠ **NO BATCH OWNS THIS — it is a design decision for the operator**, best taken after the commissioning session, when somebody has used the till for a day and knows whether the current density helps or hurts |
| **L-09** ✅ **RESOLVED in Batch 7.6** (2026-09-07 — eleven sites, not the two the row named) || Touch-target regressions from the Phase 10 pass: `h-9 min-h-[48px] w-9` (48 px tall, 36 px wide); a 28 px "Ouvrir" button on the blocking no-shift banner. | `payment-dialog.tsx:340`; `pos-view.tsx:451` |
| **L-10** ✅ **RESOLVED in Batch 7.6** (2026-09-07 — and its premise corrected: 21 of 22 icon buttons and 54 of 71 Labels were already right; the real gap was label association) || `aria-label` coverage absent from shifts, reports, orders, dashboard, settings, logs, audit and four dialogs, despite the Phase 10 claim. | `src/features/**` |
| **L-22** ✅ **RESOLVED in Batch 7.5** (2026-09-07 — fixed as a class, with zod's own French locale, not field by field) | 2026-09-03 | Batch 3.1d | **Validation errors reach the French UI as untranslated English zod messages.** `settings/route.ts` returns `parsed.error.issues[0]?.message`, and `settingsSchema` defines custom messages for only a few fields, so the operator saw `Too big: expected number to be <=48` (L-20). That specific message is now unreachable, but any other out-of-range settings value produces the same class of output. Applies to other schemas in `validation.ts` too. | LOW (operator-facing text) | ⚠ **NO BATCH OWNS THIS** — 7.1 completed 2026-09-05 without it. Note 5.7c fixed **one instance** of this class, not the class. Still open. |
| **DOC-14** ✅ **RESOLVED in Batch 7.5** (2026-09-07) | 2026-09-03 | Batch 1.2 | **Still present, re-measured 2026-09-07 at `:93`** (the citation below said `:110`). `src/components/pos/product-options-dialog-v2.tsx:93` computes `lineTotal = Math.round((unitPrice + addonsTotal) * qty * 100) / 100` and passes it to `formatEuro` at `:368-369`. `productUnitPrice()` returns integer cents (`cart-store.ts:225`) and add-on prices are cents, so `Math.round(cents * qty * 100) / 100` is exactly `cents * qty` — the displayed figure is **correct**, but the `* 100 / 100` is vestigial euros-era rounding that reads like a cents/euros confusion in a money path. Remove it or replace with a comment. | LOW (code clarity, not a defect) | ⚠ **NO BATCH OWNS THIS** — *(corrected 2026-09-07: the cell read "7.2", which completed 2026-09-05 without it. Invisible to `plan-freshness.test.ts` until the parser's bolded-rows-only hole was closed the same day.)* |
| **DOC-13** ✅ **RESOLVED in Batch 7.5** (2026-09-07 — the same defect as L-36) | 2026-09-03 | Batch 1.1 | **Still present, re-measured 2026-09-07 at `approvals.ts:23`** (the citation below said `:17`), and **this is the same defect as L-36 above** — recorded twice, once here against 1.1 and once there against 4.4c. Fix them together or merge the rows. `src/lib/approvals.ts:23` documents `ApprovalPayload.amount` as `// euros`. Every caller passes and verifies **cents** (`orders-view.tsx` → `/api/auth/approve` → `refund/route.ts:72`, and `payment-dialog.tsx` for discounts). Comment only — the code is unit-consistent and correct — but it is a misleading comment in the module that binds money to an approval, i.e. exactly the class of comment that produced C-01. | LOW (documentation) | ⚠ **NO BATCH OWNS THIS** — *(corrected 2026-09-07: the cell read "7.1", which completed 2026-09-05 without it. Invisible to `plan-freshness.test.ts` until the parser's bolded-rows-only hole was closed the same day.)* **Duplicate of L-36** |
| **DOC-15** ✅ **RESOLVED 2026-09-07 — the operator corrected the stored value.** Verified read-only the same day: `printerName` now reads `Sunso WTP-801`. The documentation half was already right (2026-09-03); this was the stale data value, and it was the one thing in the register a single Réglages edit could close. | 2026-09-03 | Batch 1.3 decision prep | **The documented printer is not the configured printer.** `IMPLEMENTATION_PLAN.md:15` names the *Sunso WTP-801*; the live `Setting` row says `printerName = "Epson TM-m30"`. **The operator confirmed on 2026-09-03 that the physical device is the Sunso WTP-801 and that it has an Ethernet port** — so the documentation is correct and the *stored setting value is wrong*. Nothing reads `printerName`, so this is cosmetic; the operator should correct the value in Réglages. Left open until that is done. | LOW (stale data value) | operator action |
| **L-58** ✅ **RESOLVED — BOTH HALVES** (label in Batch 3.10, stored per-line HT in Batch 3.11, migration applied and verified 2026-09-07) | 2026-09-06 | Batch 3.7 (the ISCA map) | **The ticket's « Caisse #N » is the shift number, not a till number, and no per-line HT total is stored.** BOFiP § 50 lists the line-by-line data in scope — as rendered by the research of 2026-09-06 (`docs/conformite-isca-recherche.md` § 9.6; re-read § 50 verbatim before acting): numéro du justificatif, date (année-mois-jour-heure-minute), numéro de la caisse, montant TTC, détail des articles (libellé, quantité, prix unitaire, total HT de la ligne, taux de TVA), mode de règlement, traces de modifications. Measured: `receipt.ts:58` prints `Caisse #${order.shift?.number}` — the shift counter (3 on production) on a single-till install, so a reader sees a third till; `OrderItem` (`schema.prisma:411-429`) stores `unitPrice` and `lineTotal` TTC and `vatRate`, and no HT line total — derivable, not stored. Everything else on the list is stored as such. Map § 6.2. **◐ AMENDED 2026-09-06 (Batch 3.10): the LABEL half is CLOSED and only the stored line HT remains.** The ticket now prints `Caisse N° 1` in the establishment block, from a literal carrying the one-database-one-till reasoning, and the shift number keeps its place under the truthful label `Service N` — verified end-to-end on a scratch copy, where a real sale rung on caisse session **3** (the production counter, the very number the finding names) printed both. **What stays open, and it is a real question rather than a formality**: storing the per-line HT needs an `OrderItem` column, therefore a migration and an operator action, and the HT the ticket prints is computed on the discounted net *after* a largest-remainder apportionment (`money.ts:120`) — so reproducing it from stored columns means re-running that algorithm rather than dividing by the rate. No source read so far says § 50 requires the figure stored rather than reproducible. **⚠ AMENDED AGAIN 2026-09-07 (Batch 3.11): the stored half is BUILT.** `OrderItem` gains `lineNetTotal` and `lineHt`, written inside the checkout transaction from the figures it already computes, so `Σ (net − HT)` equals the order's sealed `vatTotal` — asserted at one rate and at two. The net is stored beside the HT so the row is checkable by division rather than by re-running the apportionment, which was the finding's actual complaint. Rehearsed: one differing fingerprint line, the `_prisma_migrations` row. **The `prisma migrate deploy` is the operator's and has not been run.** | LOW (a derived rather than stored figure; **the label is fixed and the figure is now stored**) | **3.11** — `IMPLEMENTED — TESTING REQUIRED`; closes when the operator applies the migration |
| **L-63** ✅ **RESOLVED in Batch 1.3c** (2026-09-07 — the code half; the `[HW]` print stays with 1.3) | 2026-09-07 | Batch 1.3b | **The receipt renderer has a SECOND way to overflow the paper, and two more renderers carry the copied helper that had the first.** Batch 1.3b fixed the centred lines; three things it deliberately did not touch (safety rules 1 and 10). *(a)* **`leftRight()` overflows by a different mechanism**: `l + " ".repeat(Math.max(1, w - l.length - r.length)) + r` (`receipt.ts`) clamps the *gap* to one space, so an over-wide pair is emitted as `left + " " + right`, past the paper — a wrap is not the fix there, because the amount must stay on the right. *(b)* **The option and add-on lines are not laid out at all** — `  · ${choice}` and `  + ${name} (${price})` are raw `lines.push`. *(c)* **The same three-line `center()` helper is copied into `renderDayCloseTicket()` (`day-close-ticket.ts`) and `renderTestPage()` (`printer.ts`)**, both of which centre `restaurantName`; the day-close slip is a fiscal document the operator files with the books, and it will be produced for the first time on the first real trading day. **Measured 2026-09-07 against the real catalogue, read-only**: nothing overflows today at 48 columns — the longest product name is 21 characters (`Fanta Fruit de Dragon`, a 24-character item line against a ~40-character budget), the longest option choice 14, the longest add-on 15, and `restaurantName` is 9 — so this is latent, not live. It is one catalogue edit away: `productSchema.name` allows **80**, `userSchema.name` **60** (`Caissier : ` + 60 = 71 on 48-column paper), `categoryAddOnSchema.name` 60, and `categoryOptionGroupSchema.choices[].name` has **no maximum at all**. The operator adds product photos and names through Réglages routinely, so nothing but today's data prevents this. `wrapToWidth` is exported from `receipt.ts` and is a five-line move if the fix wants it in all three renderers. | MEDIUM (latent on today's catalogue; one operator edit from every ticket, on paper that goes live at the commissioning session) | ⚠ **NO BATCH OWNS THIS** — 1.3b was scoped to L-21, which names `renderReceipt()` and the three centred settings fields. *(a)* and *(b)* need a layout decision (truncate the label, or drop to two lines with the amount below) rather than a mechanical wrap, which is safety rule 11; *(c)* is mechanical. Cheapest to do before the commissioning session, in one batch with the `[HW]` print |
| **L-21** ✅ **RESOLVED in Batch 1.3b** (2026-09-07 — the code half; the `[HW]` print stays with 1.3) | 2026-09-03 | Batch 3.1b manual validation | **`renderReceipt()` centres but never wraps, so an over-long field overflows the paper.** A receipt rendered at the corrected 48 columns still contained a **56-character** line: the restaurant's real address, `23 Grande Rue 45210, 45210 Ferrières-en-Gâtinais, France`. On 48-column paper that wraps mid-address on every ticket. Distinct from L-14, which is about *archived* 80-column receipts — this is new output at the correct width. Affects any long `restaurantAddress`, `restaurantName` or `footerNote`. | MEDIUM (every printed ticket, once the printer is live) | **1.3** — the printing batch, still open on hardware, which is where this is felt. 3.4 completed without it |
| **L-61** ✅ **RESOLVED in Batch 2.5** (2026-09-06) | 2026-09-06 | Batch 8.2 (V-06 rehearsal) | ⚠ **The restore cannot complete on Windows while the application is running — and the application is the only way to reach it.** `restoreBackup` swaps the database with `fs.rename(staged, dbPath)`. The code does the right thing around it (`await db.$disconnect()` before, `$connect()` in `finally`), but **`$disconnect()` does not release the Windows file handle**: with the server up, an exclusive `File.Open(custom.db, ReadWrite, share=None)` fails with *"being used by another process"*, and the rename returns `EPERM: operation not permitted`. Reproduced twice, the second time on a freshly built scratch install, driving `POST /api/backups/[id]/restore` through the real route. Since that route is served by the process holding the file, it cannot free it. **This is the recovery path for a fiscal database (Batch 2.1, C-05) and it has never completed on this installation.** What holds: the failure is at the swap, *after* everything reversible and *before* anything irreversible, so the live database is never replaced, the chain stays `ok`, a `pre-restore-*.dbenc` snapshot is written first, and **no `RESTAURATION` is journalled for a restore that did not happen**. And the data IS recoverable by hand — proved end to end: events rolled back 3 → 2, grand total 6530 → 5480, all four chains `ok`, and a deleted product image restored at its exact byte size. **Not established whether it also bites in rollback-journal mode**; the rehearsal ran under WAL, which is the mode Batch 1.4's data move turns on. | **HIGH** (the documented one-click recovery for a fiscal database does not work on the target platform; manual recovery does) | ⚠ **NO BATCH OWNS THIS** — and it **blocks 8.2's V-06**. A fix belongs in `services/backup.ts` and is a code change to a fiscal recovery path, which a proof batch may not make (safety rule 10). Candidate directions, none investigated: copy-into-place instead of rename, an out-of-process swap at next start, or a launcher step |
| **L-62** ✅ **RESOLVED in Batch 2.5** (2026-09-06) | 2026-09-06 | Batch 8.2 (V-06 rehearsal) | **A failed restore leaves a 47,6 MB PLAINTEXT copy of every product photo on disk, and a safety snapshot the app cannot see.** Measured after the L-61 failure: `db/backups/` held `restore-<backupId>.uploads.tar.gz` — **unencrypted**, 47,6 MB, every image in the catalogue — beside `custom.db.restore-staged` in the data directory. Neither is cleaned up on this path; the `finally` blocks unlink the staged files only on the failures they anticipate. Separately, the `pre-restore-*.dbenc` written before the swap has **no `Backup` row**, because the row is created after the swap succeeds — so the route's own comment ("registered as its own Backup row… the operator can roll back with one click") is not true after a failure, and the file joins the ones L-46 describes. | MEDIUM (unencrypted media on disk after any failed restore; an invisible safety snapshot; litter that grows with each attempt) | ⚠ **NO BATCH OWNS THIS** — same file and same batch as **L-61**, and cheapest to fix with it |
...
| **DD-24** | **ANSWERED 2026-09-06 — the cut-off clock governs the month and the exercice as well.** June runs from 1 June at the cut-off hour to 1 July at the cut-off hour. Chosen so that a Friday service ending at 01:00 on 1 July sits in Friday **and** in June, and no two sealed documents disagree about the same tickets. **Free only because zero monthly and annual closes have ever been sealed** (re-verified 2026-09-06, and again 2026-09-07); this was the last moment it cost nothing. Implemented in Batch 3.8: `businessDayCutoffHour` (0–23, default 5) in Réglages and `settingsSchema`, with `businessDayOf` / `businessDayBounds` / `lastCompletedBusinessDay` in `src/lib/period.ts` and **`monthBounds` / `yearBounds` moved onto the same clock** — one convention, derived in one module, as L-25 established. Record → *Batch 3.8*. |
| **DD-25** | **ANSWERED 2026-09-06 — put a secret in the recipe, and print an integrity code the operator files with the books.** **Nothing in law requires it, and this row says so plainly**: BOFiP § 60 states « Le législateur n'a pas défini de cahier des charges, ni de solution technique », and § 140 names chaining **or** electronic signature as alternatives — HibaPOS chains, so the text is satisfied. What the operator chose to act on is the gap underneath: the recipe is readable in the source, so an administrator on the till could rewrite a sale **and** recompute every fingerprint after it, with `/api/fiscal/verify` still reporting `ok` — and that person is the assujetti, who is precisely who the regime exists to constrain. The private LNE referential (**not law**) requires a secret the assujetti cannot read. Implemented in Batch 3.9: `computeEventHash` / `computeCloseHash` become HMAC-SHA-256 under `FISCAL_CHAIN_KEY`, byte-identical without one; **arming happens at Batch 8.0's reset**, and arming over a journal that already holds unkeyed events is refused by design. **The limit was stated when the decision was taken**: on a till where the operator is administrator, a secret in a file on that machine is findable. Record → *Batch 3.9*. |
...
> Before those, on 2026-09-06: **2.5**, which fixed the restore (two PrismaClients held the database open, so `$disconnect()` never closed it — L-61, L-62); **8.2**, whose rehearsal found that and established L-46's cause; **8.1**, 27 checks with the Batch 0.2 baseline figures unmoved; **1.4**, the installer, launcher and update path plus L-59; **3.10**, the archive's missing rows and « date certaine » withdrawn from the notice and the signed attestation; and **8.0's reset script**, written and rehearsed. Records → those batches.

### carry-forward items 3 and 4 from *Last Updated* (session 19)

> (3) **Revert in both directions or the control is decoration.** Over-applying the wrap, and centring with `ceil` instead of `floor`, each broke pre-existing tests and both snapshots — which is what proved the untouched snapshots were real opt-in gates rather than a coincidence. (4) **Two of the batch's defects were found by its own tests and one by an old one** — the `leftRight` guard returned a line one character over the paper, and `role-model.test.ts` caught a test constant containing the removed role's name. Neither guard was weakened.

### *Ordering constraints* bullet on Batch 7.3 / DD-04 — the same fact is in *Open Threads → B*, in more detail
...
### the per-batch enumeration in the sealed-close-payloads bullet, which already pointed at the record for it

> `CLOTURE_Z` twice (3.6, 5.5); the sealed close `dataJson` — a different thing — four times (3.6, 3.6b, 5.5, 7.4a); and **3.8 added a fifth shape, `DailyClose`, plus the `CLOTURE_J` event type**. **3.8 was the last free change**:

### warning 2's history sentence

> It used to write real orders and sealed Z reports into **production**.

### the *Hardware-dependent validation* preamble on remote access

> The developer is in a different country from the restaurant. **Remote access to the POS machine is available on request** — the owner grants it on demand. So the deferral is about the app not being installed there yet, **not** about the machine being unreachable.

### warning 5's account of the two deleted scripts (L-37, C-17) and the counter guards (L-38)

> Two were deleted rather than guarded: `port-real-data.ts` (**L-37** — it wiped `db/custom.db` by a hardcoded literal, ignoring both `DATABASE_URL` and `HIBAPOS_DATA_DIR`, so the scratch-copy method did **not** protect against it) and `seed-category-options.ts` (**C-17**). `seed-users.ts` is now a PIN reset that deletes nothing, and the two counter scripts refuse to lower a fiscal counter (**L-38**).

### the *Last Batch* narrative of 1.3b and 1.3c

> **Last Batch:** **1.3c (L-63), 2026-09-07** — the receipt's *other* overflow, plus the two renderers carrying a copy of the first. `leftRight()` clamped the gap to one space and pushed the amount off the paper; the option and add-on lines were not laid out at all; `renderDayCloseTicket()` and `renderTestPage()` had their own `center()`. All three now share `services/ticket-layout.ts`, and **nothing is truncated** — BOFiP § 50 lists the article's libellé. **Ten reverts, both directions**; under the broadest the whole suite is 956/7 with every failure this batch's own. Before it, the same day: **1.3b (L-21)**, the centred lines — the restaurant's 56-character address on 48-column paper — proved on the real route and at the byte level, where under the revert the suite was 936/7, again all its own; and **SEC-ROT**, both secrets rotated by the operator and proved by the pre-rotation backup no longer decrypting. Before those, **2.5, 8.2, 8.1, 1.4, 3.10 and 8.0's reset script**, all on 2026-09-06 — one line each in the record's *Completed remediation history*, and a full section each above it.

### the *Ordering constraints* bullet's enumeration of Stage 3's run order
...
### the pre-6.2 test-count history in the *Tests* baseline

> Counts: 765 before 6.2 (11 vacuous cases out, 9 real ones in), 737 before 6.1, 715 before 5.7d.

### the *Production DB sha256* baseline's earlier-lineage sentence and the 3.10 re-verification detail
...
**RETIRED 2026-09-07 (session 19) — the *Hardware-dependent validation* expired-premise paragraph.** Compressed in the plan to two sentences and a pointer, for the ~40 KB ceiling. Nothing in it is withdrawn. Verbatim:

> **⚠ THIS SECTION'S PREMISE HAS LARGELY EXPIRED, 2026-09-07.** It was written when deployment was "deliberately on hold until the software is fit to ship". The software is now fit to ship: 1.4 built the installer, the launcher and the update path; 8.0's reset script is written and rehearsed; and `docs/mise-en-service.md` is the ordered session that installs and commissions all of it. **What is deferred is now only what a human must physically watch**, and it is a much shorter list than this section originally carried:

**RETIRED 2026-09-07 (session 19) — the *Last Updated* carry-forward of session 18.** Moved here so session 19's four lessons could take its place in the front matter without passing the ~40 KB ceiling. Nothing in it is withdrawn. Verbatim:

> **Last Updated:** 2026-09-07 (session 18 — **3.10, 1.4, 8.1, 8.2, 2.5, 8.0's script and the rotation**). Carry forward: (1) **A “done” is a claim until it is measured.** The operator reported the rotation done; the pre-rotation backup still decrypted, so it was not. The second report was checked the same way and was. **Find the one measurement that cannot be argued with** — here, AES-GCM refusing a file it had accepted hours earlier. (2) **Two batches diagnosed the same bug and the first was wrong.** 8.2 concluded `$disconnect()` does not release the Windows handle, from a test that proved *a* handle was held, not *whose*. Isolating it showed release in 4–9 ms and turned the question into “how many clients are there” — two. **A test that proves the symptom is not a test that proves the cause.** (3) **The dry run printed a flawless plan and the real run was wrong**: `Copy-Item -Recurse` into a pre-created directory nests instead of merging, so the till would have come up on correct fiscal data with no images and no backups, silently. (4) **Two reverts nothing caught were the best findings of their batches** (R7b, R17) — both exposed an assertion reading raw text where the file quotes its own history. Earlier sessions' lessons stand in the record.
...
*Moved verbatim from `REMEDIATION_PLAN.md` (commit `c8be286`) on 2026-09-07, at the operator's instruction, to keep the plan's front matter under its ~40 KB ceiling — the same reason and the same treatment as *Finding-ID prefixes* on 2026-09-05.*

**⚠ THIS SECTION IS REQUIRED READING, unlike the rest of this file.** Everything else here is evidence, sliced by heading when a batch needs it. This is the operating manual: nine methods that every session must know **before** it writes anything. `CLAUDE.md` names it for that reason, and the plan keeps a pointer where it used to sit.

**It was NOT retired for being stale.** Every method in it is current and in daily use — session 19 used the scratch-copy rule, the revert protocol and the browser-driving note in a single batch. It moved because, of everything above the plan's first stage heading, it is the one part that is **reference rather than status**: it does not change when a batch completes, and it was 10,5 KB of a 40 KB budget that had run out.
...
**RETIRED 2026-09-07 (Batch 1.4b) — the session-19 narrative from *Last Batch*, and two passages from *G*'s Tests row.**

**(1) *Last Batch*, 3 841 bytes of it.** It retold six batches that each have a full section in this record, which is the thing the plan's own rule forbids: *every fact has one home*. Retiring it took the front matter from **40 873 bytes to 38 990**, which is what made room for Batch 1.4b's status at all — there were **87 bytes** of headroom when this session started. What replaced it names the same six batches, their findings and one lesson each, and points here. Nothing in it was the only copy of anything. Verbatim:

> **Last Batch:** **7.7, 2026-09-07** — the Button primitive's base sizes, which 7.6 opened as L-64 and declined on two grounds: the decision was the operator's, and **no session had ever seen this application render**. The operator settled the first; **the second was settled by going and looking**, which turned out to be possible. `default` and `sm` are now 44 px, `lg` 48, `icon` 44 — and **103 of 144 buttons take those heights**, which is why 7.6's call-site guard could not see them. Verified across six screens including the POS and the category editor: nothing overlapping, nothing clipped, no row broken. **L-47 did not reproduce.** Before it: **7.6, 2026-09-07** — touch targets and label association, pulled forward from *Deferred*. **Eleven** undersized controls raised to 44 px, not the two L-09 names: three found by reading and sweeping, **eight more by the standing guard once it existed**, because they declare `className` on a different line from `<Button`. **L-10's premise was substantially wrong** — 21 of 22 icon buttons and 54 of 71 Labels were already correct; the real gap was label association, which the row never mentions. **Two measurements were wrong before anything was written**, both from cheap parsing, both caught by opening the files. **R5 was a revert nothing caught** and exposed a hole in the guard, now closed. Before it: **7.5, 2026-09-07** — the small findings nobody owned. **L-22** (English zod messages in a French UI) fixed as a **class**: zod 4 ships a French locale, so one `z.config` covers all 89 fields and 24 routes, and per-field messages still win so the 19 good ones are untouched. It is imported in **two** places and both are needed — 13 routes declare inline schemas and never import `validation.ts`. **R2 is the lesson: a unit test cannot prove that half**, because any test importing `validation.ts` configures zod process-wide; only the HTTP response can, and `POST /api/reports/z` answered `« Entrée invalide : nombre attendu, string reçu »`. **L-36 / DOC-13** (one defect filed twice) and **DOC-14** were the two one-line corrections. **L-14 and L-47 were excluded, with reasons** — neither is code work. Before it: **3.11 (L-58), 2026-09-07** — `OrderItem` gains `lineNetTotal` and `lineHt`, so « total HT de la ligne » is stored rather than reproducible only by re-running the discount apportionment. Written from the figures the checkout transaction already has, so `Σ (net − HT)` equals the order's sealed `vatTotal` — the assertion that would catch a second implementation of the split, and R3 proved it does. Archive schema 5 → 6. **Rehearsed, then applied by the operator and verified read-only the same day**: 10 migrations, counters 20/3/2/2 unmoved, all four chains `ok`, 82 legacy lines still `NULL`, and a fingerprint diff of one line — the `_prisma_migrations` row, exactly as the rehearsal predicted. Before it, the same day: **1.3c (L-63), 2026-09-07** — `leftRight()`'s overflow, the unlaid-out option and add-on lines, and the copies of `center()` in `renderDayCloseTicket()` and `renderTestPage()`. All three renderers now share `services/ticket-layout.ts`; **nothing is truncated**, on BOFiP § 50. Before it the same day: **1.3b (L-21)**, the centred lines — the restaurant's 56-character address on 48-column paper. **Between them, no line any of the three renderers emits can exceed the paper.** Ten and eight one-property reverts; under the broadest of each, the whole suite was 956/7 and 936/7 **with every failure the batch's own** — 934 pre-existing tests caught neither defect. Also that day: **SEC-ROT**, both secrets rotated by the operator and proved by the pre-rotation backup no longer decrypting. Before those, **2.5, 8.2, 8.1, 1.4, 3.10 and 8.0's reset script**, all on 2026-09-06. Records → those batches.

**(2) The e2e 13-versus-12 correction, from *G*'s Tests row.** Retired because it is the history of a correction rather than a current baseline, and `readme-counts.test.ts` now pins the number mechanically — including `auth.setup.ts`, because Playwright counts a setup project as a test. Verbatim:

> *(A de-stale pass earlier the same day "corrected" this to 12 from a static count of the five spec files. That count was right and the conclusion was wrong: `auth.setup.ts` logs in once per run in a Playwright **setup project**, and Playwright counts it as a test. The original 13 was correct. Pinned since by `readme-counts.test.ts`, which counts the setup file too.)*

**(3) A contradiction deleted from the same row rather than retired.** The row asserted **both** *"e2e: 13 passed — MEASURED 2026-09-07 by running the suite"* **and**, a few words later, *"the figure above is a static count rather than a run"*. Both cannot be true. The second was a leftover from the de-stale pass that wrongly changed 13 to 12 and was not withdrawn when that change was; the suite **was** run, so the clause was deleted rather than moved here. Recorded because a future session reading the retired parenthetical above should know the row it came from also contained its own contradiction.
