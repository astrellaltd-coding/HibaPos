...
**Current Batch:** none. **1.4c closed L-66 and DOC-17 on 2026-09-08**, from the operator's question hours before the remote session: *is there an installer?* Answered from the scripts rather than the runbook, and **nobody owned the question** — `install-windows.ps1` moves data out of the install directory and registers tasks; it does not fetch code, install dependencies or build, and **no document mentioned `bun install`, `prisma generate` or `bun run build` anywhere**. **§ 0b** is now that step: ~705 KB carried by hand (`db/custom.db`, the rotated `.env`), everything else from git, and the two orderings that bite — `.env` before the build, the build before the reboot. **L-66** is the launcher's fifth refusal, for the failure that gap produces. **What remains: 1.3's `[HW]` and 1.4's four `[MACHINE]` criteria**, both one session away; **8.0's reset**, which also **arms the chain key** (P-04, rehearsed); **8.2**'s trading day and **8.3**; V-01…V-03; and **L-52**, which waits on a format the administration has not published.

**Last Batch:** **1.4c, 2026-09-08** — the install step nobody had written down, and the refusal that was missing for it. **Its lesson is 1.4b's, one layer out**: 1.4b found a condition detected where a human was watching and not where nobody was; here every *step* was known to whoever had done it before and **none of it was written for the person who has not**. Two findings came out of the tests rather than the code, and both are recorded: the batch **shipped DOC-16's own defect in PowerShell** — a backtick inside an expanding here-string is an escape, so `` `bun run start` `` reached the operator as `<BACKSPACE>un run start`, invisible to the ASCII assertion and caught only by running it — and **a revert caught nothing** because a 1200-character window reached a neighbouring refusal's `Fail`, a pattern that had already weakened 1.4b's guard to 37 characters of margin. Before it, **1.4b, 2026-09-07**: § 0's two prerequisites measured, **L-65** (the launcher's one silent refusal) and **DOC-16** (the delivery-prep commit corrupting both warnings it added). Before those, **session 19's six batches on 2026-09-07** — **7.7** (L-64, and L-47 unreproducible a second time), **7.6** (L-09, L-10), **7.5** (L-22 as a class, L-36 / DOC-13, DOC-14), **3.11** (L-58's stored per-line HT, applied and verified), **1.3c** (L-63) and **1.3b** (L-21) — plus **SEC-ROT**. Records → those batches.

**Next Batch:** **DELIVERY IS TODAY, 2026-09-08.** The sequence is `docs/mise-en-service.md`, and **1.4b and 1.4c changed five things in it** — read it fresh, not from memory. **§ 0b is new and comes first**: the code from git, `db/custom.db` (704 KB) and the rotated `.env` carried by hand, then `bun install` / `bunx prisma generate` / `bun run build` — **`.env` before the build, the build before § 2's reboot**. Then § 0a's rehearsal if a spare machine exists, and it closes **1.4's four `[MACHINE]` criteria**. **Prerequisite 1 is still a live blocker: bun must be MACHINE-WIDE** — `%APPDATA%\npm\bun.ps1` on the user PATH here is the failing case; the binary to copy is `%APPDATA%\npm\node_modules\bun\bin\bun.exe`. **Prerequisite 2 is satisfied.** **§ 6a is a CHECK, not an action.** Then § 6's reset and the chain-key arming in that order, then the first real trading day (8.2 V-07). **Everything rung before § 6 needs FACTICE on.** **Two questions § 0b names and cannot answer**: whether the till has internet, and how long `next build` takes on that hardware.
...
**Last Updated:** 2026-09-08 (session 20 — **1.4b** and **1.4c**). Carry forward: (0) **A guard only counts where nobody is watching, and a step only exists if it is written for someone who has not done it.** 1.4 detected the per-user bun in the installer a human reads, not the launcher that runs as `SYSTEM`; and the whole install sequence was known to everyone who had done it and written nowhere. **Ask of every check: who reads it, and are they there?** (1) **Coverage is worth what a revert proves.** 934 tests caught neither of 2026-09-07's first two defects; 1.4b's four cases were caught by none of their 32 siblings; and **1.4c had a revert that caught nothing at all** because the assertion's window reached a neighbour. **Revert the fix and count who notices.** (2) **A fixed-width window in a source-level assertion decays as the file grows, silently** — bound it to its own block. (3) **Run the thing.** Three wrong numbers in one day came from cheap parsing; 1.4b's first snippet reported `1 chars` for a 64-character secret; and **1.4c shipped an escape bug into a French error message that only rendering it revealed**. (4) **A constraint recorded once is worth re-testing before inheriting it.** Earlier sessions' lessons stand in the record.
...
**Left open:** **1.4's four `[MACHINE]` criteria** and **1.3's `[HW]`**, untouched by this batch. **Two questions § 0b names rather than answers**: whether the till has internet, which decides `bun install` against carrying 880 MB of `node_modules`; and how long `next build` takes on all-in-one hardware. **A one-shot installer is a Stage 9 candidate**, once a till exists that can be re-imaged. A control-character guard for `docs/` remains unowned — the backtick guard added here covers `.ps1` here-strings only.
