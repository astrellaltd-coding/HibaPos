...
**Current Stage:** **Stages 0 and 2 through 7 are all COMPLETED** — Stage 3 reopened three times on 2026-09-06 (3.7, then 3.8/3.9, then 3.10) and closed the same day each time. Stage 1 is partly done — 1.1 and 1.2 COMPLETED, **1.3 `IMPLEMENTED — TESTING REQUIRED`** on hardware, **1.4 `NOT STARTED`**. **Stage 8 is `NOT STARTED`** and mostly waits on 1.3 and 1.4; 8.1 does not. C-22's chain-design half stays `REQUIRES EXTERNAL VERIFICATION` (V-01) and V-03 is open. Per-batch dates: the completion history in `REMEDIATION_RECORD.md`.

**Current Batch:** none. **Batch 3.10 completed 2026-09-06** and **Stage 3 is closed again**. What remains: **1.3's `[HW]` sign-off and 1.4**, blocked on the POS machine; **8.0's pre-go-live reset** after them, which now also **arms the fiscal chain key** (P-04, rehearsed); **8.1 and 8.2**; V-01…V-03 (external); **7.3's secret rotation**; **L-58's stored per-line HT**, which needs a migration and has no batch; and **L-52**, which waits on a format the administration has not published.

**Last Completed Batch:** Batch 3.10 — the three findings the ISCA map turned up and no batch owned. The annual archive carries **`refunds` and `cashMovements` as rows**, keyed on the date the money moved, so a refund paid in a later exercice stops falling out of every archive (schema **4 → 5**); the archive notice and **the signed attestation** both stop claiming « date certaine »; and the ticket prints a real **`Caisse N° 1`** with the shift number relabelled **`Service N`**. **L-58's stored per-line HT is deliberately left open** — it needs a migration. No migration, no production write. Record → *Batch 3.10*. Before it, 3.9 made the chain keyable (**the key is set on no machine**; arming is P-04's step) and 3.8 built the trading day.
