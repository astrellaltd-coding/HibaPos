...
**Awaiting decision:** **nothing — every design decision in this plan is answered.** DD-04 and DD-16, the last two, were answered 2026-09-05 on a measured brief, together with the two recorded-but-not-urgent questions (the callerless `/api/auth/approve` → delete in 7.2, and L-27 → done as Batch 3.6c). No batch is blocked on a decision; what remains blocked is blocked on hardware, and one thing is blocked on the operator (*Open Threads → B*). The answers and the measurements behind them: `REMEDIATION_RECORD.md` → *Answered design decisions*.
...
*Updated through Batch 5.5. **A and B both changed** — 5.5 shipped a feature that is inert until its migration runs, and that migration is now the operator's one blocking action. **F was retired**; its instruction moved into Batch 7.1. D gained 5.5's payload change. The *G* rows moved as usual.*
...
| **Entrée / sortie de caisse** (5.5) | The whole feature is inert: `CashMovement` does not exist on the live database, so the screen's button, the API and the new Z columns all fail there. **`main` will not start against production until the migration runs.** | Applying `20260905150626_cash_movements` — *B*, the one blocking action |
...
| Production DB sha256 | **`7839db18a7c8b132d974bd834d39d2921def66dd234b2059b022949f22ea6f2e`** (mtime 2026-09-04 16:41:52, 696 320 bytes) — last moved by the operator's **PIN change**, not by any batch. **Re-verified unchanged after Batches 5.3, 3.6c, 5.4 and 5.5** — 5.3, 3.6c and 5.5 validated on scratch copies, 5.4 being client-side only. 5.5's migration is **not** applied to it. The earlier lineage (`7cc3367b…` → `a66bc96c…` → `e40735ca…`) is in the record |
...
| Fiscal counters | `20/3/2/2` (receipt / shift / Z / event). Re-verified read-only 2026-09-05; **no batch this session moved any of them** — every write went to a scratch copy |
