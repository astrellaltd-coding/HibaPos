...
**Current Stage:** **Stages 0 and 2 through 7 are all COMPLETED** — **Stage 2 reopened 2026-09-06 for 2.5** (the restore could not complete on Windows) and closed the same day; Stage 3 reopened three times that day (3.7, 3.8/3.9, 3.10) and closed each time. Stage 1 is partly done — 1.1, 1.2, **1.3b**, **1.3c** and **1.4b** COMPLETED, **1.3 and 1.4 both `IMPLEMENTED — TESTING REQUIRED`**, waiting only on the till. **Stage 8 is `IN PROGRESS` — 8.1 `COMPLETED` 2026-09-06**; 8.0 and 8.2 wait on the commissioning session, 8.3 is external. C-22's chain-design half stays `REQUIRES EXTERNAL VERIFICATION` (V-01) and V-03 is open. Per-batch dates: the completion history in `REMEDIATION_RECORD.md`.

**Current Batch:** none. **1.4b closed L-65 and DOC-16 on 2026-09-07**, checking the two prerequisites the runbook calls the likeliest failures: **prerequisite 2 passes** (the `.env` here is the rotated one) and **prerequisite 1 FAILS and must be fixed before travel** — bun is `%APPDATA%\npm\bun.ps1`, on the user PATH only, so the `SYSTEM` task cannot see it. Reading the launcher to see what that failure looks like found **L-65**: it was the one refusal the launcher made *silently*, dying inside `& bunx` before its own `Fail` could run. **DOC-16** is the same day's delivery-prep commit having corrupted both of those warnings — a `\n` that became a real newline broke the § 0 table, and `\a`/`\b` became control characters in commands meant to be pasted. Session 19's six batches: record → 7.7, 7.6, 7.5, 3.11, 1.3c, 1.3b. **What remains: 1.3's `[HW]` sign-off and 1.4's four `[MACHINE]` criteria**, both one commissioning session away; **8.0's reset**, which also **arms the fiscal chain key** (P-04, rehearsed); **8.2**'s trading day and **8.3**; V-01…V-03; and **L-52**, which waits on a format the administration has not published.

**Last Batch:** **1.4b, 2026-09-07** — `docs/mise-en-service.md` § 0's two prerequisites, measured, and the two findings that came out of measuring them. Its lesson is the one this project keeps re-learning in a new place: **a guard in the place where a human is watching is not a guard.** Batch 1.4 already knew bun was per-user here — its note 6 says so — and put the detection in `install-windows.ps1`, which runs once with somebody reading the output; the launcher, which runs at every boot as `SYSTEM` with nobody watching, said nothing at all. The four `[MACHINE]` criteria are **untouched**: § 0a was not run, because there is no spare machine here and its second command registers Scheduled Tasks. Before it, **session 19's six batches on 2026-09-07** — **7.7** (L-64, the Button primitive's base sizes; it also found **L-47 unreproducible for the second time** by driving the app, which no session had done), **7.6** (L-09, L-10 — eleven undersized controls, eight of them found only by the standing guard, and L-10's premise substantially wrong), **7.5** (L-22 fixed as a class through zod's French locale, L-36 / DOC-13, DOC-14), **3.11** (L-58's stored per-line HT, migration applied by the operator and verified with a one-line fingerprint diff), **1.3c** (L-63) and **1.3b** (L-21) — after which no line any of the three ticket renderers emits can exceed the paper. Also that day: **SEC-ROT**, both secrets rotated by the operator, proved by the pre-rotation backup no longer decrypting. Records → those batches; the full session-19 narrative that stood here is in the record's *Retired open-thread rows*.

**Next Batch:** **DELIVERY IS 2026-09-08 (tomorrow morning).** The sequence is `docs/mise-en-service.md`, and **1.4b corrected four things in it** — read it fresh rather than from memory. **§ 0a rehearses §§ 1–2 on a spare Windows machine** and closes **1.4's four `[MACHINE]` criteria** where a mistake costs nothing; do it on a **copy** of the repo, never this tree, because the installer moves data *out of* the install directory. **Prerequisite 1 is a live blocker: bun must be MACHINE-WIDE.** It is `%APPDATA%\npm\bun.ps1` on the user PATH here — the failing case — and the fix is one of the installer's three options; the real binary is at `%APPDATA%\npm\node_modules\bun\bin\bun.exe`. **Prerequisite 2 is satisfied**: the `.env` here is the rotated one, verified against the pre-rotation copy. **§ 6a is now a CHECK, not an action** — the rotation is done and re-running it on the till would strand the new backup key there. Then § 6's reset and the chain-key arming in that order, then the first real trading day (8.2 V-07). **Everything rung before § 6 needs FACTICE on.**
...
**Last Updated:** 2026-09-07 (session 20 — **1.4b**; session 19 was 1.3b, 1.3c, 3.11, 7.5, 7.6, 7.7 and the `CLAUDE.md` reconciliation). Carry forward, and the first is new: (0) **A guard only counts where nobody is watching.** 1.4 detected the per-user bun in the installer, which a human reads, and not in the launcher, which runs unattended as `SYSTEM` — so the likeliest failure on delivery day was the one that printed nothing. **Ask of every check: who reads it, and are they there?** (1) **A test suite of 934 caught neither of 2026-09-07's first two defects, and 1.4b's own 32 sibling assertions caught none of its four.** **Before trusting coverage, revert the fix and count who notices.** (2) **Cheap parsing produced three wrong numbers in one day**, and 1.4b's first verification snippet reported `1 chars` for a 64-character secret while the verdict beside it was right. **Parse the thing, run the thing, or go and look.** (3) **Revert in both directions**, one property at a time, and take seriously a revert nothing catches. (4) **A constraint recorded once is worth re-testing before inheriting it** — 7.7 went and looked at a screen 7.6 was told could not be seen. Earlier sessions' lessons stand in the record.
...
| L-21 ✅ | 1.3b | L-63 ✅ | 1.3c | **L-65 ✅** | **1.4b** |
| DD-25 ✅ | 3.9 | **DOC-16 ✅** | **1.4b** | DOC-15 ✅ | operator |
| DOC-13 ✅ | 7.5 (with L-36) | DOC-14 ✅ | 7.5 | | |
