...
**Current Batch:** **1.3d and 8.0, both 2026-09-09.** **1.3d `IMPLEMENTED — TESTING REQUIRED`** — direct USB printing (L-70): the Sunso is on a USB type-B cable, the only transport was TCP, and the vendor INF (`USBPRINTSUNSOWTP-800036C`) settles it as a print QUEUE, so this is the RAW spooler transport DD-01 left a slot for. Validated byte-identical on a real queue and end to end over HTTP; the `[HW]` print stays with 1.3. **8.0 re-rehearsed** with L-72/L-73/L-74 fixed — the six menus are now VERIFIED through the wipe instead of unmentioned by it, and clients are deleted at the operator’s request. Details: those two batch sections.

**Last Batch:** **1.3d, 2026-09-09** — direct USB printing (L-70), `IMPLEMENTED — TESTING REQUIRED`. **Its lesson is R1.** Reverting the one line that WIRES the new transport into `resolvePrinter` was survived by all sixteen of its own unit tests — the transport proved perfect and nothing proved anything called it, which is 5.8, 3.12 and 5.9 in one sentence. The wiring file `printer-connection.test.ts` exists for that and fails three ways under R1. Second lesson: **the evidence settled the design.** `sunso.inf` binds `USBPRINTSUNSOWTP-800036C`, so the printer is a Windows print QUEUE and a COM transport would have been built for a device shape it does not have. Before it, **5.9 and 5.10, 2026-09-09** — menus composés, end to end, and the slot editor. Records → those batches; 5.9’s full narrative was retired from here on 2026-09-09 (record → *Retired front-matter passages*).

**Next Batch:** **8.0’s reset, run for real here**, then the trial copy ships to the owner (`CLAUDE.md` rule 6) and **§ 6 runs a SECOND time on his machine** before the first real sale, then the chain key and § 6f. After that only hardware and people: **1.3’s `[HW]`, which now includes 1.3d’s USB print**, 1.4’s `[MACHINE]`, 8.2’s V-07, and the accountant’s VAT-METHOD confirmation. *(Until 2026-09-09 this line said the six menus did not exist; they were built that morning.)*
...
**Last Updated:** 2026-09-09 (session 23 — **1.3d**, direct USB printing, `IMPLEMENTED — TESTING REQUIRED`; the builder’s picture grids were measured and fit, and **L-47 gained a fourth data point** when the browser pane again refused to advance a hydrated page. Before it, session 22 — **5.9** and **5.10**, both closed; the operator applied 5.9's migration and it was verified read-only). Carry forward: (0) **A unit test on an extracted rule proves the rule, not that anything calls it.** Both batches this session shipped that gap and both were caught by reverting the WIRING rather than the logic. (1) **Run the thing, and check the arithmetic against the answer.** 3.12's worked example ran against a build compiled before the fix; nothing but the number said so. (2) **A fallback that is safe by design is also silent by design** — 3.12's nullable column spared every existing row and thereby hid the operator's un-done step until the data was read. (3) **A repair script that exists is evidence of a cause nobody removed.** (4) Earlier sessions' lessons stand in the record.
...
| **Thermal printing + drawer** (1.3, 1.3d) | ⚠ **THIS ROW WAS STALE AND IS CORRECTED 2026-09-09**: `printerEnabled` is **`true`** on production and `printerHost` is **empty**, so every print attempt today answers *« Renseignez l’adresse IP »* rather than staying silent — very likely what prompted the operator to raise the printer at all. **And an IP was never going to be the answer**: the printer is on a USB type-B cable, so 1.3d’s `printerConnection` must be set to `usb` and the queue picked from the list. | **Runbook § 4a** (the driver, over AnyDesk) then § 4, on the real Sunso WTP-801 |
| **FACTICE simulation mode** (3.1b) | ⚠ **THIS ROW WAS STALE AND IS CORRECTED 2026-09-09: FACTICE is ON on production, and has been for some time** — 18 of the 33 sealed events are flagged `factice`, including every sale from #40 on. The row below described the opposite and was repeated in a session summary before anyone read the value. **The trial copy prepared 2026-09-09 has it deliberately OFF** (operator’s decision), which is the reverse of production. Any pre-go-live testing with it off is journalled as genuine trading. **It does NOT keep a sale out of the journal** — nothing can; it stamps the ticket and flags the row so 8.0 deletes it cleanly. | The operator turning it on — runbook § 3, before anything is rung up |
...
| **The keyed fiscal chain** (3.9) | Built and tested, and **`FISCAL_CHAIN_KEY` is set on no machine**, so the chain is plain SHA-256. Arming it on a journal that already has history is refused by design. ⚠ **The developer machine's journal is empty since 2026-09-10, which makes arming POSSIBLE there — and it must still NOT be done.** The key belongs to the RESTAURANT's machine, after ITS reset; arming it here would make a later reset refuse. | **The restaurant machine's P-04**, then the five steps — in that order |
| **The `Clôture du jour`** (3.8) | Shipped and reachable, and **zero closes of ANY kind exist on the developer machine since the reset of 2026-09-10** — the one monthly and one annual close sealed on 2026-09-09 went with it. Nobody has ever run a DAILY one. | The first real trading day, after go-live |
...
**Seven operator actions are DONE and have been retired** to the record → *Retired open-thread rows*: both secret rotations, the Réglages address, `printerName`, Batch 3.11's migration, and **Batch 5.9's migration, applied and verified read-only on 2026-09-09**.
...
- **The sealed close payloads grew five times and every one of them was free only because ZERO closes existed. ✅ THAT WINDOW IS OPEN AGAIN, as of 2026-09-10** — the operator`s reset deleted the one `CLOTURE_M` and the one `CLOTURE_A` that had shut it, so **every close payload shape is free to change again on this machine**, and stays free until the restaurant`s first real close. The paragraph below is kept because it is the argument that will apply again the moment one exists. *(It read « THAT WINDOW HAS PARTLY SHUT » from 2026-09-09 to 2026-09-10.)* Historically: **The sealed close payloads grew five times and every one of them was free only because ZERO closes existed.** Measured 2026-09-09: **one `CLOTURE_M` and one `CLOTURE_A` are sealed on production**, written by the operator at 01:30 and 01:31 that morning on empty periods. Their payload shapes are therefore **no longer free to change** while those rows exist — and they exist until **P-04 deletes them**, which it does, because this is development trading data. `DailyClose` still holds **zero** rows, so 3.8's shape is the one that is still genuinely free. **3.8 was the last free change** — it added the fifth shape, `DailyClose`, and the `CLOTURE_J` event type: it landed 2026-09-06 and **from the first real close every one of these shapes is fixed for good.** `close-timing.test.ts` pins the key list and has caught an unannounced change before — edit it deliberately, never to make a run go green. `CLOTURE_M` and `CLOTURE_A` event payloads have never been touched. Which batch added which field: record → *Retired open-thread rows*.
...
|---|---|
| Tests | **1247 pass, 0 fail** — measured 2026-09-09 after Batch 1.3d (1217 before it, and this row read 1172 until then, two batches stale). Earlier: **1172** after Batch 5.9 (1044 before it, and the figure in this row read 1007 until then, three batches stale; `readme-counts.test.ts` is what keeps `README.md` honest, not this table). Earlier counts: record → *Retired open-thread rows*. **and STABLE since Batch 6.3** — three consecutive whole-suite runs with no flake, where before it the suite failed about 2 runs in 5. **L-43 is FIXED, so a `shift-race.test.ts` failure is now a REAL failure**: investigate it. Do not re-run and dismiss it — that instruction stood here until 2026-09-05 and is withdrawn. **e2e: 13 passed — MEASURED 2026-09-07 by running the suite**, the first time it has been run in this project, and pinned since by `readme-counts.test.ts`, which counts `auth.setup.ts` because Playwright does. *(A de-stale pass the same day "corrected" 13 to 12 and was wrong; the history is in the record's *Retired open-thread rows*.)* **`bun run test:e2e` is SAFE** (6.3, T-10). **`readme-counts.test.ts` counts DECLARATIONS plus declared EXPANSIONS**, so a test generated by a loop must be declared there — 1.4c's guard was the third, and the failure message says so rather than inviting a one-digit edit. Whole-suite runtime **105–125 s**; a run of 296–422 s with the same 0 failures is **L-24**, not a defect; **`bun run test` carries the timeout itself since 7.4c**. Two sessions may now run the suite at once (warning 3b) |
| Production DB sha256 | **`c67b4b0b2e1c61e68b6461a4fa11f5344b48631c06a4071c4d2256cf89d30426`**, **884 736** bytes, mtime 2026-09-10 13:20 — after the operator’s reset of the developer machine. `integrity_check` ok, 0 FK errors, 12 migrations, **every trading table at zero**, catalogue whole (80 products, 6 menus, 19 slots). ⚠ **The file did not shrink**: SQLite frees pages for reuse rather than returning them, so the wiped database is the SAME 884 736 bytes as the one full of orders — **size cannot tell them apart, only the hash can**, which is what matters when carrying one to a till. Journal byte 18 still `1` (the WAL guard refuses this OneDrive path; `install-windows.ps1` is what changes that). Snapshot taken immediately before the reset: `db-snapshots/custom.db.before-golive-reset-2026-09-10-1319`, sha `4c72bfaa…`. Full pre-reset lineage and Batch 5.9’s 21-check migration evidence: record → *Retired front-matter passages*. |
| Fiscal chain | **EMPTY — `0` events, no `lastSequence`, MEASURED 2026-09-10** after the operator reset the developer machine. There is no chain left to verify on this machine, and that is the correct state for a database whose whole trading history was the developer`s own testing. **Still UNKEYED and it must stay that way here**: arming `FISCAL_CHAIN_KEY` is the RESTAURANT machine`s step, after ITS reset, and arming it on this one would make a future reset refuse. Superseded: **`lastSequence: 26` on production, MEASURED 2026-09-09** — the chain is linked end to end across all 26 events, recomputed read-only. *(This row said `lastSequence: 2` until then; it was true on 2026-09-04 and the operator has been trading development data since.)* All four chains `ok` since Batch 3.8 added the daily one. **UNKEYED, and that is current and correct** — Batch 3.9 built the keyed mode and `FISCAL_CHAIN_KEY` is set on no machine; arming is P-04's step. ⚠ **NO LONGER TRUE, and it was the premise of a design freedom** (see *Open Threads → D*). **One monthly and one annual close now exist**, sealed by the operator **2026-09-09 at 01:30 and 01:31** — `CLOTURE_M` for **2026-07**, `CLOTURE_A` for **2025**, both on empty periods, both development data **P-04 deletes**. **Zero DAILY closes still exist**, so 3.8's shape is the one still genuinely free. Re-verified read-only 2026-09-04 |
| Fiscal counters | **`0/0/0/0` — THE DEVELOPER MACHINE WAS RESET 2026-09-10**, by the operator, at their request: the 44 orders, 33 fiscal events and 2 customers were their own testing and are gone. 307 rows deleted, catalogue verified intact through it (34 checks). **This is NOT P-04** — P-04 runs on the RESTAURANT`s machine before its first real sale, and still has to. Snapshot before it: `db-snapshots/custom.db.before-golive-reset-2026-09-10-1319`. *(This row read `41/5/4/30` until then, and `43/5/4/32` before the reset.)* Superseded: **`41/5/4/30`** (receipt / shift / Z / event), 2026-09-09 19:18 — two more sales while testing the menus, #40 being that Menu Chill. Earlier that day **`39/5/4/26`, unmoved by Batch 5.9's migration** — a migration moves no counter, which is the point of checking. *(This row read `20/3/2/2` until then. It was true on 2026-09-06 and went stale as the operator's own development trading continued: sales #37 and #38 are Batch 3.12's own verification sales, named in this file's own status line.)* Historical: and **Batch 8.1 verified the whole live database read-only on 2026-09-06: 27 checks pass, no counter drift, no orphans, no migration drift** (`docs/verification-8.1-2026-09-06.txt`). Re-verified **2026-09-06 after Batch 3.8's migration** — a schema change moves no counter, and every batch write this session went to a scratch copy. `DailyClose` holds **zero rows** on production |
| Migrations | **12 applied on production**, latest `20260909143000_combo_menus` (Batch 5.9), applied by the operator 2026-09-09 and verified read-only. **None pending.** *(This row said 9 until 2026-09-09; 3.11's and 3.12's went in on the 7th and the 9th.)* Batches 4.1 through 4.4b added none — 4.4b's enum removal was measured with `prisma migrate diff` and emits an empty migration |
| Catalogue | **80 products in 14 categories, re-verified 2026-09-10 through the operator’s reset** — the six menus composés exist and survived it whole: **19 slots, 6 whitelist rows, 7 option rules**, listed back by name. **5.9g is DONE.** *(This row still carried « Zero combo rows … creating the six menus is the operator’s outstanding action » until 2026-09-10, contradicting its own opening sentence — a leftover from before they were built.)* **The rates are no longer a property of the product alone**: since Batch 3.12 the `Canette` and `Bouteilles` categories carry 10 % sur place and 5,5 % à emporter et en livraison, so the old « 17 drinks at 5,5 % » cannot be stated without naming an order type. Earlier lineage: record → *Retired open-thread rows*. |
...
| L-21 ✅ | 1.3b | L-63 ✅ | 1.3c | | |
| **L-70** | **1.3d** (code done; 1.3’s `[HW]` print still open) | **L-71** | none — out of scope, owned by nobody | | |
...
| **L-72** | **8.0** | **L-73** | **8.0** | **L-74** | **8.0** |
| **L-75** | operator (hardware — no software fix) | | | | |
...
## Batch 1.3d — Direct USB printing, alongside the network path (L-70)

**Status:** `IMPLEMENTED — TESTING REQUIRED` (opened and implemented 2026-09-09 at the operator’s request; blocked only on the physical printer, exactly as 1.3 is)

### L-70 — The only transport is TCP, and the restaurant's printer is on a USB cable

**Status:** `IMPLEMENTED — TESTING REQUIRED` · Severity: HIGH · Category: incomplete functionality / hardware

**Problem.** `resolvePrinter()` can build exactly one transport, `createTcpTransport`, and refuses with `NOT_CONFIGURED` when `printerHost` is empty. The Sunso WTP-801 in the restaurant is connected to the till by a **USB type-B cable**, not by Ethernet, so `printerHost` will be empty for ever and printing is unreachable on the real hardware.

**Evidence.** `printer.ts:47-62` — an empty host is the only branch, and its operator message is *« Renseignez l'adresse IP dans les réglages. »* `printer-transport.ts` exports one factory. Measured on the developer machine 2026-09-09: the vendor driver `SUNSO WTP-800` (installed by the operator at 19:50 that evening) declares its hardware as **`USBPRINT\SUNSOWTP-800036C`** in `sunso.inf` — a USB **printer-class** device, which Windows drives through a print queue, not a virtual COM port. That is what settles the transport: a Windows RAW spooler job, not a serial write.

**This is not a redesign.** DD-01 anticipated it exactly, and `printer-transport.ts`'s own header records the slot: *"A Windows RAW spooler transport for a USB-attached printer would implement the same interface and drop in here without touching the command layer."* The byte layer (`escpos.ts`) and the command layer (`printer.ts`) are unchanged by this batch except for one branch.

**Decision taken (operator, 2026-09-09).** Add a second transport and choose between the two with an **explicit setting** — « Connexion : Réseau (IP) / USB (imprimante Windows) » — rather than inferring the mode from which field happens to be filled. Rationale, in the operator's words: they did not know what the printer would be called or how it would present itself, and an explicit switch means the error messages can name the right thing instead of telling a cashier to check a network cable that does not exist. **The queue name is chosen from a list the app reads from Windows**, not typed: at the commissioning session nobody will know the queue's name in advance, and a typed name is a silent misconfiguration that surfaces as "nothing printed".

**Deliberately NOT in this batch.** No serial/COM transport — the INF says USBPRINT, so a COM transport would be built for a device shape this printer does not have. If the till turns out to differ, that is a new finding with its own batch.

### Batch 1.3d — Validation Required

- Unit tests for the spooler transport against an **injected** spawn, mirroring `printer-transport.test.ts`'s `FakeSocket` pattern: success, queue not found, spooler write failure, timeout, and an empty queue name.
- Unit tests that `resolvePrinter()` builds the transport the `printerConnection` setting names, and that each mode's `NOT_CONFIGURED` message names **its own** missing field.
- Unit tests for the new settings keys: defaults, validation bounds, and that an existing install with neither key set still resolves to the network transport it has today.
- The printer-list route returns the queues and refuses an unauthenticated caller.
- **`printerName` still reads as a label and nothing functional consumes it** — the new queue name is a separate key, so DOC-15's note stays true.
- `print-raw.ps1` passes the `.zscripts` guards: pure ASCII, UTF-8 BOM, no backtick inside an expanding here-string, and the parser accepts it.
- **Loopback validation against a real Windows print queue** using the real `SUNSO WTP-800` driver on a file port: the bytes that arrive must be byte-identical to the bytes `buildPrintJob()` produced, control codes and CP1252 high bytes included.
- Revert protocol: one property at a time, both directions, restored from a copy taken before the revert.
- `bun run test`, `typecheck`, `lint`, `build`.

- **[HW]** A real print over the USB cable on the restaurant's Sunso WTP-801, and a real drawer kick through it. This batch does not close 1.3's `[HW]` criteria; it adds a second path that needs the same physical proof.

### Batch 1.3d — Status Record

**Status:** `IMPLEMENTED — TESTING REQUIRED` (the code half is `COMPLETED` and validated; the **[HW]** print keeps this open, as it does 1.3)
**Completed:** — (code 2026-09-09; cannot be marked `COMPLETED` until a real print comes out of the restaurant's Sunso over its USB cable)
**Changes:** A second transport, chosen by an explicit setting, with the byte layer untouched. **(1) `.zscripts/print-raw.ps1`** — the piece Node cannot do: `Add-Type` compiles a P/Invoke into `winspool.drv` and calls `OpenPrinter` / `StartDocPrinter(RAW)` / `WritePrinter` / `EndDocPrinter`, so the ESC/POS reaches the printer without passing through the driver's rendering. `-List` mode enumerates the queues via `Win32_Printer`. Exit codes are the transport's error vocabulary: 2 queue-not-found, 3 spooler-refused, 4 short-write, 5 no-input. **(2) `createWindowsRawTransport`** in `printer-transport.ts` — the same `PrinterTransport` interface, injectable process runner, staging the job in an unguessable temp file deleted in a `finally` whether the print succeeded or not. **The job goes through a FILE and not stdin** because a receipt is binary — CP1252 high bytes and control codes below 0x20 — and PowerShell's stdin applies an encoding that corrupts exactly those. **(3) `PrinterError.connection`** — `"network" | "usb"`, defaulting to `"network"`, so every message that existed before this batch is byte-identical and the USB paths get advice naming the USB cable and the Windows picker instead of a network cable that does not exist. **(4) Two settings keys**, `printerConnection` (default `"network"`) and `printerQueue`; `resolvePrinter()` gained one branch. **`printerName` was deliberately NOT reused** — it is a label, its live value is `Sunso WTP-801`, and the actual Windows queue on the developer machine is `SUNSO WTP-800`; repurposing it would have addressed a queue that does not exist, and DOC-15's note that nothing functional reads it stays true, with a test. **(5) `GET /api/print/printers`** and a picker in Réglages, because **nobody knows what the printer will be called until its driver is on the till** — a typed name is a silent misconfiguration whose symptom is "nothing printed". **(6) `docs/mise-en-service.md` § 4a** — the AnyDesk driver install, the `pnputil` export/import, the `PortName` check that catches a queue bound to `COM1:`, and the generic-driver fallback if the WTP-800 INF will not match a WTP-801.
**Files:** `.zscripts/print-raw.ps1` (new), `src/lib/services/printer-transport.ts`, `src/lib/services/printer.ts`, `src/lib/services/settings.ts`, `src/lib/validation.ts`, `src/types/api.ts`, `src/app/api/print/printers/route.ts` (new), `src/features/admin/settings-view.tsx`, `docs/mise-en-service.md`, plus `src/lib/services/printer-spooler.test.ts` (new), `src/lib/services/printer-connection.test.ts` (new), and the four pinned-number guards: `deployment.test.ts` (ALL_SCRIPTS 7 → 8), `readme-counts.test.ts` (both expansions 7 → 8), `api-authorization.test.ts` (BOTH 30 → 31), `plan-freshness.test.ts` (open findings 8 → 10), `README.md` (1217 → 1247).
**Tests:** `bun run test` — **1247 pass, 0 fail** (1217 before; 30 added). `typecheck`, `lint`, `build` all pass. **Nine reverts, one property at a time, both directions, restored from copies taken before the revert.** R1 (the `resolvePrinter` usb branch) → 3 fail; **every one of the 16 spooler unit tests survived R1**, which is this project's recurring gap stated exactly: the transport can be perfect and the till still print nothing. R2 (`connection: "usb"` not passed) → 3; R3 (temp file never deleted) → 2; R4 (exit 2 no longer means queue-not-found) → 1; R5 (empty-queue guard removed) → 1; R6 (default flipped to `usb`) → 2; **R7, the opposite of R2 (the TCP path claims to be USB) → 5, one of them a PRE-EXISTING test** (`printReceiptText > returns a failure outcome instead of throwing`), which is the proof the network wording was not moved; R8 (job staged as text not bytes) → 1; R9, the opposite of R1 (the usb branch swallows network too) → 3. **Passing under no revert and named as such:** `printer.test.ts`'s 14 are controls — they inject a transport and never reach `resolvePrinter` — and `settings.test.ts`'s L-20 round-trip is a regression assertion, not coverage of this batch.
**Loopback validation (2026-09-09).** Against a **real Windows print queue using the real `SUNSO WTP-800` driver** — the vendor package the operator installed on this machine at 19:50 the same evening — on a file port, so every byte the spooler emitted could be read back. **All 256 byte values pass through unchanged (0 differing).** Then through the real code: `buildPrintJob()` → `createWindowsRawTransport` → `print-raw.ps1` → spooler → port, **byte-identical at 458, 94 and 7 bytes** for a test page, a receipt with accents and the euro sign, and a lone drawer kick; `ESC @`, `ESC t 16`, `GS V B 0` and `ESC p 0 25 250` all arrived, and `0xE9` (é) and `0x80` (€) survived. **Then end to end over HTTP** on a scratch copy of production (marker `SCRATCH-COPY-A7F3` read back from the pre-auth `GET /api/auth/profiles` before any write, build 20:29 newer than the source 20:27): `GET /api/print/printers` refused unauthenticated with **401** and listed three queues when authorised; `PUT /api/settings` persisted `usb` + the queue; `POST /api/print/test` answered `{"ok":true,"target":"HibaPOS Loopback","columns":48}` and the 48-character ruler arrived exactly 48 characters on one line. Both failure paths: a queue that does not exist → **502** `UNREACHABLE` with the USB message, no queue selected → **409** `NOT_CONFIGURED` naming the picker. The loopback queue and its port were removed afterwards; the operator's own `SUNSO WTP-800` queue and the driver were untouched.
**Commit:** `e1bc65f`
**Notes:** (1) **The transport question was settled by evidence, not by preference.** `sunso.inf` binds `USBPRINT\SUNSOWTP-800036C` — a USB printer-class device, which Windows always drives through a print queue — so a serial/COM transport would have been built for a device shape this printer does not have. It was deliberately not built. (2) **A resolved `send()` means the spooler accepted every byte, not that a ticket came out.** Identical limit to port 9100, and the reason the `[HW]` criteria stay open. (3) **~510 ms per job**, dominated by the PowerShell spawn and the `Add-Type` compile. Acceptable because printing happens after the sale is committed and is never awaited into the failure path; if it ever is not, that is a finding, not a tuning exercise. (4) **`printerEnabled` is `true` on production with `printerHost` empty** — measured 2026-09-09. So every print attempt on the live install is currently answering *« Renseignez l'adresse IP »*, which is very likely what prompted the operator to raise this. *Open Threads → A* says `printerEnabled` is `false`; that row is stale and § 4a now sets both correctly. (5) **L-71 was found and not fixed** (safety rule 10): `auth.ts:279`'s best-effort session touch has `.catch(() => {})`, but Prisma logs the failure before the catch, so 12 lines of `prisma:error` appear in a suite run — into the very log `docs/mise-en-service.md` tells the operator to read first. No test and no request fails. (6) **The browser pane failed again** — hydrated, empty console, API 200s, and neither a synthetic click nor React's own `onClick` invoked off the fibre props advanced the page. **That is L-47's fourth data point**, and it is why the builder-picture check below was measured rather than walked through. (7) The developer machine now carries the Sunso driver, which means the driver package can travel to the till by `pnputil /export-driver` — § 4a.1 has the exact commands.
...
- ~~**FACTICE must be on for the client's three-day trial**, where the old till is the system of record.~~ **VOID — the trial was cancelled 2026-09-09**; there is no client copy before installation. *(record, note 12, and its appended note of the same day)* **The underlying point survives it:** the sales rung on this machine were journalled as genuine because FACTICE is off, and § 6 is what clears them.
...
**Status:** `COMPLETED` · **Completed:** 2026-09-09 · **Commits:** `0e80c73`, `8955d60`, `8b4b48f`, `29398a2`, `830cd45`, `2725cef` and the documents commit · **Findings:** none from the audit — the batch is the operator's rulings of 2026-09-09
**Record:** `REMEDIATION_RECORD.md` → *Batch 5.9* — specification, the operator's rulings, validation criteria and status record, moved there verbatim on 2026-09-09.

**Migration `20260909143000_combo_menus` is REHEARSED AND NOT APPLIED to production.** It is the operator's to run: `bunx prisma migrate deploy`. Four in-place `ADD COLUMN`s and three `CREATE TABLE`s; the fingerprint diff on a copy of production showed exactly that and the `_prisma_migrations` row.

**Constraints this batch leaves behind** *(sentences copied from the record, not paraphrased)*
- **A COMBO MAY NEVER FILL A SLOT.** `Menu Eco` sits under `Pizzas`, so an « any pizza » slot would otherwise offer the menu itself; every filler lookup filters on `isCombo: false` for that reason. *(record, Changes; `combo.ts`)*
- **The policy document's figures were corrected to what the software computes, not the other way round** — six of nine rows, by a cent each. *(record, note 2)*
- **The accountant has not confirmed the allocation METHOD.** The rates are settled and live; the division of a forfait between them is not. Nothing here may be described as compliant. *(record, Changes; `combo.ts` header)*
- **The migration is hand-written and deliberately not what `prisma migrate diff` produced**, and `isCombo` came out LAST in `Product`'s column list — the proof it was an in-place add and not a rebuild. *(record, note 3)*
- **A menu with no slots FALLS BACK rather than being refused**; a MISMATCH between what the client sent and what the menu defines *is* refused. *(record, note 5)*
- **An absent `comboSlots` means « leave them alone », not « you have none »** — C-24's rule. A partial update that wiped a menu's slots would leave it selling at its forfait under the higher-rate fallback, over-taxing every sale with nothing on screen to say so. *(record, note 6)*
- **A slot surcharge is printed; a component's allocated share is not.** A test fails if 10,86 / 10,85 / 3,19 ever appear on a Menu Chill's ticket. *(record, note 7)*
- **The Réglages policy block is READ-ONLY and not a stored setting**: an editable field would let the stated method drift from the one `services/combo.ts` performs. *(`vat-allocation-policy.ts`)*
- **The production database's file hash moves on its own while the operator's app is open** — the fiscal fingerprint is the invariant to assert, not the sha. *(record, note 9)*
- **Eight of the nine worked cases did not go through a browser** and are named as such. *(record, note 10)*
- A paid add-on on a component whose resolved rate is not the component's own would book at the component's rate. **No such configuration exists** — add-ons are offered on `Pizzas` and `Sandwichs` only, both 10 % in every mode — and a test pins the current shape. *(record, Changes)*

**Left open:**
- **5.9g is the operator's and is NOT done.** The six menus must be created in the catalogue: the three pizza menus and the three Duo. **The Duo burgers — « Cheese Royal », « Chicken Royal », « Giant Royal » — do not exist as products yet**, and until they do no Duo worked example can be published, because there is no reference price to apportion by. Nothing in the code waits on this.
- **The method's own justification narrows if a Duo burger is never sold à la carte.** § 2 rests on « les prix pratiqués séparément lorsque les produits sont aussi vendus à la carte »; if those burgers exist only inside the Duo their reference price is notional. Added to the document's § 8 for the accountant; selling them separately would settle it.
- **`Box 15`, `Box 35`, `Tenders box` and `Box Bowl` are the same shape and are untouched** — forfaits containing a sealed bottle or can, sold as ordinary single-rate products, booking 100 % at 10 %. That errs high, not low, so nothing is under-declared. Recorded as **L-69**; not this batch's to fix (safety rule 10).

## Batch 5.10 — There was no way to enter a menu

**Status:** `COMPLETED` · **Completed:** 2026-09-09 · **Findings:** none from the audit — found by the operator asking how to create the six menus
**Record:** `REMEDIATION_RECORD.md` → *Batch 5.10* — specification, validation criteria and status record, moved there verbatim on 2026-09-09.

**No migration.** Batch 5.9's schema already held everything this screen writes.

**Constraints this batch leaves behind** *(sentences copied from the record, not paraphrased)*
- **THE FORM AND THE SERVER SHARE ONE VALIDATOR** — `slotFormErrors` hands the payload to `validateComboShape`, the function the routes run. A second implementation is how the two come to disagree about what a valid menu is. *(record, Changes)*
- **`candidateProducts` never offers a menu** — `Menu` is a child of `Pizzas`, so an « any pizza » slot reaches the menus themselves. *(record, Changes)*
- `governableGroups` reads the FILLERS' inherited groups, because `GET /api/catalog/categories` returns no option groups at all. *(record, Changes)*
- Turning the switch on also turns OFF the size group and category inheritance, or a menu would inherit `Pizzas`' required « Taille » and hang a size question the till never asks. *(record, Changes)*
- **The editor was NOT driven by hand** — the browser pane would not dispatch events to React. **It stays an [OWNER] check on the till**, the treatment Batch 5.8 gave L-67d. *(record, note 1)*
- **A test that passes its own argument in proves the argument, not the function** — R5's stale-choice guard survived its revert until the test stopped handing it `null`. *(record, Tests)*

**Left open:** creating the six menus is the operator's, and the three Duo burgers do not exist as products yet.
...
**Re-rehearsed end to end 2026-09-09** (the 2026-09-06 rehearsal is superseded: the database has traded since, and L-72/L-73/L-74 changed the script): **~180 rows deleted including 2 customers**, counters to `0/0/0/0`, **catalogue intact across all 16 preserved tables and the six menus listed back by name**, and the resulting database passes **all 28 of Batch 8.1's checks** with an empty chain reporting `ok` at `lastSequence: 0` (hard constraint 5). The app was then started on it and the first sale came out as **Caisse #1, Ticket N° 1, fiscal event #1, `GrandTotal` 1 050 / 1 order** — which is the whole point of the batch.
...
**Amendment, 2026-09-09 (L-72, L-73, L-74) — the first amendment written because the script was READ against the current schema rather than because a batch added a table.** Three changes, all rehearsed the same day:

- **The six composed menus join the KEEP list, explicitly.** `ComboSlot`, `ComboSlotChoice` and `ComboSlotOptionRule` postdate this decision (Batch 5.9) and appeared in **neither** list. **No data was ever at risk** — nothing deleted them — but the script signed off with *« Catalogue intact (14 tables verifiees) »* having never looked at a menu, and that sentence is read at the one moment in this project with no undo. `PRESERVED` is now **16** tables.
- **`Customer` moves from KEEP to DELETE**, at the operator's request of 2026-09-09: *« sales, orders, clients »*. **This supersedes the ruling of 2026-09-03** which kept customers deliberately. It is placed **after `Order`** in `DELETION_ORDER`, never before — `Customer` is the parent of `Order.customerId` and deleting a parent with children is an FK violation, not a cascade. Both live rows are development data (« Test » and the developer's own name).
- **Guard 2 is narrowed, not weakened (L-74).** It refused on *any* live server, including one holding a **completely different** database, which made this batch's own rehearsal impossible on a machine where the operator's till was running — and that is exactly how it was found. It is now skipped only in the mode this script already documents for rehearsals: **`--yes` AND a disposable `DATABASE_URL`**. On the real till `--yes` is refused outright, so the guard there is untouched. **Proved both ways** on 2026-09-09: with a live server on 3000 it refused the rehearsal (the finding); and `--apply --yes` against a copy at a non-disposable path was refused by guard 3 *before any deletion*, counters still 44/5/4/33.

**Re-rehearsed end to end 2026-09-09** on a copy of production taken at 44 orders. **Deleted:** 44 `Order`, 44 `Receipt`, 44+ `OrderItem`, `Payment`, 1 `Refund`, 5 `Shift`, 4 `ZReport`, 33 `FiscalEvent`, 1 `MonthlyClose`, 1 `AnnualClose`, 1 `GrandTotal`, 1 `Table`, **2 `Customer`**; counters to `0/0/0/0`. **Kept, and verified rather than asserted:** 80 products, 14 categories, **6 menus with 19 slots, 6 whitelist rows and 7 option rules**, 8 option groups, 39 choices, 21 add-ons, 2 users, 18 settings, 588 audit rows. `integrity_check` **ok**, **0** foreign-key errors. The six menus were listed back by name with their slots — *Menu Eco* still four seats, *Menu XXL* still three — because « 19 rows survived » is a count and « the menus still exist » is the claim.
...
|---|---|---|---|---|---|
| **L-75** | 2026-09-10 | installing on the restaurant's till | **The application cannot run on a 32-bit Windows, and nothing said so.** The till the owner intends to use reported *« Bun for Windows is only available for x86 64-bit and ARM64 Windows »*. Bun is the visible blocker; **Prisma is the real one and it is unfixable by changing runtime** — measured the same day from the PE headers of the engines this repo actually ships: `query_engine-windows.dll.node` and `schema-engine-windows.exe` are both `machine 0x8664` (x86-64), and Prisma publishes no 32-bit Windows engine. So on a 32-bit OS the data layer cannot load at all. **`docs/mise-en-service.md` § 0 listed bun, the `.env`, the printer and a backup volume — and never the machine's architecture**, which is the one prerequisite that cannot be satisfied on the day. Now stated there, with the trap that the bun message alone does not prove it: the installer reads the PROCESS architecture, so a 32-bit PowerShell on a 64-bit Windows prints the same refusal. | **HIGH** (blocks deployment entirely on the affected machine; costs nothing on a 64-bit one) **MEASURED ON THE TILL 2026-09-10: `DataWidth` **64**, `OSArchitecture` **32 bits**.** The CPU is 64-bit capable and only the installed Windows is 32-bit, so the hardware is not the problem. ⚠ **But a 32-bit Windows cannot be upgraded in place to 64-bit** — Microsoft has never supported it; it is a wipe-and-clean-install, which destroys everything on the machine. **And that machine is the one the restaurant is TRADING ON**, with the owner's existing POS on it, so reinstalling it for a *trial* would take his working till away. Compounding it: Windows 10 left support in October 2025, so a 64-bit Windows 10 reinstall would put an unpatched OS under a fiscal journal, and a machine old enough to ship with 32-bit Windows is unlikely to meet Windows 11's requirements. **Recommendation recorded for the operator: a separate 64-bit machine for the trial** — which is also exactly what parallel running needs. | **Operator action** — a separate 64-bit machine, or a clean 64-bit install on this one only when the restaurant actually switches and never for the trial. It is a hardware decision: no software change can address it, and no batch is waiting on it. |
| **L-74** | 2026-09-09 | rehearsing the wipe | **The reset’s “app must not be running” guard checked the wrong thing.** `pre-golive-reset.ts` refused whenever anything answered on port 3000 or 3001 — **including a server holding a completely different database** — so the batch’s own rehearsal method (apply to a scratch copy, diff the fingerprint) was impossible on a machine where the operator’s till was running. Found by it refusing exactly that. **Narrowed, not weakened**: skipped only for `--yes` **and** a disposable `DATABASE_URL`, which is the rehearsal mode the script already documents, and `--yes` is refused outright on anything else. The guard on the real till is untouched. | LOW (blocked a rehearsal; no data risk either way) | **Batch 8.0** — fixed and proved both ways 2026-09-09 |
| **L-72** | 2026-09-09 | the operator’s wipe request | **The pre-go-live reset does not know the six menus exist.** `scripts/pre-golive-reset.ts` names 15 tables it deletes and 14 it preserves, and **`ComboSlot`, `ComboSlotChoice` and `ComboSlotOptionRule` are in neither list** — they postdate it (Batch 5.9). The data is SAFE: nothing deletes them, and they hold 19 / 6 / 7 rows on production. What is not safe is the reassurance: the script ends by printing *« Catalogue intact (14 tables verifiees, aucun changement) »*, and it would print exactly that having never looked at a single menu. That sentence is read at the one moment in this project with no undo. | MEDIUM (no data loss; a false all-clear at the point of no return) | **Batch 8.0** — add the three tables to `PRESERVED`, and re-pin the count in the report |
| **L-73** | 2026-09-09 | the operator’s wipe request | **`Customer` is preserved and the operator has asked for it to be wiped.** The reset’s *WHAT IT KEEPS* comment lists customers deliberately, from the decision of 2026-09-03; on 2026-09-09 the operator asked for « sales, orders, clients » to go, so the earlier decision is superseded. Two rows on production. `Order.customerId` exists, and orders are already deleted before this would run, so the deletion is FK-safe wherever it is placed after `Order`. **This is a change of the operator’s own earlier ruling, not a defect** — recorded so the reversal is visible rather than silently applied. | LOW (2 rows; but it changes what a one-way script destroys) | **Batch 8.0** — move `Customer` from `PRESERVED` to `DELETION_ORDER`, after `Order` |
| **L-70** | 2026-09-09 | Batch 1.3d (operator report) | **The only printer transport was TCP, and the restaurant’s printer is on a USB type-B cable.** `resolvePrinter()` could build `createTcpTransport` and nothing else, and refused with `NOT_CONFIGURED` — *« Renseignez l’adresse IP »* — whenever `printerHost` was empty, which it will be for ever on this install. The vendor driver’s own INF declares `USBPRINT\SUNSOWTP-800036C`, a USB printer-class device, so Windows drives it through a print QUEUE: the fix is a RAW spooler transport, which is exactly the slot DD-01 left in `printer-transport.ts`. | HIGH (printing unreachable on the real hardware) | **Batch 1.3d** — code done and validated on a loopback queue; the `[HW]` print stays with 1.3 |
| **L-71** | 2026-09-09 | Batch 1.3d (test-run noise) | **The sliding session tracker logs a Prisma error for a write it deliberately ignores.** `src/lib/auth.ts:279` fires `db.session.update()` for `lastActivityAt` without awaiting it and swallows the rejection with `.catch(() => {})` — but Prisma logs the failure itself before the catch, so a vanished session row or a slow disk prints `prisma:error` regardless. Measured over one suite run: **12 lines from this one call site**, 5 « No record was found » and 7 « Socket timeout ». **No test fails and no request fails** — the write is genuinely optional. The cost is on the till: `docs/mise-en-service.md` tells the operator that `server.log`’s last line is the first thing to read when something breaks, and this writes noise into it that looks like a database fault and is not. | LOW (log noise only; no behaviour) | **NO BATCH OWNS THIS** — a one-line fix (log at debug, or drop the tracker) but it touches authentication, so it is a batch, not a change-log row |
| **L-69** | 2026-09-09 | Batch 5.9 | **Four products are menus in everything but name, and each contains a sealed drink.** `Box 15` and `Box 35` (29,90) carry a required « Choix de bouteille »; `Tenders box` (9,90) carries a required « Boisson » of eighteen cans; `Box Bowl` carries a frite choice. All four are ordinary single-price products, so the whole forfait books at the product's own 10 % — including the drink, which à emporter and en livraison is a 5,5 % item (L-68). **Nothing is under-declared**: taxing the lot at the higher rate is the direction `docs/politique-ventilation-tva.md` § 4 calls for when a forfait cannot be justified, so this errs high. What it costs is the relief the operator is entitled to on the drink, and a fiscal record that does not say what was sold. Batch 5.9 built exactly the mechanism these need and did not touch them (safety rule 10). Measured read-only 2026-09-09. | Low | **NO BATCH OWNS THIS** — converting them is catalogue work the operator can do with 5.9's admin screens once the six menus exist; only decide it after the accountant answers § 8. |
| **L-60** | 2026-09-06 | Batch 8.1 | **Eighteen of the twenty orders on production carry no `fiscalEventId`, and the journal holds only two `VENTE` events.** ⚠ **RE-MEASURED 2026-09-09: the defect is HISTORICAL AND BOUNDED, not ongoing.** Still exactly **eighteen** orders — but of **thirty-nine**, and precisely orders **#1–#18**. Every order from **#19 on carries one**, 21 of 21, matched by 21 `VENTE` events. The same boundary shows in the Z reports: **#1 and #2 (August) are sealed to no fiscal event; #3 and #4 (September) are.** Read against a 39-order database the original sentence suggests journalling is broadly broken; it is not. All development trading data, which P-04 deletes. Measured read-only, then traced rather than guessed: orders **1–18** were rung 2026-08-20 to 2026-08-29 01:38 and orders **19–20** on 2026-09-01, while the fiscal journal entered the checkout path in commit **`d7ceb18`** (2026-08-29), the day the repository was re-initialised at `be9113e`. So the eighteen predate the journal's existence. **Not a defect in today's code** — `checkout.ts:293` appends the `VENTE` inside the same transaction that creates the order and writes the back-link, and `sale-journal.test.ts` covers it. Consequence while it lasts: the Z reports (which aggregate `Order` rows) total 36 300 while `GrandTotal` (which counts only journalled sales) reads 5 480, and **both are correct about different questions** — Z#1 and Z#2 each equal their own shift's orders to the cent. Nothing can be reconciled across the two until the reset. | LOW (development data only; no code defect, and it never reaches real trading) | **8.0** — P-04 deletes all twenty orders before the first real sale, which is what resolves this. One more reason the reset is not optional |
