...
**Current Batch:** none. **Batches 3.8 and 3.9 both completed 2026-09-06**, and **Stage 3 is closed again**. What remains: **1.3's `[HW]` sign-off and 1.4**, blocked on the POS machine; **8.0's pre-go-live reset** after them, which now also **arms the fiscal chain key** (P-04, rehearsed); **8.1 and 8.2**; V-01…V-03 (external, better informed since 3.7); **7.3's secret rotation**; **L-55, L-56, L-58**, which have no batch; and **L-52**, which waits on a format the administration has not published.

**Last Completed Batch:** Batch 3.9 — the fiscal chain can be keyed, and arming it onto a journal that already holds unkeyed events is refused with a French `503` rather than a mute 500. **The key is set on no machine**; arming is P-04's step. Before it, Batch 3.8 built the trading day and L-57. Record → *Batch 3.9*.

**Next Batch:** **Stage 8 — final validation**, in order: **8.0's fiscal reset must run AFTER Batches 1.3 and 1.4**, both blocked on hardware, or commissioning puts fresh test sales into the journal just reset; **8.0 now also arms the chain key, and the order inside it matters** (P-04). **8.1's review work can start now.** Before any of it, the operator actions in *Open Threads → B*.
...
**Awaiting decision:** **nothing, and the decisions table is now empty.** DD-23, DD-24 and DD-25 were the last three, all put to the operator in plain till terms on 2026-09-06 and all answered. Full rows: `REMEDIATION_RECORD.md` → *Answered design decisions*.
