...
| Production DB sha256 | **`0ad41ae70803bae0f3ee6cc249be7f2615e12523f034250afa27676ded997f67`** (mtime 2026-09-09, **876 544** bytes) — the operator's own catalogue work: the six menus. `integrity_check` ok, no FK errors, 12 migrations. **AND A MENU HAS TRADED ON PRODUCTION** — ticket #40, a Menu Chill sur place at 29,40, the forfait apportioned exactly as the tests predict with its supplements outside it. Read-only; the figures are in the record → *Batch 5.9 → Appended 2026-09-09*. Superseded lineage: record → *Retired open-thread rows*. — **Batch 5.9's migration, applied by the OPERATOR** 2026-09-09 and verified read-only, **21 checks, all pass** — among them: `Product.isCombo` is the **LAST** column, which is the proof it was an in-place `ADD COLUMN` and not the generator's table rebuild; **no existing `OrderItem` row was backfilled**; counters unmoved; the 26-event chain linked end to end; and the newest order (#39, 10:37) predates this session. **The fingerprint diff against the pre-migration copy is 16 lines, every one the intended schema change or the `_prisma_migrations` row.** Full list: record → *Batch 5.9*. ⚠ **This file's hash moves on its own while the operator's app is open** — a session touch rewrites pages without changing a fiscal row (record → Batch 5.9, note 9), so **the fingerprint is the invariant to assert, not the sha**. *(Superseded: `d09369c0…`, 704 512 bytes, mtime 2026-09-07 15:33:56, from Batch 3.11's migration.)* Earlier lineage and 3.11's evidence: record → *Retired open-thread rows*. Journal byte 18 is still `1`. |
| Fiscal chain | **`lastSequence: 26` on production, MEASURED 2026-09-09** — the chain is linked end to end across all 26 events, recomputed read-only. *(This row said `lastSequence: 2` until then; it was true on 2026-09-04 and the operator has been trading development data since.)* All four chains `ok` since Batch 3.8 added the daily one. **UNKEYED, and that is current and correct** — Batch 3.9 built the keyed mode and `FISCAL_CHAIN_KEY` is set on no machine; arming is P-04's step. ⚠ **NO LONGER TRUE, and it was the premise of a design freedom** (see *Open Threads → D*). **One monthly and one annual close now exist**, sealed by the operator **2026-09-09 at 01:30 and 01:31** — `CLOTURE_M` for **2026-07**, `CLOTURE_A` for **2025**, both on empty periods, both development data **P-04 deletes**. **Zero DAILY closes still exist**, so 3.8's shape is the one still genuinely free. Re-verified read-only 2026-09-04 |
| Fiscal counters | **`41/5/4/30`** (receipt / shift / Z / event), 2026-09-09 19:18 — two more sales while testing the menus, #40 being that Menu Chill. Earlier that day **`39/5/4/26`, unmoved by Batch 5.9's migration** — a migration moves no counter, which is the point of checking. *(This row read `20/3/2/2` until then. It was true on 2026-09-06 and went stale as the operator's own development trading continued: sales #37 and #38 are Batch 3.12's own verification sales, named in this file's own status line.)* Historical: and **Batch 8.1 verified the whole live database read-only on 2026-09-06: 27 checks pass, no counter drift, no orphans, no migration drift** (`docs/verification-8.1-2026-09-06.txt`). Re-verified **2026-09-06 after Batch 3.8's migration** — a schema change moves no counter, and every batch write this session went to a scratch copy. `DailyClose` holds **zero rows** on production |
...
| Catalogue | **80 products in 14 categories, 2026-09-09 19:18 — the six menus composés EXIST**, built by the operator through Batch 5.10's editor: 19 slots, 6 whitelist rows, 7 option rules. **5.9g is DONE.** *(78 until the operator deleted the four ordinary-product « menus » on 2026-09-09 — `Menu Eco` and the three Duo — so that Batch 5.9 could rebuild them as real composed menus. The `Menu` and `Duo Meals` categories are now empty and waiting.)* **The rates are no longer a property of the product alone**: since Batch 3.12 the `Canette` and `Bouteilles` categories carry 10 % sur place and 5,5 % à emporter et en livraison, so the old « 17 drinks at 5,5 % » is not a statement that can be made without naming an order type. **Zero combo rows**: `ComboSlot`, `ComboSlotChoice` and `ComboSlotOptionRule` all hold 0 — **creating the six menus (5.9g) is the operator's outstanding action.** **Intact through 8.0's rehearsal**: all 14 preserved tables unchanged |
