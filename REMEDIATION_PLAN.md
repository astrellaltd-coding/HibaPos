...
**Current Batch:** **Batch 3.9** (DD-25, the integrity code), `NOT STARTED` and specified. **Batch 3.8 completed 2026-09-06** — the trading day is a sealed document, every close records the perpetual total, and **its migration is applied on production**, backed up first, with the rehearsal predicting the result exactly. Also remaining: **1.3's `[HW]` sign-off and 1.4**, blocked on the POS machine; **8.0's reset** after them; **8.1 and 8.2**; V-01…V-03 (external); **7.3's secret rotation**; and **L-52**, which waits on a format the administration has not published.

**Last Completed Batch:** Batch 3.8 — a sealed **Clôture du jour** on a trading-day clock that also governs the month and the exercice (DD-23, DD-24), and **L-57**: BOFiP § 170 requires every close to record the perpetual total and none did. **Migration applied on production 2026-09-06, zero differing fingerprint lines.** Twenty reverts, nineteen caught; the twentieth found that the Z report had been missed. Record → *Batch 3.8*.

**Next Batch:** **3.9** (DD-25 — the keyed fingerprint and the integrity code printed on the day-close slip, armed at 8.0's reset), then **Stage 8**, in order: **8.0's fiscal reset must run AFTER Batches 1.3 and 1.4**, both blocked on hardware, or commissioning puts fresh test sales into the journal just reset; **8.1's review work can start now.** Before any of it, the operator actions in *Open Threads → B*. **L-55, L-56 and L-58 have no batch yet.**
