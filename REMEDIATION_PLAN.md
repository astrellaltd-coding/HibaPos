...
*Updated through Batch 5.6. **Only G moved** — the test count, and L-43's observed rate. Batch 5.6 shipped no mechanism, waits on nobody and adds no ordering constraint, so A, B, C, D and E are unchanged. Before that, updated through Batch 5.5 and the operator's migration of the same day: **A and B both shed a row** — 5.5's feature is no longer inert and its migration is no longer pending. **F was retired**; its instruction moved into Batch 7.1. D gained 5.5's payload change. **G's production-database row changed for the first time since 2026-09-04** — check it before trusting any hash.*
