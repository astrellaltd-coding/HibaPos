...
2. **`bun run test:e2e` is SAFE since Batch 6.3 and this warning is lifted** — it prepares its own disposable database under temp, refuses to start if that path is not disposable, and runs on port 3100. **`bunx vitest` is still forbidden — warning 3.** Full history: record → *Retired open-thread rows*.
