...
**Current Batch:** **1.3d, `IMPLEMENTED — TESTING REQUIRED` 2026-09-09** — direct USB printing (L-70). The Sunso is on a USB type-B cable and the only transport was TCP, so printing was unreachable on the real hardware; the vendor INF settles the shape (`USBPRINTSUNSOWTP-800036C` — a print QUEUE, not a COM port) and the fix is the Windows RAW spooler transport DD-01 left a slot for. **Validated byte-identical on a real queue running the real SUNSO driver, and end to end over HTTP; the `[HW]` print stays with 1.3.** `docs/mise-en-service.md` gains **§ 4a**, the AnyDesk driver install. Before it, **5.10 closed on 2026-09-09** — the catalogue screen can now build a menu. 5.9 shipped the model, the till flow and the validation and left the product form sending neither `isCombo` nor `comboSlots`, so its own 5.9g was impossible until this. Before it, **5.9 closed on 2026-09-09** — menus composés, end to end: a data model for slots, a basket that can hold independently configured components, the slot-by-slot flow, the VAT allocation, the higher-rate fallback with admin validation to make it unreachable, and the composition on the ticket. **Its migration is REHEARSED AND NOT APPLIED — that is the operator's next action** (`bunx prisma migrate deploy`), and **5.9g, creating the six menus in the catalogue, is the operator's too**. Before it, **3.12 closed L-68 on 2026-09-09**, end to end — the migration and the category rates were applied by the operator the same day, and the live journal now shows the same 7 Up at **10 % sur place (#37)** and **5,5 % à emporter (#38)**. **Where the project actually is: not installed, § 6 not run, nothing traded.** Delivery did not happen on 2026-09-08. **The plan is to finish the work here, then install over an AnyDesk session and run `docs/mise-en-service.md` — § 6 included — at that point**; no copy goes to the restaurant before then. *(A three-day parallel trial was considered on 2026-09-08 and **cancelled on 2026-09-09**.)* `CLAUDE.md` rule 6 carries the same statement.
...
**Next Batch:** **none is specified.** What is ahead is not code: **the operator applies Batch 5.9's migration** (`bunx prisma migrate deploy`, rehearsed with a one-line fingerprint diff) and **creates the six menus** (5.9g) — for which the three Duo burgers must exist as products first, and they do not. Then § 0b's install, § 6's reset and the chain-key arming, 1.3's `[HW]` and 1.4's `[MACHINE]` criteria, and 8.2's V-07. **One thing is open and it is not code:** the accountant has not confirmed the VAT allocation METHOD. The rates are settled and live; the division of a forfait between them is the claim nobody qualified has signed off, and `docs/politique-ventilation-tva.md` § 8 now lists four points for them rather than three.
