...
**Awaiting decision:** **nothing. Every design decision in this plan is answered** — DD-04 and DD-16, the last two, were answered 2026-09-05 on a measured brief, along with the two recorded-but-not-urgent questions (the callerless `/api/auth/approve`, and L-27). No batch in this plan is now blocked on a decision; what remains blocked is blocked on hardware. Answers: DD-04 **rotate and accept the loss** (7.3), DD-16 **keep `public/uploads/` tracked and fix the documentation** (7.1), delete `/api/auth/approve` and `manager-approval-dialog.tsx` (7.2), and widen the close guard to **any caisse still open at sealing time** (**Batch 3.6c**, `COMPLETED` the same day).

**Last Updated:** 2026-09-05 (session 14 — Batches 5.3, 3.6c and 5.4, plus the last four decisions). Three things worth carrying forward. (1) **Do not trust a library's documented pattern without loading it.** 5.4's version guard was written the way zustand documents it, unit-tested, and green — and did nothing for the payload the finding names, because `migrate` is not called when the stored version key is absent. A test that imports the REAL module against a stubbed `localStorage` caught it; reading the code did not. (2) **A *Validation Required* can be sound and still incomplete** (5.3), and a batch's own earlier test can encode the decision you are now reversing (3.6c). Read the criteria for what they omit, and expect to invert rather than delete. (3) **Front matter is at ~40 5xx bytes of its ~40 960 ceiling**; the next session retires before it adds, and says in the record what moved.

### OPEN THREADS — read this before starting a batch

*Updated through Batch 5.4. Nothing in A–F changed in these batches; the *G* rows are what moved.*
