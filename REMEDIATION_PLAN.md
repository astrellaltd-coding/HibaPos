...
**Current Batch:** Batch 1.3 — Printing and cash-drawer strategy (`IMPLEMENTED — TESTING REQUIRED`; code complete, awaiting real-hardware validation)
...
**Status:** `NOT STARTED` · Severity: CRITICAL · Category: data integrity
...
**Status:** `NOT STARTED` · Severity: HIGH · Category: fiscal traceability
...
| DOC-15 | 2026-09-03 | Batch 1.3 decision prep | **The documented printer is not the configured printer.** `IMPLEMENTATION_PLAN.md:15` names the *Sunso WTP-801* as target hardware; the live `Setting` row is `printerName = "Epson TM-m30"`. Neither value is read by any code path (C-03), so the contradiction has been invisible. Resolve against the physical device before writing any transport code. | LOW (documentation vs reality) | 1.3 |
