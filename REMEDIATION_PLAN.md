...
**Current Stage:** **Stages 0 and 2 through 7 are all COMPLETED** — Stage 3 reopened on 2026-09-06 for Batch 3.7 (the French-law gaps) and closed the same day. Stage 1 is partly done — 1.1 and 1.2 COMPLETED, **1.3 `IMPLEMENTED — TESTING REQUIRED`** on hardware, **1.4 `NOT STARTED`**. **Stage 8 is `NOT STARTED`** and mostly waits on 1.3 and 1.4; 8.1 does not. C-22's chain-design half stays `REQUIRES EXTERNAL VERIFICATION` (V-01) and V-03 is open. Per-batch dates: the completion history in `REMEDIATION_RECORD.md`.

**Current Batch:** none. **Batch 3.8 completed 2026-09-06** — the trading day is a sealed document and every close records the perpetual total. **Its migration is prepared, rehearsed and NOT applied**; running it is the operator's (*Open Threads → B*). Next in this stage is **Batch 3.9** (DD-25, the integrity code), `NOT STARTED` and specified. Also remaining: **1.3's `[HW]` sign-off and 1.4**, blocked on the POS machine; **8.0's reset** after them; **8.1 and 8.2**; V-01…V-03 (external); **7.3's secret rotation**; and **L-52**, which waits on a format the administration has not published.

**Last Completed Batch:** Batch 3.8 — a sealed **Clôture du jour** on a trading-day clock that also governs the month and the exercice (DD-23, DD-24), and **L-57**: BOFiP § 170 requires every close to record the perpetual total and none did. **The migration is NOT applied to production.** Twenty reverts, nineteen caught; the twentieth found that the Z report had been missed. Record → *Batch 3.8*.

**Next Batch:** **3.9** (DD-25 — the keyed fingerprint and the integrity code printed on the day-close slip, armed at 8.0's reset), then **Stage 8**, in order: **8.0's fiscal reset must run AFTER Batches 1.3 and 1.4**, both blocked on hardware, or commissioning puts fresh test sales into the journal just reset; **8.1's review work can start now.** Before any of it, the operator actions in *Open Threads → B* — **now three, the migration included**. **L-55, L-56 and L-58 have no batch yet.**
...
**Awaiting decision:** **nothing.** DD-23, DD-24 and DD-25 were put to the operator in plain till terms on 2026-09-06 and all three are answered — a separate day close on a trading-day clock that refuses nothing, the same clock for the month, and a keyed fingerprint printed on the day close and filed with the books. **The operator's own research changed DD-23's question rather than picking from it**, which is the second time that has happened; DD-14 was the first. Full rows: `REMEDIATION_RECORD.md` → *Answered design decisions*.

**Last Updated:** 2026-09-06 (session 16 — **the ISCA conformity map `docs/conformite-isca-map.md`, the seven research questions, and Batch 3.7 opened**). Carry forward: (1) **Measure the artifact, not the message.** The first L-53 implementation read `package.json` and passed every test; `grep` of the built client chunks showed the whole file — dependency list included — shipped to the browser. Replaced by a pinned literal. (2) **A new assertion can weaken an old one**: once every ticket names the software, `toContain("HibaPOS France")` proves nothing about the restaurant-name fallback. Pinned to the header line, and shown to fail only in the new form. (3) **The map found what the plan's prose had not** — L-55, L-56, the chain covering the payload only, the perpetual total written into no close. Verify against the code. Session 15's lessons (assert the finding, run the revert, CI catches what `bun test` compiles away, the law moved) stand in the record.
...
|---|---|---|
| **Apply Batch 3.8's migration** | **`bunx prisma migrate deploy`**, after a backup. It creates `DailyClose` and adds the perpetual-total and cut-off columns. **Rehearsed on a copy with a fingerprint diff: 12 lines differ, all schema, and not one data row** (record → *Batch 3.8*). Until it runs, the code expects a table the live database does not have; nothing runs against that database today, which is why this is a hand-over and not an outage. | DD-23, L-57 |
...
| Fiscal chain | `/api/fiscal/verify` → **all FOUR chains** `ok` since Batch 3.8 added the daily one, `lastSequence: 2` on production. **Zero monthly and annual closes have ever been sealed** — which is why M-01's guard, DD-18's timing rules and the payload changes of L-26 **and Batch 5.5** could all be imposed with nothing to accommodate. Re-verified read-only 2026-09-04 |
...
| Migrations | **8 applied on production**, latest `20260905162220_remove_product_addons_and_postal_code` (Batch 5.7a), applied by the operator 2026-09-05 17:48:00. **⚠ ONE PENDING since 2026-09-06: `20260906153622_daily_close_and_perpetual_totals` (Batch 3.8), rehearsed and handed over — *Open Threads → B*.** Batches 4.1 through 4.4b added none — 4.4b's enum removal was measured with `prisma migrate diff` and emits an empty migration |
...
| L-55, L-56, L-58 | found by the map, no batch | DD-23 ✅, DD-24 ✅ | 3.8 | DD-25 | 3.9 |
...
**Twenty-four answered decisions have been retired from this table** (DD-23 and DD-24 on 2026-09-06, when Batch 3.8 completed them) (eighteen on 2026-09-05; **DD-20, DD-21, DD-22 and DD-04 on 2026-09-06**, when DD-23 needed the room), each closed with its batch `COMPLETED` and no forward link. Full rows: `REMEDIATION_RECORD.md` → *Answered design decisions*; the retired one-line pointers: → *Retired open-thread rows*.
...
**⚠ THE MIGRATION IS NOT APPLIED TO PRODUCTION.** It is prepared and rehearsed; running it is the operator's — *Open Threads → B*.
...
| **V-08** | `REQUIRES EXTERNAL VERIFICATION` | What must a compliant Z report and period close contain, and how must refunds and corrections be presented? (M-07, C-10) **RESEARCHED 2026-09-06.** BOFiP requires that the software provide *« obligatoirement une clôture journalière et une clôture mensuelle et annuelle […] cumulatives et impératives »*, with a grand total and a perpetual total per period. HibaPOS has all three and the perpetual `GrandTotal` — but its daily close is keyed to a **shift**, not to a calendar day: see **L-54**. **Mapped 2026-09-06 (map §3.5, §1.1): the perpetual total is maintained but written into NO close** — Z, monthly and annual payloads carry the period's figures only, and `GrandTotal` is snapshotted only into the annual archive — so whether « cumul perpétuel » must be *recorded at each close* or merely *maintained* is a question this row now carries; and a fully refunded order has its `status` flag written onto the original row (`refund.ts:157`), amounts untouched. **Researched 2026-09-06 (§ 9.5): BOFiP § 170 answers the first half — « Pour chaque clôture … doivent être calculées et enregistrées … le total perpétuel » — so the perpetual total not being recorded at each close is now a finding, L-57, not a question.** The status flag stays a question. |
...
**⚠ EIGHT ROWS HERE SAY *NO BATCH OWNS THIS*, and that is the plan's honest state**: Stages 0–7 are finished and these outlived them. **The three French-law gaps (L-52, L-53, L-54) got their batch on 2026-09-06 — Batch 3.7, on the operator's instruction.** It fixed L-53 and L-54, **left L-52 open because the format it needs is not published**, and its map and research added **L-55, L-56, L-57 and L-58** below — L-57 is the one with a clock on it (before the first real close).
