...
**Current Stage:** **Stage 5 is COMPLETED** — 5.1 through 5.6 and 5.7a–5.7d, all 2026-09-05, alongside **3.6c** the same day. **The next unstarted stage is 6 (TESTING).** **Stage 4 is COMPLETED** (4.1 through 4.4, 4.4b, 4.4c, 4.5, 4.6 and 4.7, all 2026-09-04). **Stage 3 is COMPLETED** (twelve batches; **3.6c** reopened it 2026-09-05 for L-27 and closed it the same day). C-22's chain-design half stays carried forward as `REQUIRES EXTERNAL VERIFICATION`, and V-03 is open. Stage 1 is **partly done**: 1.1 and 1.2 COMPLETED, 1.3 `IMPLEMENTED — TESTING REQUIRED` on hardware, 1.4 deferred. Stage 2 is COMPLETED.

**Current Batch:** none. **Stage 5 is finished**; Stage 6 (6.1, 6.2, 6.3) is `NOT STARTED` and is where the plan resumes. **5.7 was SPLIT on 2026-09-05 into 5.7a–5.7d** — it held twelve items across four risk classes, where every completed batch here has been one finding or a tight cluster. The router section carries the evidence, where each item and each criterion went, and three things measurement found that the rows did not say. Audit IDs are unchanged; the index still maps them to 5.7.

**Last Completed Batch:** Batch 5.7d — POS resilience, **closing M-20, M-21 and M-22 plus L-42, and with them Stage 5**. **M-21 is the one that lost money**: `fetchUser` folded *every* failure to `null`, which reached `operatorChanged(someone, null) → true → clearForOperatorChange()`, destroying the in-progress sale Batch 5.4 built persistence to protect. Three cases now — a 401 or an explicit `{ user: null }` signs out and clears; **anything else keeps the operator and their basket**. L-42 suppresses every shortcut while a modal is open, Escape left to Radix. M-20 tests the failure branch **before** the empty one. M-22 adds a per-view boundary — with an `inline` variant, or the fix would have been the symptom — and the `error.tsx` that never existed. **All three inherited criteria were *Manual*, L-47 blocked every one, and each was converted per 5.4's precedent.** *(Before it: 5.7c, 5.7b, 5.7a. Accounts: the record.)*

**Next Batch:** **Batch 6.1** — tests for the things that can lose money (T-01…T-07). Stage 6 is `NOT STARTED` and holds three: **6.1**, **6.2** (remove misleading tests — **L-49** is now an instance of exactly that), and **6.3**, which owns the two things every session has had to work around — **L-43**'s flaky race and **L-47**, the browser pane that cannot reach an authenticated view. **Clearing L-47 first would unblock manual validation for every batch after it**, including the three criteria 5.7d had to convert.
...
**Last Updated:** 2026-09-05 (session 15 — Batches 5.6, 5.7a, 5.7b, 5.7c, 5.7d and the 5.7 split; **Stage 5 completed**). Three things to carry forward. (1) **Four tripwires from earlier batches fired this session and every one was amended or inverted, never adjusted to whatever the code returned** — C-25's image-column count, C-21's view table, C-23's cart version, and C-23's session test, which had to be **inverted** because it encoded the behaviour M-21 names as the defect (3.6c's shape). (2) **Drive it, or you will not see the operator's text**: 5.7c's new bound answered in zod's English until the walkthrough showed it, and 5.7a's surviving media scan was deleted despite the trap being written down first. (3) **When a revert catches nothing, ask whether it took effect before concluding the test is weak** — a `schema.prisma` revert without `prisma generate` is a no-op. **Front matter: retire before you add, and measure at the END of a session.**

### OPEN THREADS — read this before starting a batch

*Updated through Batch 5.7d, which **completes Stage 5**. **Only G has moved since Batch 5.7a's migration** — the test count, batch by batch. None of 5.7b, 5.7c or 5.7d ships a mechanism, needs a migration or waits on anybody, so **A, B, C, D and E are unchanged**. This thread records the CURRENT state, not its history: per-batch update notes live in each batch's record section.*
