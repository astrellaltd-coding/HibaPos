...
**Current Stage:** **Stage 5 is IN PROGRESS** — **5.1 through 5.6, 5.7a and 5.7b COMPLETED 2026-09-05**, alongside **3.6c** the same day. **5.7d alone remains of Stage 5.** **Stage 4 is COMPLETED** (4.1 through 4.4, 4.4b, 4.4c, 4.5, 4.6 and 4.7, all 2026-09-04). **Stage 3 is COMPLETED** (twelve batches; **3.6c** reopened it 2026-09-05 for L-27 and closed it the same day). C-22's chain-design half stays carried forward as `REQUIRES EXTERNAL VERIFICATION`, and V-03 is open. Stage 1 is **partly done**: 1.1 and 1.2 COMPLETED, 1.3 `IMPLEMENTED — TESTING REQUIRED` on hardware, 1.4 deferred. Stage 2 is COMPLETED.

**Current Batch:** none. **5.7d is next, and it is the last of Stage 5.** **5.7 was SPLIT on 2026-09-05 into 5.7a–5.7d** — it held twelve items across four risk classes, where every completed batch here has been one finding or a tight cluster. The router section carries the evidence, where each item and each criterion went, and three things measurement found that the rows did not say. Audit IDs are unchanged; the index still maps them to 5.7.

**Last Completed Batch:** Batch 5.7c — pricing and validation, **closing M-19, M-12, M-15 and M-16 plus L-41**. **M-19 survived because the tests built the shape the dialog never produced** — a hand-made `CartItem` with `priceModifier: 0, // dine-in`, which is exactly the shape the component did not write — so the mapping was **extracted** from the component into `toCartOptions` and every case now goes through it. M-15 **refuses** a negative unit price rather than clamping, because a clamp sells the item free and silently; measured first, so the refusal breaks nothing that exists. M-16 bounds quantity at 99 against a measured maximum of 2. M-12 corrected the comment, not the code. **L-41 is narrowed, not closed** — the in-transaction assertion is still the guarantee. **All 19 new tests fail under some revert, including the control.** *(Before it: 5.7b, closing M-11; 5.7a, closing M-09 and M-10; 5.6, closing M-08. Accounts: the record.)*

**Next Batch:** **5.7d** — POS resilience: M-20 (the grid's false empty state), M-21, M-22 and **L-42**. No migration. It is the slice whose three *Manual* criteria **L-47 blocks**, so read Batch 5.4's precedent for converting them into automated coverage rather than dropping them, and **M-21 is worse than its row records** — a transient `/api/auth/me` failure clears the cart, not just the session. **Stage 5 ends there.**
...
**Last Updated:** 2026-09-05 (session 15 — Batches 5.6, 5.7a, 5.7b, 5.7c and the 5.7 split). Three things to carry forward. (1) **A test that builds its fixture by hand proves nothing about the code that builds it for real.** M-19 lived for months behind `cart-store-math.test.ts`, which hand-made the ideal `CartOption`; the fix was to EXTRACT the component's mapping so the test and the till run one function. (2) **Drive it — reading will not show you the operator's text.** 5.7c's new quantity bound answered in zod's English until the walkthrough showed it; 5.7b's four refusals and 5.7a's route removals were the same. (3) **Three tripwires from earlier batches fired this session** (C-25's image-column count, C-21's view table, C-23's cart version) and every one was amended with a dated comment, never adjusted to whatever the code now returned. **Front matter: retire before you add, and measure at the END of a session.**

### OPEN THREADS — read this before starting a batch

*Updated through Batch 5.7c. **Only G moved** — the test count. Neither 5.7b nor 5.7c ships a mechanism, needs a migration or waits on anybody, so A, B, C, D and E are unchanged. Before that, updated through Batch 5.7b, when also **only G moved** — the test count; 5.7b ships no mechanism, needs no migration and waits on nobody, so A, B, C, D and E are unchanged. Before that, updated through Batch 5.7a **and the operator's migration of the same day**: **A and B each gained a row and then shed it within the hour** — 5.7a's migration was handed over and applied at 17:48, so its removal is no longer inert and nothing waits on the operator. G moved: the test count, the migration row and **the production hash**. C, D and E are unchanged. Earlier per-batch update notes were retired to the record on 2026-09-05 (Batch 5.7a) — this thread records the CURRENT state, not its history.*
