...
**Current Batch:** none. What remains: **L-57 — the perpetual total is recorded in no close, and BOFiP § 170 says it must be — which has to land BEFORE the first real close** and has no batch yet; **Batch 1.3's `[HW]` sign-off and 1.4**, blocked on the POS machine; **8.0's pre-go-live fiscal reset** after them; **8.1 and 8.2**; C-22's chain-design half and V-01…V-03 (external); **7.3's secret rotation**, prepared and waiting on the operator; and **L-52**, which waits on a format the administration has not published.
...
**Next Batch:** **L-57 first** — seal the perpetual total into the Z, monthly and annual closes while zero real closes exist; a small fiscal batch, and it must precede 8.0. Then **Stage 8 — final validation**, in order: **8.0's fiscal reset must run AFTER Batches 1.3 and 1.4**, both blocked on hardware, or commissioning puts fresh test sales into the journal just reset; **8.1's review work can start now.** Before any of it, the two operator actions in *Open Threads → B* and DD-23. **L-55, L-56 and L-58 have no batch yet.**
...
**Awaiting decision:** **DD-23** (2026-09-06) — should the till refuse sales once the open caisse has crossed midnight, or after a cut-off hour, until a Z is run? Written in till terms in *Design Decisions Required*; **it blocks nothing** — today the screen warns and refuses nothing. Every earlier decision is answered (`REMEDIATION_RECORD.md` → *Answered design decisions*).
...
| L-53 ✅, L-54 ✅ | 3.7 | L-52 | 3.7 searched — **open, no format published** | L-55…L-58 | found by the ISCA map and research, no batch (**L-57 before the first real close**) |
