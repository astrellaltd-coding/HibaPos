...
**Current Stage:** **Stages 0 and 2 through 7 are all COMPLETED** — Stage 3 reopened three times on 2026-09-06 (3.7, then 3.8/3.9, then 3.10) and closed the same day each time. Stage 1 is partly done — 1.1 and 1.2 COMPLETED, **1.3 and 1.4 both `IMPLEMENTED — TESTING REQUIRED`**, waiting only on the till. **Stage 8 is `NOT STARTED`** and mostly waits on 1.3 and 1.4; 8.1 does not. C-22's chain-design half stays `REQUIRES EXTERNAL VERIFICATION` (V-01) and V-03 is open. Per-batch dates: the completion history in `REMEDIATION_RECORD.md`.

**Current Batch:** none. **Batch 1.4 built 2026-09-06** (startup, supervision, update path, and the app is now installable). What remains: **1.3's `[HW]` sign-off and 1.4's four `[MACHINE]` criteria**, both one commissioning session away; **8.0's pre-go-live reset** after them, which now also **arms the fiscal chain key** (P-04, rehearsed); **8.1 and 8.2**; V-01…V-03 (external); **7.3's secret rotation**; **L-58's stored per-line HT**, which needs a migration and has no batch; and **L-52**, which waits on a format the administration has not published.

**Last Batch:** **1.4 — `IMPLEMENTED`, not `COMPLETED`.** Task Scheduler starts the server at boot (restart 3x/1min) and the kiosk at logon; `install-windows.ps1` moves data to `C:HibaPOSdata`; `update.ps1` ships fixes without touching data; a manifest and real icons make it installable. **L-59 found and fixed**: `start.ps1` used to create and seed a database on a wrong path, with the PINs this repo publishes. **The data move is what turns WAL on** — measured, byte 18 `01` on production, `02` on a copy outside OneDrive. Record → *Batch 1.4*. Before it, Batch 3.10 — the three findings the ISCA map turned up and no batch owned. The annual archive carries **`refunds` and `cashMovements` as rows**, keyed on the date the money moved, so a refund paid in a later exercice stops falling out of every archive (schema **4 → 5**); the archive notice and **the signed attestation** both stop claiming « date certaine »; and the ticket prints a real **`Caisse N° 1`** with the shift number relabelled **`Service N`**. **L-58's stored per-line HT is deliberately left open** — it needs a migration. No migration, no production write. Record → *Batch 3.10*. Before it, 3.9 made the chain keyable (**the key is set on no machine**; arming is P-04's step) and 3.8 built the trading day.
