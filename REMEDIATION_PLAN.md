...
**Current Batch:** **Batch 3.8 — the trading day**, `NOT STARTED`, **specified 2026-09-06 and waiting on the operator to read the spec before any code is written** — their choice, asked and answered. It carries DD-23, DD-24 and **L-57**, the one open finding with a deadline: BOFiP § 170 requires the perpetual total to be recorded at every close and HibaPOS records it at none, which is free to fix while zero closes exist and permanent after the first. **Batch 3.9** (DD-25, the integrity code) is specified behind it and arms at 8.0.

**Last Completed Batch:** Batch 3.7 — the French-law gaps: L-53 fixed (the software states its version), L-54 fixed as far as the research allows (the refusal is DD-23), **L-52 left open because no restitution format is published**. It produced `docs/conformite-isca-map.md` and the seven answers in `docs/conformite-isca-recherche.md` § 9. Before it, 7.3's rotation is **still NOT done** (*Open Threads → B*). Record → *Batch 3.7*.

**Next Batch:** **3.8**, then **3.9**, then **Stage 8 — final validation**, in order: **8.0's fiscal reset must run AFTER Batches 1.3 and 1.4**, both blocked on hardware, or commissioning puts fresh test sales into the journal just reset; **8.1's review work can start now.** Before any of it, the two operator actions in *Open Threads → B*. **L-55, L-56 and L-58 have no batch yet**, and **L-52 waits on a text the administration has not published.**
...
- **Two payloads have GROWN — `CLOTURE_Z` twice, the sealed close THREE times —
  ...
  **Still free only because zero closes are sealed, and 3.8 is the last time
  that is true.**
...
**Twenty-two answered decisions have been retired from this table** (eighteen on 2026-09-05; **DD-20, DD-21, DD-22 and DD-04 on 2026-09-06**, when DD-23 needed the room), each closed with its batch `COMPLETED` and no forward link. Full rows: `REMEDIATION_RECORD.md` → *Answered design decisions*; the retired one-line pointers: → *Retired open-thread rows*.
