...
| ~~**Create the six menus in the catalogue**~~ **✅ DONE 2026-09-09** | All six exist and one has traded (see *G*). | 5.9g |
| **`Menu XXL`: the second pizza is imposed as Senior, not Mega** | Measured read-only 2026-09-09 19:18. The menu is « 2 pizzas Mega »; as configured the customer pays 33,90 for a Mega and a Senior, and the VAT lands at 2,94 where 2,96 is due. **Fix**: `Menu XXL` → Pizza 2 → Taille imposée → Mega. Figures: record → *Batch 5.9 → Appended 2026-09-09*. | 5.9g |

**Five operator actions are DONE and have been retired** to the record → *Retired open-thread rows*: both secret rotations, the Réglages address, `printerName`, Batch 3.11's migration, and **Batch 5.9's migration, applied and verified read-only on 2026-09-09**.
