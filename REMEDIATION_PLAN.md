...
**Current Batch:** none. **8.1 completed and 8.2 `BLOCKED` on L-61, both 2026-09-06.** What remains: **1.3's `[HW]` sign-off and 1.4's four `[MACHINE]` criteria**, both one commissioning session away; **8.0's pre-go-live reset** after them, which also **arms the fiscal chain key** (P-04, rehearsed); **L-61** (the restore does not complete on Windows) and **L-62**, which block 8.2 and own no batch; **8.2**'s full trading day and **8.3** (external); V-01…V-03; **7.3's secret rotation**; **L-58's stored per-line HT**, which needs a migration and has no batch; and **L-52**, which waits on a format the administration has not published.

**Last Batch:** **8.2 — `BLOCKED`, and the rehearsal is why.** The restore, driven through the real route on a scratch copy, **failed**: `fs.rename` over the open database returns `EPERM` on Windows because `db.$disconnect()` does not release the handle, so **the one-click restore cannot complete while the app runs — and the app is the only way to reach it (L-61, HIGH)**. The live database is never touched and no `RESTAURATION` is journalled for a restore that did not happen; **manual recovery works, proved end to end**. **L-46's cause established** (the 2026-08-29 rebuild: 30 `CREATE TABLE`s, zero `INSERT`s), which also explains L-60, and **none of the three real backups is restorable anyway** — a fresh one is a commissioning step. **L-51 measured: 510 MB peak.** Record → *Batch 8.2*. Before it, **8.1 `COMPLETED`**: 27 checks pass and `GrandTotal`, `FiscalCounter` and `lastSequence` are unchanged from the Batch 0.2 baseline in every field (`docs/verification-8.1-2026-09-06.txt`). Before it: **1.4** (`IMPLEMENTED` — Task Scheduler at boot, kiosk at logon, `C:\HibaPOS\data`, a safe update path, and the app is installable; **L-59** found and fixed; **the data move is what turns WAL on**, measured) and **3.10** (the archive gains `refunds` and `cashMovements` as rows, schema 4 → 5; « date certaine » withdrawn from the notice *and* the signed attestation; the ticket prints a real `Caisse N° 1`).
...
**Status:** `BLOCKED` — V-06 rehearsed 2026-09-06 and **failed**; the failure is **L-61**, a code change this batch may not make. V-07 needs the till.

| ID | Status | Task |
|---|---|---|
| **V-06** | `BLOCKED` (L-61) | Restore rehearsal onto a clean machine: database and images both come back; product images render in the POS; the fiscal chain verifies. |
