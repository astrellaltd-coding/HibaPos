...
**Current Stage:** **Stages 0 and 2 through 7 are all COMPLETED** — **Stage 2 reopened 2026-09-06 for 2.5** (the restore could not complete on Windows) and closed the same day; Stage 3 reopened three times that day (3.7, 3.8/3.9, 3.10) and closed each time. Stage 1 is partly done — 1.1, 1.2, **1.3b** and **1.3c** COMPLETED, **1.3 and 1.4 both `IMPLEMENTED — TESTING REQUIRED`**, waiting only on the till. **Stage 8 is `IN PROGRESS` — 8.1 `COMPLETED` 2026-09-06**; 8.0 and 8.2 wait on the commissioning session, 8.3 is external. C-22's chain-design half stays `REQUIRES EXTERNAL VERIFICATION` (V-01) and V-03 is open. Per-batch dates: the completion history in `REMEDIATION_RECORD.md`.

**Current Batch:** **3.11**, `IMPLEMENTED — TESTING REQUIRED` — L-58's stored per-line HT is built and rehearsed and **waits on one operator command**, `bunx prisma migrate deploy`. Before it, **1.3b and 1.3c closed L-21 and L-63 on 2026-09-07** — no line of any printed ticket can now exceed the paper — and before them the rotation closed SEC-ROT and L-04. What remains: **1.3's `[HW]` sign-off and 1.4's four `[MACHINE]` criteria**, both one commissioning session away; **8.0's pre-go-live reset** after them, which also **arms the fiscal chain key** (P-04, rehearsed); **8.2**'s full trading day and **8.3** (external); V-01…V-03; **L-58's stored per-line HT**, which needs a migration and has no batch; and **L-52**, which waits on a format the administration has not published.

**Last Batch:** **3.11 (L-58), 2026-09-07** — `OrderItem` gains `lineNetTotal` and `lineHt`, so « total HT de la ligne » is stored rather than reproducible only by re-running the discount apportionment. Written from the figures the checkout transaction already has, so `Σ (net − HT)` equals the order's sealed `vatTotal` — the assertion that would catch a second implementation of the split, and R3 proved it does. Archive schema 5 → 6. **Rehearsed: one differing fingerprint line, the `_prisma_migrations` row.** Before it, the same day: **1.3c (L-63), 2026-09-07** — `leftRight()`'s overflow, the unlaid-out option and add-on lines, and the copies of `center()` in `renderDayCloseTicket()` and `renderTestPage()`. All three renderers now share `services/ticket-layout.ts`; **nothing is truncated**, on BOFiP § 50. Before it the same day: **1.3b (L-21)**, the centred lines — the restaurant's 56-character address on 48-column paper. **Between them, no line any of the three renderers emits can exceed the paper.** Ten and eight one-property reverts; under the broadest of each, the whole suite was 956/7 and 936/7 **with every failure the batch's own** — 934 pre-existing tests caught neither defect. Also that day: **SEC-ROT**, both secrets rotated by the operator and proved by the pre-rotation backup no longer decrypting. Before those, **2.5, 8.2, 8.1, 1.4, 3.10 and 8.0's reset script**, all on 2026-09-06. Records → those batches.

**Next Batch:** **the commissioning session, and it is written out step by step in `docs/mise-en-service.md`** — install, prove the till boots by itself, commission the printer, demo, then **8.0's reset and the chain-key arming in that order**, then the first real trading day (8.2 V-07). **Everything rung before § 6 needs FACTICE on.** 8.0's script exists and is rehearsed, and the three operator actions still in *Open Threads → B* all happen inside that runbook. **L-21 and L-63 are both fixed** (1.3b, 1.3c), so no line of any ticket exceeds the paper. **The operator still owes two Réglages edits** — the duplicated postcode in the address, and `printerName` — both in *Open Threads → B*.
...
**Last Updated:** 2026-09-07 (session 19 — **1.3b, 1.3c and a de-stale pass** over this file, the runbook and the map). Carry forward: (1) **A test suite of 934 caught neither defect.** Under the full revert, every failure was the batch's own — 936/7 for L-21, 956/7 for L-63. **Before trusting coverage, revert the fix and count who notices.** (2) **The best finding of 1.3c came from reading the ticket, not the tests.** Every unit test passed while the renderer tore `(1,50 €)` in half, because JavaScript's `\s` includes the NO-BREAK SPACE that `formatEuro` emits to forbid exactly that break. **Look at the artifact.** (3) **Revert in both directions**, or the control is decoration: over-applying the wrap broke eight pre-existing tests and both snapshots, which is what proved they were real gates. Session 18's four lessons, and earlier sessions', stand in the record.
...
| **Apply Batch 3.11's migration** | `bunx prisma migrate deploy` — adds `lineNetTotal` and `lineHt` to `OrderItem` (L-58, BOFiP § 50). Two `ADD COLUMN`s, no table rebuild; rehearsed with a fingerprint diff whose only difference was the `_prisma_migrations` row. **Claude cannot run it** (CLAUDE.md rule 4). Command and checks: Batch 3.11's status record. | L-58 |
...
|---|---|
| Tests | **976 pass, 0 fail** (967 before Batch 3.11, 963 before `readme-counts.test.ts`, 943 before Batch 1.3c, 934 before 1.3b, 923 before 2.5, 891 before 1.4, 879 before 3.10), **and STABLE since Batch 6.3** — three consecutive whole-suite runs with no flake, where before it the suite failed about 2 runs in 5. **L-43 is FIXED, so a `shift-race.test.ts` failure is now a REAL failure**: investigate it. Do not re-run and dismiss it — that instruction stood here until 2026-09-05 and is withdrawn. **e2e: 13 passed — MEASURED 2026-09-07 by running the suite**, the first time it has been run in this project. *(A de-stale pass earlier the same day "corrected" this to 12 from a static count of the five spec files. That count was right and the conclusion was wrong: `auth.setup.ts` logs in once per run in a Playwright **setup project**, and Playwright counts it as a test. The original 13 was correct. Pinned since by `readme-counts.test.ts`, which counts the setup file too.)* **`bun run test:e2e` is SAFE** (6.3, T-10); the figure above is a static count rather than a run. *(`CLAUDE.md` rule 2 forbade it until 2026-09-07 — obsolete since 6.3, never updated. Corrected.)* Whole-suite runtime 60–80 s; a run of 296–422 s with the same 0 failures is **L-24**, not a defect; **`bun run test` carries the timeout itself since 7.4c**. Two sessions may now run the suite at once (warning 3b) |
| Production DB sha256 | **`58781596521e95c030c3535789479b57d05f5bbd…`** (mtime 2026-09-07 12:40:47, **704 512** bytes) — **moved by the OPERATOR on 2026-09-07**, saving Réglages: the address correction and `printerName`. Verified read-only the same day: the write is journalled (`SETTINGS_UPDATED`, audit 468 → 471), `Setting` 12 → 18 rows, and **every fiscal figure is unmoved** — counters 20/3/2/2, grand total, and zero rows in all four close tables. Journal byte 18 still `1`. Previous value `c9f26516…` stood from Batch 3.8's migration; **the pre-3.11 snapshot in `../db-snapshots/` carries this one**. Earlier lineage is in the record |
...
| L-55 ✅, L-56 ✅ | 3.10 | L-58 ◐ | 3.10 (label) / **3.11 (stored line HT — migration pending)** | DD-23 ✅, DD-24 ✅ | 3.8 |
...
**Status:** `IMPLEMENTED — TESTING REQUIRED` — reopens Stage 3 a fourth time. The migration waits on the operator.
...
**Status:** `IMPLEMENTED — TESTING REQUIRED` · Severity: LOW · Category: fiscal data in scope
...
**Status:** `IMPLEMENTED — TESTING REQUIRED` — the code and the migration are done, rehearsed and committed; **`prisma migrate deploy` against production is the operator's action and has not been run.**
**Completed:** — (code 2026-09-07; `COMPLETED` when the migration is applied and the counters re-verified)
