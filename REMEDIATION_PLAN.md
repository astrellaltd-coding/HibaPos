...
**Current Batch:** none. **5.8 closed L-67 on 2026-09-08**, opened at the operator's instruction from a duplicated « Sauces » group on every Croustillant. **Stage 5 reopened and closed again.** The cause was a round-trip, not a data accident: the GET merged the category's option groups with the product's own and marked neither, the admin editor round-tripped that merged list, and `PUT` stored all of it with `productId` set — so **every save of a product copied its category's groups onto it**, and `scripts/fix-duplicate-product-options.ts` had been repairing the symptom on a schedule nobody had written down. Twelve products were affected, all Croustillants, exactly the twelve edited that afternoon for their new photographs; **the operator has run the repair and the dry run now reports 0**. The rule now lives in `src/lib/services/product-options.ts` and the write path refuses the duplicates.

**Last Batch:** **5.8, 2026-09-08** — L-67, the catalogue editor writing inherited option groups back onto the product. **Its lesson is the revert protocol's, and it cost the batch a second test file**: fourteen unit tests proved the inheritance rule and **all 1025 tests survived reverting the route's single call to it** — *Methods*' own words, « the test proves less than it claims ». A route-level file driving the real `PUT` through `route-harness.ts` was written for that, and 4 of its 6 fail under the same revert. Two further traps are recorded: **the pizzas were a near-miss** (the Pizzas category owns « Taille » and the form resends it, so narrowing the guard to spare it would have left the same corruption open for every size group — checked rather than assumed), and **a test-isolation fault only the full suite could show**, six tests passing alone and failing together because the file's first `wipe()` deleted shared rows. Before it, **1.4c, 2026-09-08** — the install step nobody had written down, and the refusal that was missing for it. Records → those batches.
...
**Last Updated:** 2026-09-08 (session 21 — **5.8**). Carry forward: (0) **A unit test on an extracted rule proves the rule, not that anything calls it.** 5.8's fourteen passed while the route ignored the module entirely; only a revert showed it. **Revert the wiring, not just the logic.** (1) **A repair script that exists is evidence of a cause nobody removed.** `fix-duplicate-product-options.ts` had been needed before and treated a recurring corruption as an event. **Ask what re-creates the data it deletes.** (2) **A guard narrowed to spare the case that scares you leaves that case unguarded** — the « Taille » group was checked instead, and the guard stayed general. (3) Earlier sessions' lessons stand in the record; 1.4c's four are the ones directly above this line in the record's *Retired* section.
