...
**Current Stage:** **Stages 0 and 2 through 7 are all COMPLETED** — **Stage 2 reopened 2026-09-06 for 2.5** (the restore could not complete on Windows) and closed the same day; Stage 3 reopened three times that day (3.7, 3.8/3.9, 3.10) and closed each time. Stage 1 is partly done — 1.1 and 1.2 COMPLETED, **1.3 and 1.4 both `IMPLEMENTED — TESTING REQUIRED`**, waiting only on the till. **Stage 8 is `IN PROGRESS` — 8.1 `COMPLETED` 2026-09-06**; 8.0 and 8.2 wait on the commissioning session, 8.3 is external. C-22's chain-design half stays `REQUIRES EXTERNAL VERIFICATION` (V-01) and V-03 is open. Per-batch dates: the completion history in `REMEDIATION_RECORD.md`.

**Current Batch:** none. **The secret rotation was done and verified 2026-09-07**, closing SEC-ROT and L-04; before it 2.5 closed L-61/L-62 and unblocked 8.2's V-06, and 8.0's reset script was written and rehearsed. What remains: **1.3's `[HW]` sign-off and 1.4's four `[MACHINE]` criteria**, both one commissioning session away; **8.0's pre-go-live reset** after them, which also **arms the fiscal chain key** (P-04, rehearsed); **8.2**'s full trading day and **8.3** (external); V-01…V-03; **L-58's stored per-line HT**, which needs a migration and has no batch; and **L-52**, which waits on a format the administration has not published.

**Last Batch:** **SEC-ROT, done and verified 2026-09-07** — both secrets rotated by the operator, proved by the pre-rotation backup no longer decrypting. Before it, on 2026-09-06: **2.5**, which fixed the restore (two PrismaClients held the database open, so `$disconnect()` never closed it — L-61, L-62); **8.2**, whose rehearsal found that and established L-46's cause; **8.1**, 27 checks with the Batch 0.2 baseline figures unmoved; **1.4**, the installer, launcher and update path plus L-59; **3.10**, the archive's missing rows and « date certaine » withdrawn from the notice and the signed attestation; and **8.0's reset script**, written and rehearsed. Records → those batches.

**Next Batch:** **the commissioning session, and it is written out step by step in `docs/mise-en-service.md`** — install, prove the till boots by itself, commission the printer, demo, then **8.0's reset and the chain-key arming in that order**, then the first real trading day (8.2 V-07). **Everything rung before § 6 needs FACTICE on.** 8.0's script exists and is rehearsed, and the three operator actions still in *Open Threads → B* all happen inside that runbook. **Before the session, consider L-21**: every printed ticket will wrap the address until it is fixed.
...
- **1.3's `[HW]`** — a real print at 48 columns, a real drawer kick, real paper-out behaviour, and confirming the box is a Sunso WTP-801. Validated meanwhile against a mock ESC/POS printer on loopback.
