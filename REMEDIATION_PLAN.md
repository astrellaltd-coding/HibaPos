...
**Current Batch:** **3.12 — `IMPLEMENTED — TESTING REQUIRED` 2026-09-09**, on the operator's ruling that a sealed bottle or can is **10 % sur place and 5,5 % à emporter et en livraison**. `resolveVatRate` took the product and nothing else, so **every dine-in drink booked at 5,5 %** — an under-declaration across 17 products, exposed while analysing the combo feature and not confined to it. Code, tests and the migration rehearsal are done; **two [OWNER] steps remain and the till is wrong until both are taken**: apply `20260909101500_category_vat_rate_takeaway`, then set `vatRate = 10` **and** `vatRateTakeaway = 5.5` on *Bouteilles* and *Canette* — a **swap**, because that column now means sur place. Nothing wrong has been recorded: the app has never traded and § 6 deletes the development data.

**Last Batch:** **3.12, 2026-09-09** — L-68, VAT not varying by order type. **Its lesson is *Methods → Run the thing*, and it cost a wrong answer to learn twice in one session.** The worked example on real data came back 10 % for both order types; the code was right and the **build was stale** — `.next` compiled at 22:27, `pricing.ts` edited at 00:15 — and only the arithmetic (takeaway VAT should be 8, the response said 14) exposed it, after every test had passed. The revert protocol also reproduced Batch 5.8's lesson deliberately: under R1 the route stops passing the order type, **all 20 unit tests pass and 2 of 5 route tests fail**. Before it, **5.8, 2026-09-08** — the catalogue editor writing inherited option groups back onto the product. Records → those batches.

**Next Batch:** **DELIVERY IS TONIGHT, 2026-09-08 — not yet done.** We are in the final tweaks; `docs/CHANGES-LOG.md` is where those are recorded, and nothing in it is a substitute for a batch. The sequence is `docs/mise-en-service.md`, and **1.4b and 1.4c rewrote § 0, § 0a, § 2 and § 6a and added § 0b** — read it fresh, not from memory. **§ 0b is new and comes first**: the code from git, `db/custom.db` (704 KB) and the rotated `.env` carried by hand, then `bun install` / `bunx prisma generate` / `bun run build` — **`.env` before the build, the build before § 2's reboot**. Then § 0a's rehearsal if a spare machine exists, and it closes **1.4's four `[MACHINE]` criteria**. **Prerequisite 1 is still a live blocker: bun must be MACHINE-WIDE** — `%APPDATA%\npm\bun.ps1` on the user PATH here is the failing case; the binary to copy is `%APPDATA%\npm\node_modules\bun\bin\bun.exe`. **Prerequisite 2 is satisfied.** **§ 6a is a CHECK, not an action.** Then § 6's reset and the chain-key arming in that order, then the first real trading day (8.2 V-07). **Everything rung before § 6 needs FACTICE on.** **§ 0b's internet question is answered — the till has it** (operator, 2026-09-08), so `bun install` is the path; what nothing here can measure is how long `next build` takes on that hardware.
...
**Status:** `IMPLEMENTED — TESTING REQUIRED` · **Opened:** 2026-09-09, on the operator's ruling, out of the combo analysis that preceded it. **Code complete and validated the same day; two [OWNER] steps remain — the migration and the rates.**
...
| **L-68f** | `NOT STARTED` | **[OWNER]** Set 10 % / 5,5 % on *Bouteilles* and *Canette* — **a SWAP, not an addition**: both carry `vatRate = 5.5` today and that column now means *sur place*, so setting only the new rate would leave dine-in wrong. A write to real menu data, so it is the operator's (warning 9), **after** the migration is applied. |
