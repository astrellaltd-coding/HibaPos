...
**Current Batch:** none. **Stages 5 and 6 are both finished**; Stage 7 (7.1, 7.2, 7.3) is `NOT STARTED` and is where the plan resumes. **5.7 was SPLIT on 2026-09-05 into 5.7a–5.7d** — it held twelve items across four risk classes, where every completed batch here has been one finding or a tight cluster. The router section carries the evidence, where each item and each criterion went, and three things measurement found that the rows did not say. Audit IDs are unchanged; the index still maps them to 5.7.

**Last Completed Batch:** Batch 6.3 — e2e and CI safety, **closing T-10, T-11, T-12, L-06, L-40 and L-43, and with them Stage 6**. **Guards first**: `test-setup.ts` now ABORTS unless the database is under temp, and `vitest.config.ts` throws at import — because L-06's real hole was that a guard cannot protect a runner that never loads it. The per-run database path closes warning 3b, and **L-43 is fixed as its own row prescribes** (the eleventh promise is asserted): the suite ran **763/0 three times with no flake**, from ~2 failures in 5. **`bun run test:e2e` is now safe and warning 2 is lifted** — its own disposable database, port 3100, the production build, and a marker-proof spec that runs first. **The suite had never actually been run**: seven specs failed and not one was a regression. **Batch 4.1's rate limiter refused the suite and the SUITE changed.** *(Before it: 6.2, 6.1, and the whole of Stage 5. Accounts: the record.)*

**Next Batch:** **Batch 7.1** — documentation corrections (DOC-01…DOC-12), which *Open Threads → D* has been collecting instructions for since Batch 2.3. Then **7.2** (dead code and dependency removal) and **7.3** (secret rotation, DD-04). **What remains open beyond Stage 7**: Batch 1.3's `[HW]` sign-off and 1.4, both blocked on hardware; C-22's chain-design half and V-01…V-03, which need a qualified external party; and Batch 8.0's pre-go-live fiscal reset, which must run **after** 1.3 and 1.4.
...
**Seventeen answered decisions have been retired from this table (2026-09-05, Batches 5.3, 5.5, 5.6, the 5.7 split, 5.7a, 5.7b and 6.3), because each is closed, its batch is `COMPLETED`, and every one of them was already a pointer.** Their full rows — question, answer, and the measurements the answer turned on — are in `REMEDIATION_RECORD.md` → *Answered design decisions*, unchanged: **DD-03** (VAT key format, closed not-applicable), **DD-05** (out-of-order closes: refuse), **DD-06** (no LAN access; bind `127.0.0.1`), **DD-08** (dangerous scripts: split, six parts), **DD-09** (no table service; withdraw), **DD-10** (cross-shift refunds: allow, attributed to the current open till), **DD-17** (VAT rate on the category, nearest-wins), **DD-18** (premature close: refuse, no override), **DD-19** (step up with the operator's OWN PIN), **DD-11** (one till, so held orders stay device-local) **DD-12** (entrée/sortie de caisse with a fixed category list, plus the step-up rule 5.5 settled: a PIN for money LEAVING the drawer only) **DD-13** (no pre-payment order state; remove both dead `OrderStatus` values and the zero counter) **DD-07** (one operational role: `CASHIER` removed from the product, which is what let M-19s close — retired in 5.6, Stage 4 being complete) **DD-02** (the data directory is `C:\HibaPOS\data`; Batch 2.2 shipped the plumbing, and both the answer and the deployment step are in *Open Threads → A*) **DD-15** (remove `ProductAddon`, `AddOn` and `Customer.postalCode` — discharged by Batch 5.7a, which closed M-09 and M-10) **DD-14** (the zero-total sale goes through under its own « Offert / repas personnel » tender — discharged by Batch 5.7b, which closed M-11) and **DD-01** (build the ESC/POS bridge now, raw TCP to 9100 — Batch 1.3 is `IMPLEMENTED — TESTING REQUIRED` and blocked on the physical printer, not on the decision, which is recorded in the record and inline in that batch; *Open Threads → A* carries the hardware row). Each also has its answer inline in the batch that carried it. The rows kept below are the ones still open, still shaping unfinished work, or blocking a Stage 5 batch not yet done.
...
| **DOC-04** | `NOT STARTED` | `README.md:76` "105 tests" | 136 at audit time | Update to the real number after Stage 6. *Unblocked 2026-09-05: **Stage 6 is COMPLETED**, and the number is **763** unit/integration tests plus **13** e2e — re-measure before writing it, do not copy this line.* |
