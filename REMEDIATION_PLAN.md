...
**Last Completed Batch:** Batch 5.5 — entrée / sortie de caisse, which **closes M-05**. `expectedCash` was `openingFloat + cash − cashRefunds` with no model of any kind for cash moving otherwise, so a 200 € supplier payment produced a phantom 200 € shortfall at every close — which is how staff learn to ignore the variance figure, defeating C-02. Four fixed categories (DD-12), a **signed** amount so an *erreur de caisse* can go either way, the till resolved **inside** the transaction (C-15 at a fifth site), a `MOUVEMENT_CAISSE` event, and deliberately **no** touch of the perpetual `GrandTotal`. **The PIN gate is the direction of the money, not the category name.** 3.6b's sealed-payload tripwire fired as designed and was amended, not silenced; a defect in this batch's own code was found by **driving the API**, not by reading it. **Its migration was applied by the operator the same day, and production came out exactly as the rehearsal predicted.** *(Before it: 5.4, closing C-23; 3.6c, closing L-27. Accounts: the record.)*

**Next Batch:** Batch 5.6 — order cancellation (DD-13), which needs **no migration** (measured; enums are TEXT in SQLite). Then 5.7, which needs the stage's second and last one. **5.5's migration is applied**, so the live database matches `main` and 5.6 can start on a scratch copy of it as usual.
...
**Awaiting decision:** **nothing — every design decision in this plan is answered.** DD-04 and DD-16, the last two, were answered 2026-09-05 on a measured brief, together with the two recorded-but-not-urgent questions (the callerless `/api/auth/approve` → delete in 7.2, and L-27 → done as Batch 3.6c). No batch is blocked on a decision, and nothing is now waiting on the operator; what remains blocked is blocked on hardware. The answers and the measurements behind them: `REMEDIATION_RECORD.md` → *Answered design decisions*.

**Last Updated:** 2026-09-05 (session 14 — Batches 5.3, 3.6c, 5.4 and 5.5, plus the last four design decisions). Three things worth carrying forward. (1) **Do not trust a library's documented pattern without loading it.** 5.4's version guard was written the way zustand documents it, unit-tested, and green — and did nothing for the payload the finding names, because `migrate` is never called when the stored version key is absent. A test that imports the REAL module against a stubbed `localStorage` caught it; reading did not. 5.5 repeated the shape at a different layer: **driving the API found a defect that reading the route had not**. (2) **A *Validation Required* can be sound and still incomplete** (5.3), and a batch's own earlier test can encode the decision you are now reversing (3.6c) — read the criteria for what they omit, and expect to invert rather than delete. (3) **Front matter is ~350 bytes under its ~40 960 ceiling**; retire before you add, and say in the record what moved.

### OPEN THREADS — read this before starting a batch

*Updated through Batch 5.5 and the operator's migration of the same day. **A and B both shed a row** — 5.5's feature is no longer inert and its migration is no longer pending. **F was retired**; its instruction moved into Batch 7.1. D gained 5.5's payload change. **G's production-database row changed for the first time since 2026-09-04** — check it before trusting any hash.*
...
| Migrations | **7 applied on production**, latest `20260905150626_cash_movements` (Batch 5.5), applied by the operator 2026-09-05 14:41:21. **None pending.** Batches 4.1 through 4.4b added none — 4.4b's enum removal was measured with `prisma migrate diff` and emits an empty migration |
...
**Eleven answered decisions have been retired from this table (2026-09-05, Batches 5.3 and 5.5), because each is closed, its batch is `COMPLETED`, and every one of them was already a pointer.** Their full rows — question, answer, and the measurements the answer turned on — are in `REMEDIATION_RECORD.md` → *Answered design decisions*, unchanged: **DD-03** (VAT key format, closed not-applicable), **DD-05** (out-of-order closes: refuse), **DD-06** (no LAN access; bind `127.0.0.1`), **DD-08** (dangerous scripts: split, six parts), **DD-09** (no table service; withdraw), **DD-10** (cross-shift refunds: allow, attributed to the current open till), **DD-17** (VAT rate on the category, nearest-wins), **DD-18** (premature close: refuse, no override), **DD-19** (step up with the operator's OWN PIN), **DD-11** (one till, so held orders stay device-local) and **DD-12** (entrée/sortie de caisse with a fixed category list, plus the step-up rule 5.5 settled: a PIN for money LEAVING the drawer only). Each also has its answer inline in the batch that carried it. The rows kept below are the ones still open, still shaping unfinished work, or blocking a Stage 5 batch not yet done.
