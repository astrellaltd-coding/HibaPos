...
**Current Stage:** **Stages 0, 2, 4, 5, 6 and 7 are COMPLETED; Stage 3 is REOPENED for Batch 3.7** (2026-09-06, the French-law gaps). Stage 1 is partly done — 1.1 and 1.2 COMPLETED, **1.3 `IMPLEMENTED — TESTING REQUIRED`** on hardware, **1.4 `NOT STARTED`**. **Stage 8 is `NOT STARTED`** and mostly waits on 1.3 and 1.4; 8.1 does not. C-22's chain-design half stays `REQUIRES EXTERNAL VERIFICATION` (V-01) and V-03 is open. Per-batch dates: the completion history in `REMEDIATION_RECORD.md`.

**Current Batch:** **Batch 3.7 — French-law gaps (L-52, L-53, L-54), `IN PROGRESS` since 2026-09-06.** L-53 is fixed; L-54 and L-52 wait on the research answers to questions 5 and 2 in `docs/conformite-isca-recherche.md`. Also remaining: **Batch 1.3's `[HW]` sign-off and 1.4**, blocked on the POS machine; **8.0's pre-go-live fiscal reset** after them; **8.1 and 8.2**; C-22's chain-design half and V-01…V-03 (external); and **7.3's secret rotation**, prepared and waiting on the operator.
...
**Next Batch:** finish **3.7** (L-54 and L-52 as the research decides), then **Stage 8 — final validation**, in order: **8.0's fiscal reset must run AFTER Batches 1.3 and 1.4**, both blocked on hardware, or commissioning puts fresh test sales into the journal just reset; **8.1's review work can start now.** Before any of it, the two operator actions in *Open Threads → B* — the secret rotation (7.3) and the `printerName` correction. **L-55 and L-56, found by the ISCA map, have no batch yet.**
...
| L-52, L-53 ✅, L-54 | **3.7** (opened 2026-09-06) | L-55, L-56 | found by the ISCA map, no batch | | |
...
| **L-54** | `IN PROGRESS` — waiting on research question 5 | The Z seals a shift and `reports.ts:226` calls it the daily close; nothing keys a close to a calendar day. Measured on production: Z #2 covers five calendar days of test trading. |
| **L-52** | `BLOCKED` — on the implementing text (research question 2) | Restitution format of loi 2026-534 art. 87 unknown; the archive schema is bespoke. |
...
**⚠ THREE ROWS HERE SAY *NO BATCH OWNS THIS*, and that is the plan's honest state**: Stages 0–7 are finished and these outlived them. **The three French-law gaps (L-52, L-53, L-54) got their batch on 2026-09-06 — Batch 3.7, on the operator's instruction** — and the map that batch produced added L-55 and L-56 below.
