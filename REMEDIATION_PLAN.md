...
**Current Stage:** **Stage 5 is IN PROGRESS** — **5.1 through 5.6 and 5.7a COMPLETED 2026-09-05**, alongside **3.6c** the same day. **5.7b, 5.7c and 5.7d are what remain of the whole plan's Stage 5.** **Stage 4 is COMPLETED** (4.1 through 4.4, 4.4b, 4.4c, 4.5, 4.6 and 4.7, all 2026-09-04). **Stage 3 is COMPLETED** (twelve batches; **3.6c** reopened it 2026-09-05 for L-27 and closed it the same day). C-22's chain-design half stays carried forward as `REQUIRES EXTERNAL VERIFICATION`, and V-03 is open. Stage 1 is **partly done**: 1.1 and 1.2 COMPLETED, 1.3 `IMPLEMENTED — TESTING REQUIRED` on hardware, 1.4 deferred. Stage 2 is COMPLETED.

**Current Batch:** none. **5.7b is next**, of the three left. **5.7 was SPLIT on 2026-09-05 into 5.7a–5.7d** — it held twelve items across four risk classes, where every completed batch here has been one finding or a tight cluster. The router section carries the evidence, where each item and each criterion went, and three things measurement found that the rows did not say. Audit IDs are unchanged; the index still maps them to 5.7.

**Last Completed Batch:** Batch 5.7a — DD-15's two removals, **closing M-09 and M-10**. `AddOn` and the `ProductAddon` join go, and with them everything only they could reach: two API route files, the **446-line « Suppléments » screen** and its nav row, `addOnSchema`, `SEED_ADDONS` in **both** seed files, and a `productAddons` include in four more places. `Customer.postalCode` goes with zero code to change. **The risk was `addon` naming two things** — the dead `AddOn` (0 rows) and the live `CategoryAddOn` (21 rows) shared one `addonMap`, one request field and consecutive lines of `IMAGE_COLUMNS` — and **the surviving media scan was deleted beside the dying one anyway**, reintroducing C-25 until `eslint` caught it. Two earlier tripwires fired and were **amended, not silenced**. **Its migration is rehearsed and NOT applied** — the one thing it leaves outstanding, and it is the operator's. *(Before it: 5.6, 5.5, 5.4. Accounts: the record.)*

**Next Batch:** **5.7b** — DD-14's « Offert / repas personnel » tender (M-11), the **fiscal** slice: a zero-total sale under its own tender, VAT at zero, which **must not inflate revenue** in Batch 3.2's aggregation or a sealed period total. No DDL. Then **5.7c** (pricing and validation, plus L-41) and **5.7d** (POS resilience, plus L-42, and the slice L-47 blocks). **None depends on 5.7a's migration.**
...
**Last Updated:** 2026-09-05 (session 15 — Batch 5.6, the 5.7 split, Batch 5.7a). Three things to carry forward, all from 5.7a. (1) **Writing the trap down is not protection from it.** 5.7a's section stated the `addon` collision before the work and its test asserts both directions — and the surviving `CategoryAddOn` media scan was still deleted beside the dying one; `eslint`'s unused-variable warning caught it. **A removal beside a survivor needs a positive assertion ON the survivor.** (2) **Let the compiler find the surface**: M-09's row named 2 files, measurement raised it to 10, `tsc` found 2 more. (3) **Two tripwires from earlier batches fired in one batch** and both were amended with a dated comment, never adjusted to whatever the code now returned. **Front matter is only ~200 under its ceiling** — thinner than the ~1 000 recent sessions inherited, and the next batch must retire before it adds. Recording 5.7a took it 698 OVER; DD-15 and four paragraphs paid that back (record → retired lines). **Measure at the END.**

### OPEN THREADS — read this before starting a batch

*Updated through Batch 5.7a. **A and B each GAINED a row** — 5.7a's removal is inert until its migration is applied, and applying it is the operator's one outstanding action. G moved: the test count and the migration row. C, D and E are unchanged. Earlier per-batch update notes were retired to the record on 2026-09-05 (Batch 5.7a) — this thread records the CURRENT state, not its history.*
...
| **DD-15's removals** (5.7a) | The code is on `main`; the LIVE database still has `AddOn`, `ProductAddon` and `Customer.postalCode`. Harmless — nothing reads them — but `main` and production disagree until it lands. | The operator running the command in *B* |
...
| **Apply Batch 5.7a's migration** | `20260905162220_remove_product_addons_and_postal_code` — two `DROP TABLE`s and a `Customer` rebuild. **Rehearsed** on a copy with a 107-line fingerprint diff: both `Customer` rows and the 2 orders referencing one survive, all three named indexes are recreated, and nothing fiscal moves. Snapshot `../db-snapshots/custom.db.pre-5.7a.2026-09-05T16-07-54Z` was taken first. **The command, from the repo root:** `bunx prisma migrate deploy` | M-09, M-10 |
...
| Migrations | **7 applied on production**, latest `20260905150626_cash_movements` (Batch 5.5), applied by the operator 2026-09-05 14:41:21. **ONE IS NOW PENDING** — Batch 5.7a's `20260905162220_remove_product_addons_and_postal_code`, rehearsed and handed over (*Open Threads → B*); production will read **8** once applied. Batches 4.1 through 4.4b added none — 4.4b's enum removal was measured with `prisma migrate diff` and emits an empty migration |
...
**Status:** `COMPLETED` (code) · **Completed:** 2026-09-05 · **Commit:** `982168c` · **Findings:** M-09, M-10 (**closes both**), DD-15
...
**⚠ THE MIGRATION IS REHEARSED AND NOT APPLIED.** `20260905162220_remove_product_addons_and_postal_code` is prepared, rehearsed on a copy of production with a 107-line fiscal fingerprint diff, and **waiting on the operator** — *Open Threads → B* carries the command. Until it is applied, the live database still has `AddOn`, `ProductAddon` and `Customer.postalCode` while `main` does not. **Batch 5.7b, 5.7c and 5.7d do not depend on it** and can start on a scratch copy of the un-migrated live database as usual.
...
**Left open:** nothing in scope. The migration hand-over is the only outstanding item, and it is the operator's.
