...
**Current Stage:** **Stages 0 and 2 through 7 are all COMPLETED** — **Stage 2 reopened 2026-09-06 for 2.5** (the restore could not complete on Windows) and closed the same day; Stage 3 reopened three times that day (3.7, 3.8/3.9, 3.10) and closed each time. Stage 1 is partly done — 1.1, 1.2 and **1.3b** COMPLETED, **1.3 and 1.4 both `IMPLEMENTED — TESTING REQUIRED`**, waiting only on the till. **Stage 8 is `IN PROGRESS` — 8.1 `COMPLETED` 2026-09-06**; 8.0 and 8.2 wait on the commissioning session, 8.3 is external. C-22's chain-design half stays `REQUIRES EXTERNAL VERIFICATION` (V-01) and V-03 is open. Per-batch dates: the completion history in `REMEDIATION_RECORD.md`.

**Current Batch:** none. **1.3b closed L-21 on 2026-09-07** — the ticket no longer overflows the paper — and before it the rotation closed SEC-ROT and L-04. What remains: **1.3's `[HW]` sign-off and 1.4's four `[MACHINE]` criteria**, both one commissioning session away; **8.0's pre-go-live reset** after them, which also **arms the fiscal chain key** (P-04, rehearsed); **8.2**'s full trading day and **8.3** (external); V-01…V-03; **L-58's stored per-line HT**, which needs a migration and has no batch; and **L-52**, which waits on a format the administration has not published.

**Last Batch:** **1.3b (L-21), 2026-09-07** — `renderReceipt()` centres and now wraps, so the restaurant's 56-character address stops overflowing 48-column paper. Proved by ringing a real sale through `POST /api/orders` on a scratch copy and decoding what a mock ESC/POS printer received: **0 lines over 48**, on the wire and in the stored `Receipt.content`. **No pre-existing test caught it** — under the revert the whole suite is 936/7 and all seven failures are this batch's. Opened **L-63**. Before it: **SEC-ROT** (2026-09-07), both secrets rotated by the operator, proved by the pre-rotation backup no longer decrypting; and on 2026-09-06: **2.5**, which fixed the restore (two PrismaClients held the database open, so `$disconnect()` never closed it — L-61, L-62); **8.2**, whose rehearsal found that and established L-46's cause; **8.1**, 27 checks with the Batch 0.2 baseline figures unmoved; **1.4**, the installer, launcher and update path plus L-59; **3.10**, the archive's missing rows and « date certaine » withdrawn from the notice and the signed attestation; and **8.0's reset script**, written and rehearsed. Records → those batches.

**Next Batch:** **the commissioning session, and it is written out step by step in `docs/mise-en-service.md`** — install, prove the till boots by itself, commission the printer, demo, then **8.0's reset and the chain-key arming in that order**, then the first real trading day (8.2 V-07). **Everything rung before § 6 needs FACTICE on.** 8.0's script exists and is rehearsed, and the three operator actions still in *Open Threads → B* all happen inside that runbook. **L-21 is fixed** (1.3b), so the ticket fits the paper; **L-63 is its unowned remainder** and is cheapest to do with the `[HW]` print.
...
**Last Updated:** 2026-09-07 (session 19 — **1.3b**). Carry forward: (1) **The defect was one line and finding it took one measurement; the value was in what the measurement covered.** The address was the symptom; the cause was that `center()` clamps the padding and not the string, and the schema lets three fields be four times the paper. Fix the mechanism, not the field. (2) **A test suite of 934 did not catch it.** Under the full revert every failure was this batch's own. **Before trusting coverage, revert the fix and count who notices.** (3) **Revert in both directions or the control is decoration.** Over-applying the wrap broke eight pre-existing tests and both snapshots — which is what proved the untouched snapshot was a real opt-in gate and not a coincidence. (4) **A success message is not the artifact.** `{"printed":true}` says the socket accepted bytes; only decoding those bytes as the printer's own CP1252 showed the ticket fits. Session 18's four lessons, and earlier sessions', stand in the record.

### OPEN THREADS — read this before starting a batch

*Updated through **2026-09-07**. **A, B, C and G all moved on 2026-09-06/07** and the previous "only G has moved" note was itself stale — it had survived nine batches. What changed: **A** — WAL and `HIBAPOS_DATA_DIR` now have a mover (1.4) rather than a wish; **B** — the secret rotation is **done**, leaving three small operator actions all of which happen inside `docs/mise-en-service.md`; **C** — 1.4 is built and 8.2's restore rehearsal is done, so what waits on hardware is narrower than it was; **G** — tests 879 → 934, and the live database was verified end to end by 8.1. **E** was trimmed to a pointer. This thread records the CURRENT state, not its history: each batch's account of what it moved is in that batch's record section.*
...
- **The sealed close payloads grew five times and every one of them was free only because ZERO closes existed.** `CLOTURE_Z` twice (3.6, 5.5); the sealed close `dataJson` — a different thing — four times (3.6, 3.6b, 5.5, 7.4a); and **3.8 added a fifth shape, `DailyClose`, plus the `CLOTURE_J` event type**. **3.8 was the last free change**: it landed 2026-09-06 and **from the first real close every one of these shapes is fixed for good.** `close-timing.test.ts` pins the key list and has caught an unannounced change before — edit it deliberately, never to make a run go green. `CLOTURE_M` and `CLOTURE_A` event payloads have never been touched. Which batch added which field: record → *Retired open-thread rows*.
...
- **1.3's `[HW]`** — a real print at 48 columns, a real drawer kick, real paper-out behaviour, and confirming the box is a Sunso WTP-801. **Since 1.3b the ticket wraps**, so the print also settles what this printer does with an over-long line, which no test here can (1.3b note 2). Validated meanwhile against a mock ESC/POS printer on loopback.
...
2. **`bun run test:e2e` is SAFE since Batch 6.3 and this warning is lifted** — it prepares its own disposable database under temp, refuses to start if that path is not disposable, and runs on port 3100. It used to write real orders and sealed Z reports into **production**. **`bunx vitest` is still forbidden — warning 3.** Full history: record → *Retired open-thread rows*.
...
| C-25 ✅ | 4.6 | M-24 ✅ | 4.4 | V-04 ✅, V-05 ✅ | 8.1 | V-06 ⚠, V-07 | 8.2 (**V-06 blocked by L-61**) |
...
**Status:** `COMPLETED` · **Completed:** 2026-09-07 · **Commit:** `PLACEHOLDER_SHA` · **Findings:** L-21 (**closed** — the code half; the `[HW]` print stays with 1.3) · **Decisions:** none; the table stays empty.
...
**Left open:** **L-63** (*Newly Discovered Issues*) — `leftRight()`'s different overflow, the unlaid-out option/add-on lines, and the same `center()` helper copied into `renderDayCloseTicket()` and `renderTestPage()`; recorded, not fixed (safety rules 1 and 10). **L-14** unresolved by choice. **1.3's `[HW]` print** of a wrapped ticket, at the commissioning session. The rest of the *Hardware-dependent validation* re-triage — L-21 was the first item on that list and is the only one done.
...
**Stage status:** `IN PROGRESS` — **8.1 `COMPLETED`** (the live database verified read-only: 27 checks pass, 1 flagged and explained) and **8.2 `BLOCKED`**: its restore rehearsal was run on 2026-09-06 and **failed**, which is the point of a rehearsal. **L-61** — `fs.rename` over the open database returns `EPERM` on Windows, so the one-click restore cannot complete while the app is running, and the app is the only way to reach it. Manual recovery works and was proved end to end. 8.0 waits on 1.3/1.4's commissioning and must run before the first real sale; 8.3 is external.
