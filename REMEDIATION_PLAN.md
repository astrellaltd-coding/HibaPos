...
**Current Batch:** none. **7.7 closed L-64 on 2026-09-07** — and, driving the app to check it, **found L-47 unreproducible for the second time**; **7.6 closed L-09 and L-10**; **7.5 closed L-22, L-36 / DOC-13 and DOC-14** the same day, taking the unowned rows from seven to three; **3.11 closed L-58** the same day — the stored per-line HT, migration applied by the operator and verified — and **1.3b and 1.3c closed L-21 and L-63** the same day — no line of any printed ticket can now exceed the paper — and before them the rotation closed SEC-ROT and L-04. What remains: **1.3's `[HW]` sign-off and 1.4's four `[MACHINE]` criteria**, both one commissioning session away; **8.0's pre-go-live reset** after them, which also **arms the fiscal chain key** (P-04, rehearsed); **8.2**'s full trading day and **8.3** (external); V-01…V-03; **L-58's stored per-line HT**, which needs a migration and has no batch; and **L-52**, which waits on a format the administration has not published.
...
**Next Batch:** **DELIVERY IS 2026-09-08 (tomorrow morning).** The sequence is `docs/mise-en-service.md`, unchanged, with one addition: **§ 0a rehearses §§ 1–2 on a spare Windows machine the evening before**, which closes **1.4's four `[MACHINE]` criteria** where a mistake costs nothing — do it on a **copy** of the repo, never this tree, because the installer moves data *out of* the install directory. **Two prerequisites are the likely failure modes and both are checkable tonight: (1) bun must be installed MACHINE-WIDE** — it is currently at `AppData\Roaming
pm` on the development machine, which is exactly the case where the `SYSTEM` task silently never finds it and the till never comes up; **(2) the `.env` carried to the till must be the ROTATED one** (2026-09-07), or the backups written there open with nobody's keys. Then § 6's reset and the chain-key arming in that order, then the first real trading day (8.2 V-07). **Everything rung before § 6 needs FACTICE on.**
...
**Last Updated:** 2026-09-07 (session 19 — **1.3b, 1.3c, 3.11, 7.5, 7.6, 7.7**, a de-stale pass and the `CLAUDE.md` reconciliation). Carry forward, and all four are about MEASUREMENT: (1) **A test suite of 934 caught neither of the day's first two defects** — under the full revert every failure was the batch's own. **Before trusting coverage, revert the fix and count who notices.** (2) **Cheap parsing produced three wrong numbers in one day** — a line grep said 10 nameless icon buttons (0), a regex stopping at the first `>` said 33 nameless inputs (5), and both were caught only by opening the files. **Parse the thing, or go and look.** (3) **Revert in both directions**, and take seriously a revert nothing catches: two of them this session exposed weak assertions rather than no-ops. (4) **A constraint recorded once is worth re-testing before inheriting it** — 7.6 deferred partly because "no session can see this app render"; 7.7 went and looked, and could. Earlier sessions' lessons stand in the record.
