...
**Status:** `NOT STARTED` — **opened 2026-09-05**, after the post-Stage-6 sweep found nine open findings pointing at batches that had since completed. **It runs BEFORE 7.3**, whose own prerequisite is "every other Stage 7 batch complete"; the numbering is a label, not an order (*Open Threads → D*).
...
**Two of the nine are decisions, not code, and must be answered before either is written** (safety rules 10 and 11). Bring them as briefs in plain language before touching a file.
...
| **L-50** | `REQUIRES DECISION` | `isFullyRefunded` treats a zero-total order as fully refunded, so a give-away never appears in `salesCount`, `itemsCount` or `topProducts`. **Decide first**: does a comp count as a ticket, and do its items belong in the product breakdown? Fixing it means editing Batch 3.2's unified aggregation, a fiscal core, for a reporting question nobody has been asked. | LOW (under-counts only) |
| **L-44** | `REQUIRES DECISION` | The four non-fiscal reports attribute a refund to the refunded **order's** dimension, so the dashboard's "today" and `/api/reports/sales` disagree once a refund is paid on a different day from its sale. **Decide first**: does a cross-period refund reduce the *selling* cashier's takings or the *refunding* one's? A management question, not a mechanical change. | MEDIUM |
