...
**Next Batch:** **the commissioning session, and it is written out step by step in `docs/mise-en-service.md`** — install, prove the till boots by itself, commission the printer, demo, then **8.0's reset and the chain-key arming in that order**, then the first real trading day (8.2 V-07). **Everything rung before § 6 needs FACTICE on.** 8.0's script exists and is rehearsed. Before any of it, the operator actions in *Open Threads → B*.
