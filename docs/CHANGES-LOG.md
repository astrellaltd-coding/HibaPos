...
|---|---|---|---|---|---|---|---|
| 2026-09-09 | **Not a row — a batch.** Direct USB printing went through `REMEDIATION_PLAN.md` as **Batch 1.3d** (L-70), not through this file. Recorded here only so that somebody reading this table after a printer problem is not misled by its absence. | The printer is a transport for a fiscal document and Batch 1.3 is still open, so `CLAUDE.md`’s rule applies: it is a batch. | see the batch | (this commit) | 1247 pass, 0 fail · 30 added | Per the batch, not per this row. | See the batch’s status record. |
