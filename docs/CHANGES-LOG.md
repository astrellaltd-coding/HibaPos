...
| 2026-09-09 | **A menu line in the cart lists what is in it** — one `· ` line per component, with its own options and supplements, and a `+ €` only where the customer is actually charged extra. | The operator, same session: the cart showed « Menu Eco » and a total and nothing else. A menu line's `options` and `addOns` are both empty — the components live in `components` — and the panel rendered only the first two, so **the cashier had no way to check what they had just built before charging for it**. | `src/components/pos/cart-panel.tsx`, `src/store/cart-combo.test.ts` | *(this row's commit)* | 1212 pass, 0 fail · 3 added | `git revert` — one file, one block inside the existing detail list, plus its three guards; the totals row and every handler are untouched. | **None to money, and the important part is what is NOT shown.** No per-component amount appears, and there is none to show: the forfait is not divided until the server allocates it, and those shares are artefacts of that division rather than prices anyone pays — the same rule the printed ticket follows. What does carry a price is a component's own extras, and that figure comes from `componentExtras`, the same function `computeLineTotal` uses, so what is shown and what is charged cannot disagree. **The edit pencil is deliberately still hidden on a menu line**: it opens the ordinary options dialog, which is the wrong screen for a menu. Re-ringing is the way to change one, and a builder in edit mode is a batch, not a row. |
