...
| ⚠ Backup gap | **SCHEMA HALF CLOSED 2026-09-14. THE OFF-MACHINE HALF IS NOT OPEN — IT IS MEASURED, AND THE ANSWER IS NO.** A fresh backup now exists at the current schema; the previous two predated four schema changes and would have restored a database at `schema_version` 171. **But nothing has ever left this machine.** This row used to say « the folder is inside OneDrive so it should sync, and whether OneDrive is actually running is the operator's to check ». Checked, 2026-09-14: **OneDrive is not installed** — no `OneDrive.exe` in any of the three standard locations, no `HKCU:\Software\Microsoft\OneDrive\Accounts` key, no client logs, no autostart entry, and the directory carries no reparse point. `$env:OneDrive` is set, which is what made it look synced; it is a leftover variable, not a running client. **And there is one volume**: `C:` (237,1 GB, 77,3 GB free) is the only fixed disk, so the live database, every `.dbenc` backup and all 15 plaintext snapshots share one device. No File History, no shadow copies, no third-party sync client running. A copy on the same disk as the database is not a backup (C-06) — this is **L-194**, and it is why **R6.5 needs hardware, not a setting**. |
