...
| Production DB | sha256 `44a45a71b776d330c4322fda541ec04e2b0bb92f985132a39ca96a0afe63b2d9`, 884 736 bytes, app stopped — **re-measured 2026-09-13**, after R8.2's and R8.5's migrations were applied. `integrity_check` ok, 0 FK errors, **17 migrations, none pending**, **18 `Product`, 18 `OrderItem` and 28 `ZReport` columns** (unchanged — the two new columns are on `Order` (20) and `CategoryAddOn` (9)), `schema_version` **175**, journal mode `delete`. Every trading table is still at zero. *(The previous value was `0d304ee7…` / `schema_version` 171, held from 2026-09-11 to 2026-09-13. The SIZE has never moved across four schema changes — do not use it as a check. Expect the sha to move again: the operator edits the catalogue between sessions.)* |
...
| ⚠ Backup gap | **Still open, and WIDER since 2026-09-13.** The Desktop is inside OneDrive so it does sync off the machine — but **OneDrive was not running** when this was set. And both copies now predate **four** schema changes, not two: R7.1's migration, R7.2's renames, and R8.2's and R8.5's migrations applied on 2026-09-13. Restoring one would land a database at `schema_version` 171 with 15 migrations — recoverable, because PREP-4's startup gate would migrate it forward behind its own verified backup, but it is no longer a copy of what is on the disk. **A FRESH backup is the outstanding action**, it is the operator's, and it is the oldest open item in the plan. |
