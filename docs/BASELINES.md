...
| Production DB | sha256 `fbf2f75f78880e309de91da2130380786e9345b225c2bc3afa4d20610d6d5913`, 884 736 bytes, app stopped — **re-measured 2026-09-16**. It was `af995bc7…` on 2026-09-14; **two operator actions moved it and neither is trading data**: a backup taken through the app on 2026-09-15 (which records itself — a `Backup` row and a `BACKUP_CREATED` audit entry), and L-81's product deletion the same day. **The SIZE did not move**, across five schema changes, a row insert and now a row DELETE — SQLite frees pages for reuse rather than returning them, so **never use the size as a check** — **re-measured 2026-09-14 after the operator applied L-171's migration**, which is what moved it: `_prisma_migrations` gained a row and `Refund` gained the `itemsJson` column. **Verified, not assumed** — a fingerprint of the live file is IDENTICAL to `../db-snapshots/r171-acceptance/fp-r171-after.json` on every key, checksums included, and `prisma migrate status` answers « Database schema is up to date ». The size is unchanged because SQLite had free pages. *(It was `14ebf310…` earlier the same day, after a fresh backup, and `44a45a71…` from 2026-09-13 before that. **The sha moved because taking a backup records itself**: `createBackup` writes a `Backup` row and its `BACKUP_CREATED` audit entry into this database. Two rows, no trading data, and the operator asked for the backup — but `CLAUDE.md` reserves writes to this file, so it is recorded rather than left to be noticed.)* Every trading table is still at zero and `FiscalCounter` is still `0/0/0/0`, re-checked after the write; catalogue unchanged at 84 products in 14 categories. **The SIZE has never moved across five schema changes and now a row insert — do not use it as a check.** |
...
| Catalogue | **83 products in 14 categories** — 84 until 2026-09-15, when the operator ran `scripts/delete-product.ts --apply` on `5 nuggets test` and closed **L-81**; **zero inactive products remain**, that was the only one. **9 menus composés · 25 slots · 9 whitelist rows · 7 option rules.** 17 active drinks in `Canette`/`Bouteilles`, all resolving to 5,5 % à emporter and 10 % sur place. **NO two products share a name** since R7.2 (2026-09-11) — the three pairs became `Coca`/`Coca 1.5L`, `Fanta`/`Fanta 1.5L`, `Orangina`/`Orangina 1.5L`. **Longest name 26 characters**, against the 36 at which a ticket line would wrap. **80 products on the till grid, unchanged by that deletion**: 83 less the 3 hidden components R3.3 created (`showOnPos = 0`). It was 84 less those 3 less `5 nuggets test` — the same 80, which is why L-81 was only ever cosmetic. **0 names carry stray whitespace** since R4.4. |
...
| Backups | **FOUR backup rows, four files, and the folder and the table agree in both directions** — re-measured 2026-09-15 after the operator had L-190's five unmanaged files removed: **0 unmanaged, 0 missing**. The newest is `hibapos-backup-2026-09-15T16-14-03-911Z.dbenc`, 745 516 bytes, taken by the operator through the app on 2026-09-15 — and **opened, not assumed**: decrypted with the real `scripts/decrypt-backup.ts`, plaintext sha256 `a34d434452265e328abfad066221031447799585e29d20a8ad9e9d30f21848ca`, valid SQLite header, `integrity_check` ok, **34 tables, 84 products, 14 categories, 2 users**, `Refund.itemsJson` present (so it carries L-171's migration and is at today's schema), every trading table at zero and `FiscalCounter` still 0/0/0/0. The 2026-09-14 one was verified the same way. The 2026-09-10 and 2026-09-11 pair are kept — retention is 30 and nothing was pruned — but they **predate four schema changes**. All four **reuse** the content-addressed media archive `hibapos-media-4b5ed80dca201113.enc`, 49 129 786 bytes: one copy backs all four, and it is why a naive unmanaged-file check must never be pointed at this folder. |
| ⚠ Backup gap | **SCHEMA HALF CLOSED 2026-09-14. THE OFF-MACHINE HALF IS STILL OPEN, AND IT NOW HAS TWO HALVES OF ITS OWN.** A fresh backup exists at the current schema and is verified restorable. **But nothing has ever left this machine.** Measured 2026-09-14: **OneDrive is not installed** — no `OneDrive.exe` in any of the three standard locations, no `HKCU:\Software\Microsoft\OneDrive\Accounts` key, no client logs, no autostart entry, and the directory carries no reparse point. `$env:OneDrive` is set, which is what made it look synced; it is a leftover variable, not a running client. **And there is one volume**: `C:` (237,1 GB) is the only fixed disk, so the live database, every `.dbenc` and all 15 plaintext snapshots share one device. A copy on the same disk as the database is not a backup (C-06) — that is **L-194**, and it is why **R6.5 needs hardware, not a setting**. **THE SOFTWARE NOW SAYS SO**: Réglages warns, in French, when the backups and the database share a volume. **THE SECOND HALF IS L-197, opened 2026-09-15**: `BACKUP_ENCRYPTION_KEY` lives only in `.env`, which `.gitignore` correctly excludes from the one off-machine copy this project has — so a drive full of verified backups and no key restores nothing. **R6.5 is two acts, not one**: a second volume, and the key somewhere that is not this disk. The operator chose a USB drive on 2026-09-15. |
