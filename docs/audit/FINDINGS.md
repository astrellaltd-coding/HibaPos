...
| **L-154** | — | *escalation, not a new finding* | **L-154 has now bitten.** It was recorded as « a standing wipe-order hazard that has not yet bitten » — sixteen files wiping in an order FK enforcement would refuse, two of them deleting `User` with nothing else cleared, and « under today's data none trips ». R8.2's new `checkout-idempotency.test.ts` is the first file to leave orders, payments and shifts behind, and `fiscal-verify-software.test.ts` then failed three tests down on `db.user.deleteMany()` with a foreign-key violation it had no way to explain. Patched at the source (that one file now cleans up after itself, following `orders-route.test.ts`), so the suite is green — **but the hazard is now demonstrated rather than theoretical, and the next file that forgets an `afterAll` will do it again.** | R8.2 (2026-09-13) | Unchanged: **R9.7**, one shared wipe helper in FK order. This raises its priority — sixteen hand-maintained lists that already disagree is no longer a latent problem. |
