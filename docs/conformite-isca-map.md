...
> § 6.2). **L-58's other half — whether the per-line HT total should be STORED
> — is deliberately still open**: it needs a column, therefore a migration.
...
| **L-58** ◐ | 6.2 | Ticket's « Caisse #N » was the shift number; no stored per-line HT. | **Label half CLOSED by Batch 3.10** (`Caisse N° 1` + `Service N`). **Stored-line-HT half OPEN** — needs a migration; NO BATCH OWNS IT |
