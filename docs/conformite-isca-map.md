...
>
> **THIRD MEASUREMENT, 2026-09-06 (Batch 3.10).** Three more of this map's own
> gaps closed: the archive now carries **`refunds` and `cashMovements` as rows**
> (L-55, § 4.4), its notice no longer claims **« date certaine »** and neither does
> the signed attestation (L-56, § 4.6, § 5.3), and the ticket prints a real
> **`Caisse N° 1`** with the shift number relabelled **`Service N`** (L-58,
> § 6.2). **L-58's other half — the STORED per-line HT — was
> closed on 2026-09-07 by Batch 3.11**: `OrderItem.lineHt` and
> `OrderItem.lineNetTotal` were added by migration
> `20260907114611_order_item_line_ht`, which is row-present in production with
> `finished_at` set. *(This paragraph said it was « deliberately still open » for four
> days after it had been closed.)*
> **The `services/` prefix was missing from this map's `fiscal.ts` and
> `receipt.ts` citations**; `src/lib/fiscal.ts` is the pure half and holds
> neither. Corrected in the rows below.
>
> **FOURTH MEASUREMENT, 2026-09-07 (session 18).** Nothing in §§ 1–6 moved, but
> **§ 3.6's retention row did**: the restore this map cites as part of
> conservation could not complete on Windows at all until Batch 2.5 (L-61), and
> the three backups on the live install are not restorable for two independent
> reasons (L-46, established by 8.2). Both are now stated in that row. The
> mechanism works; **this install has no restore point until a fresh backup is
> taken at commissioning.**
>
> **SIXTH MEASUREMENT, 2026-09-11 (Phase 0 closed).** One verdict below is now
> out of date in the install's favour, and § 3.6 is amended accordingly:
> **this install HAS a restore point.** Two encrypted backups exist, both
> verified by actually decrypting them — 2026-09-10 and 2026-09-11 — and the
> second matches production table for table. **L-46 is closed.** The nine
> pre-rotation files that could not be decrypted were deleted by R0.2. What
> remains open is custody, not existence: both backups sit on the same disk as
> the database they protect, and `BACKUP_LOCATION` is still unset (R6.5).
>
> **FIFTH MEASUREMENT, 2026-09-07 (session 19, after Batches 1.3b and 1.3c).**
> No requirement moved and no verdict changed. What moved is **§ 6.1**: the
> ticket is now laid out to the paper. `renderReceipt()` centred and never
> wrapped, so the restaurant's own 56-character address went onto 48-column
> paper whole (L-21); `leftRight()` overflowed by a second mechanism and the
> option and add-on lines were not laid out at all, and the same defective
> helper had been copied into the day-close slip and the printer self-test
> (L-63). All three renderers now share `src/lib/services/ticket-layout.ts`.
> **Nothing is truncated**, deliberately and on this map's own § 6.2: an
> ellipsis on an article's *libellé* would drop § 50 data in order to fit the
> paper. **`receipt.ts`'s line numbers moved with it and every citation to it
> below was re-measured on 2026-09-07** — the rest of the map is still at
> `beabccd`.
...
**Every `file:line` below was re-measured at `beabccd`, except `receipt.ts`'s,
re-measured 2026-09-07 after Batches 1.3b and 1.3c.** Where a line number and a
symbol name disagree after a later edit, the symbol name is the one to trust.
...
| 2.8 | FACTICE / simulation mode | `src/lib/services/settings.ts:23`, `src/lib/validation.ts:234`; `services/receipt.ts:71-75` (ticket stamp); `factice` column on every event (`schema.prisma:644`) | When on, every ticket is stamped `*** FACTICE — SIMULATION *** / TICKET NON VALABLE` and every event carries `factice: true`. **Off on production today**, so the 20 development orders read as genuine (Batch 8.0 / P-04 resets them before go-live). | **fact** |
...
| 3.6 | Six-year retention | `fiscal.ts:653` (the notice states it); no deletion path (1.1, 2.2); `backup.ts:157` (AES-256-GCM backups), `close/route.ts:51` (a backup at every Z close) | Retention is achieved by **not deleting**, and by encrypted backups. Two facts a professional should weigh: `BACKUP_LOCATION` is unset, so backups land on the same disk (*Open Threads → A*); and — **as of 2026-09-11 this is no longer true, see the amendment below** — on the live install **no backup was restorable through the application** (L-46). **AMENDED 2026-09-07**: Batch 8.2 established WHY — 0 `Backup` rows against 9 files, because the 2026-08-29 schema rebuild re-imported the data and not those rows; and, independently, the three files **predate seven fiscal tables**, so `assertCompatibleSchema` refuses them however the rows stand. They still decrypt, so they are readable evidence, not a restore point. **The restore mechanism itself was broken on Windows until Batch 2.5** (L-61: two PrismaClients held the database open, so the swap failed `EPERM`); it is fixed and was rehearsed end to end. **A fresh backup at commissioning is what turns this row from a mechanism into a restore point** — `REMEDIATION_PLAN.md`. **AMENDED 2026-09-11 (Phase 0):** that fresh backup was taken. `hibapos-backup-2026-09-11T12-40-36-138Z.dbenc` was created through the real `createBackup` service and then **decrypted to verify it** — valid SQLite, `integrity_check` ok, 0 FK errors, and every table equal to production. A second, from 2026-09-10, is also verified. **The retention claim is now true of this install.** The nine undecryptable pre-rotation files were deleted. What is still open is CUSTODY: both backups are on the same disk as the database, and `BACKUP_LOCATION` remains unset (R6.5). | **fact**, with the custody question left to the operator (V-09, V-12, R6.5). |
...
| 4.4 | Content | `services/fiscal.ts:1017` (`buildAnnualArchive`) | `fiscalEvents` (by timestamp in the year), `orders` (by `createdAt`, with items, payments, refunds, receipt, cashier, shift number), **`refunds` and `cashMovements` (Batch 3.10, L-55 — both by the date the MONEY MOVED, not by the sale's date, each joined to its order or shift so a row reads on its own)**, `zReports`, `monthlyCloses`, `annualClose` (if sealed), `dailyCloses` (Batch 3.8), `grandTotalSnapshot`, `software` (L-53), a French `notice`. Schema version **5**. **Still not present as rows:** `Shift`. | **✅ L-55 CLOSED by Batch 3.10.** Before it, `CashMovement` — added two batches after the archive was built — appeared nowhere, and a refund **paid in year N+1 against a year-N order** was in no `orders` section of any archive (year N's window closed before it; year N+1's holds no such order); both reached the file only as journal events. A same-exercice refund is now listed **twice**, under its order and in the period section: deliberate, pinned by a test, and explained in the notice's new « Contenu » paragraph. |
...
| 4.6 | What the notice claims | `services/fiscal.ts:969` (`ARCHIVE_NOTICE`) | The notice **used to say** the archive « fige les données […] et leur donne **date certaine** ». « Date certaine » is a legal term (art. 1377 code civil: registration, death, or an authentic act); a self-generated file timestamped by the till's clock does not confer it. The rest of the notice (open format, reproducible checksum, chaining, six years) states things the code does. | **✅ L-56 CLOSED by Batch 3.10.** The claim is withdrawn and replaced by what is true: the file is frozen at its `generatedAt`, on the till's own clock, and its anteriority rests on the chained `ARCHIVE_GENEREE` entry and the reproducible checksum. **The same sentence was in `docs/attestation-conformite.md` § Archivage** — the document the éditeur signs under art. 441-1 — and was corrected there too, with editorial note 6. Both are pinned by tests, and the attestation's is asserted on the *declaration* so the notes may still quote the withdrawn phrase. |
...
| 5.3 | Individual, nominative, two volets, French | `docs/attestation-conformite.md` | The template follows the BOFiP model's structure. Its ISCA section (« Mise en œuvre… ») restates §1–§4 above **without the gaps** — it must be rewritten against this map before signature. **Batch 3.10 corrected its Archivage bullet** (L-56: the « date certaine » claim withdrawn; the archive's contents listed, L-55) and added editorial note 6. | **⚠ — narrowed, not closed.** L-55 and L-56 are now reflected; **L-52, L-54, V-01 and V-13 still have to be reflected or excluded** before signature. |
...
|---|---|---|---|
| 6.1 | Restaurant name, address, phone, SIRET, TVA number; ticket number and time; cashier and caisse number; order type and table; lines with options; subtotal, discount, **per-rate VAT with HT base**, total VAT, total; payments with tendered/change; article count; footer; **software and version**; FACTICE stamp when on — **and, since Batches 1.3b and 1.3c (2026-09-07), all of it laid out to the paper**: no line the renderer emits exceeds the configured column count, nothing is truncated (an ellipsis on an article's libellé would drop § 50 data to make it fit), and a line that already fitted is emitted byte-identically | `services/receipt.ts:58-175`, `services/ticket-layout.ts` | **fact.** V-03 asked what a compliant receipt must contain; per-rate VAT and the TVA number were added in Batch 3.6 on the operator's own determination, the software identification in 3.7. Whether anything further is required (a per-ticket hash or signature is what NF525-certified software prints; the law does not name one) — **? unknown, V-03**. |
| 6.2 | **The line-by-line data BOFiP § 50 puts in scope** — as rendered by research § 9.6, to be re-read verbatim: numéro du justificatif; date année-mois-jour-heure-minute; numéro de la caisse; montant TTC; per line libellé, quantité, prix unitaire, total HT de la ligne, taux de TVA; mode de règlement; traces de modifications et corrections | `prisma/schema.prisma` — `model Order` (`number`, `createdAt`, `total`), `model OrderItem` (`productName`, `unitPrice` TTC, `quantity`, `lineTotal` TTC, `vatRate`, `lineNetTotal`, `lineHt`), `model Payment` (`method`), `model Refund`. **Cited by model name, not line number:** the numbers this row used to carry (`:373`, `:411-429`, `:448`, `:465`) were commit `beabccd`'s and two migrations' worth of columns have moved them since — `Order.number` is around `:558` today and will move again; `services/receipt.ts:58` (`renderReceipt`) | Present: ticket number (gapless, `sequence.ts:31`), timestamp to the millisecond, TTC total, per-line label, quantity, unit price and **snapshotted VAT rate**, tender per payment, corrections as `Refund` rows and journal events, and — since Batch 3.10 — a **numéro de caisse**, printed as `Caisse N° 1` in the establishment block from a literal, because this install is one restaurant and one till (§ 1). The shift number is still on the ticket, relabelled **`Service N`**. **The per-line HT total IS stored**, since Batch 3.11 (migration `20260907114611_order_item_line_ht`, applied): `OrderItem.lineHt` holds the figure the till actually used, and `OrderItem.lineNetTotal` is stored beside it so the row is self-verifying — a reader can check `lineHt = round(lineNetTotal / (1 + vatRate/100))` with a calculator. This matters because the HT is taken on the line's net **after** the order's discount has been apportioned by largest remainder (`money.ts` `apportion`), so dividing `lineTotal` by the rate does **not** reproduce it. Both columns are **nullable and not backfilled**: null means « recorded before the till kept this figure », and computing it retroactively for completed sales would be writing invented figures into the fiscal record. | **◐ L-58: label half CLOSED by Batch 3.10** — the ticket no longer presents the shift counter (3 on production) as a till number, verified end-to-end on a scratch copy where a real sale printed `Caisse N° 1` and `Service 3`. **The stored-line-HT half stays OPEN**: it needs an `OrderItem` column, therefore a migration, and whether § 50 requires the figure *stored* rather than reproducible is not settled by any source read so far. Everything else on the list is stored as such. |
...
| **L-53** ✅ | 5.1 | Software stated its version nowhere. | **CLOSED by Batch 3.7** |
...
| **L-55** ✅ | 4.4 | Archive had no row-level section for cash movements or for refunds paid after the order's year; both reached it only through journal events. | **CLOSED by Batch 3.10**: `refunds` and `cashMovements` sections, keyed on the date the money moved. Schema 4 → 5 |
| **L-56** ✅ | 4.6, 5.3 | Archive notice claimed « date certaine » — and so did the signed attestation. | **CLOSED by Batch 3.10** in both places, with editorial note 6 recording the withdrawal |
...
| **L-58** ✅ | 6.2 | Ticket's « Caisse #N » was the shift number; no stored per-line HT. | **CLOSED, both halves.** Label by Batch 3.10 (`Caisse N° 1` + `Service N`); stored line HT by **Batch 3.11** — `lineHt` + `lineNetTotal`, migration `20260907114611_order_item_line_ht`, applied and verified in production 2026-09-11 (`pragma table_info(OrderItem)` → 18 columns, both present) |
| **L-21** ✅ | 6.1 | `renderReceipt()` centred but never wrapped, so the restaurant's own 56-character address went onto 48-column paper whole. | **CLOSED by Batch 1.3b** (2026-09-07), proved on the real route and at the byte level |
| **L-63** ✅ | 6.1 | `leftRight()` overflowed by a second mechanism; the option and add-on lines were not laid out at all; the same helper was copied into the day-close slip and the printer self-test. | **CLOSED by Batch 1.3c** (2026-09-07). All three renderers share `services/ticket-layout.ts`; **nothing is truncated**, on § 6.2's own list |
