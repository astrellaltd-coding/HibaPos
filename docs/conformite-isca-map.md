...
changes (L-53, L-54)** (line numbers are those of the tree after those changes).
**Research-dependent cells were filled on the same day from the second research
pass** (`docs/conformite-isca-recherche.md` § 9). Companion to
...
| 1.4 | The chain is **unkeyed** | `src/lib/fiscal.ts:77` | No secret, no signature, no external anchor. Anyone with write access to `custom.db` can recompute the whole chain and `GET /api/fiscal/verify` reports `ok`. BOFiP § 100 gives « empreinte numérique à clé privée, chaînage, etc. » and § 140 « chaînage des enregistrements ou de signature électronique »; no official source addresses a chain **without** a key either way. The LNE referential (private, rév. 1.7, Exigence 8) treats every acceptable chaining solution as **keyed (HMAC-SHA-256) or signed, with a key the assujetti cannot know**, and requires a restore to an earlier state to be « détectée ou rendue impossible ». | **? unknown — V-01**, now **indicated against** by the private referential (research § 9.4). Not settled by any official source. |
...
| **L-54** | 3.1 | Daily close is per shift; Z #2 on production spans five calendar days. | **Batch 3.7 — the mislabel and the missing notice fixed**; acceptance of a per-shift close settled by no source (§ 9.5); refusing sales past midnight is **DD-23** |
...
| **L-57** (new) | 3.5 | **The perpetual total is recorded in no close**, against BOFiP § 170's « calculées et enregistrées … le total perpétuel ». | recorded in `REMEDIATION_PLAN.md`, MEDIUM — **before the first real close** |
| **L-58** (new) | 6.2 | Ticket's « Caisse #N » is the shift number; no stored per-line HT. | recorded in `REMEDIATION_PLAN.md`, LOW |
| **V-01** | 1.4, 1.5, 1.6, 2.9 | Unkeyed chain; chain covers the event payload, not the detailed rows; an out-of-app restore is undetected. **Official sources silent; the LNE referential expects a key or signature** (§ 9.4). | external |
