...
> **⚠ NOT FOR THE OWNER’S TRIAL COPY, and that is a deliberate exception.** The operator chose on 2026-09-09 to run the trial with **FACTICE OFF** so the owner’s tickets are not stamped; the copy prepared that day has it off already. **This section still applies in full at the REAL commissioning**, after the trial. And the thing the exception costs: with FACTICE off nothing in the journal distinguishes a trial sale from a real one, so **§ 6 must be run a second time on that machine before the first real sale** (`CLAUDE.md` rule 6).
