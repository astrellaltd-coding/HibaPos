...
echo "[BUN] Applying pending migrations to local DB..."
bun run db:deploy
log_step_end "bun run db:deploy"

# First-boot seed — idempotent no-op if users already exist. Useful when the
# local DB has just been reset or freshly cloned.
log_step_start "bun run db:seed (no-op if already seeded)"
bun run db:seed || echo "[BUN] db:seed skipped (already initialized or env not configured)"
log_step_end "bun run db:seed (no-op if already seeded)"
