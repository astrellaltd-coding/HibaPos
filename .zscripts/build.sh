...
# Ship an EMPTY schema (not the dev test DB) plus migrations so the first boot
# at deploy time runs `prisma migrate deploy` against a clean DB. The previous
# approach copied `./db/custom.db` (with dev test data) into the package —
# bad for prod hygiene + a schema-mismatch clobber risk.
echo "🗄️  Preparing empty prod schema + prisma/migrations for first-boot init..."
mkdir -p "$BUILD_DIR/db"
cp -r ./prisma "$BUILD_DIR/prisma"

# package.json is required at runtime for TWO first-boot steps:
#   - `bunx prisma` resolves the prisma CLI from the dependency manifest
#   - start.sh reads the prisma.seed config (informational)
cp package.json "$BUILD_DIR/package.json"

# Copy .env.example as the prod .env fallback (operators override via env vars at start.sh)
