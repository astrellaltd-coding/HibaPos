...
describe("plan freshness — an open finding may not point at a finished batch", () => {
  it("parses the plan at all, so a silent zero cannot pass this file", () => {
    ...
    expect(openFindings(src).size).toBe(10); // 13 → 12 (L-58) → 11 (DOC-15) → 7 (7.5) → 8 (L-64 by 7.6) → 7 (L-64 closed by 7.7) → 8 (L-67 opened by 5.8) → 7 (L-67 closed by 5.8) → 8 (L-68 opened by 3.12) → 7 (L-68 closed by 3.12) → 8 (L-69 opened by 5.9, and NOT closed by it — safety rule 10) → 10 (L-70 and L-71 opened by 1.3d; L-70 is the batch’s own subject and its code half is done, L-71 is out of scope and owned by nobody)
