...
describe("plan freshness — an open finding may not point at a finished batch", () => {
  it("parses the plan at all, so a silent zero cannot pass this file", () => {
    ...
    expect(openFindings(src).size).toBe(7); // 13 → 12 (L-58) → 11 (DOC-15) → 7 (7.5) → 8 (L-64 by 7.6) → 7 (L-64 closed by 7.7) → 8 (L-67 opened by 5.8) → 7 (L-67 closed by 5.8, row moved to the record)
