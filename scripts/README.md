...
| `empty-catalogue.ts` | ⚠ **L-226 — deletes the whole catalogue**, so that `importCatalogue` can be used on an install that already has one. Until this existed the import's own refusal named an action nothing could perform, and the transfer worked exactly once per installation. **Refuses unless every table `pre-golive-reset.ts` deletes is already empty** — a sealed receipt names its products under their identity (R2.1) and `OrderItem` snapshots `productName` beside a nullable `productId` for that reason, so emptying the catalogue under a history does not break a foreign key, it makes a fiscal document illegible in silence. On a till that has traded the answer is « no » and it says so. Also refuses on a stray `-wal`/`-shm`, asks for **`VIDER`** typed at the prompt, takes a sha-verified restore point, deletes in the reverse of the export's FK-safe order, reads every table back, and journals `CATALOGUE_EMPTIED`. | ⚠ **YES — every catalogue table**: `Category`, `Product`, `OptionGroup`, `OptionChoice`, `CategoryOptionGroup`, `CategoryOptionChoice`, `CategoryAddOn`, `ComboSlot`, `ComboSlotChoice`, `ComboSlotOptionRule`, `ProductOptionQuota`. Users, settings, the audit log and the fiscal journal are untouched. | `bun scripts/empty-catalogue.ts`<br>`bun scripts/empty-catalogue.ts --apply` |
